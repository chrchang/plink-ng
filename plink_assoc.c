...
THREAD_RET_TYPE model_maxt_best_thread(void* arg) {
  ...
  uint32_t case_ct = g_case_ct;
  uintptr_t* __restrict__ loadbuf = g_loadbuf;
  uintptr_t* is_invalid = g_is_invalid_bitfield;
  uint32_t* __restrict__ perm_vecst = g_perm_vecst;
  uint32_t* __restrict__ perm_2success_ct = g_perm_2success_ct;
  uint32_t* __restrict__ precomp_ui = g_precomp_ui;
  uint32_t* __restrict__ precomp_start = g_precomp_start;
...
int32_t model_assoc(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uint32_t model_modifier, uint32_t model_cell_ct, uint32_t model_mperm_val, double ci_size, double ci_zt, double pfilter, uint32_t mtest_adjust, double adjust_lambda, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude_orig, uintptr_t marker_ct_orig, char* marker_ids, uintptr_t max_marker_id_len, uint32_t plink_maxsnp, uint32_t* marker_pos, char** marker_allele_ptrs, uintptr_t max_marker_allele_len, uintptr_t* marker_reverse, uint32_t zero_extra_chroms, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uint32_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, Aperm_info* apip, uint32_t mperm_save, uint32_t pheno_nm_ct, uintptr_t* pheno_nm, uintptr_t* pheno_c, uintptr_t* sex_male, Set_info* sip) {
 ...
 model_assoc_more_perms:
  ...
  do {
	...
	if (model_assoc) {
	  if (spawn_threads(threads, &assoc_maxt_thread, assoc_thread_ct)) {
	    goto model_assoc_ret_THREAD_CREATE_FAIL;
	  }
	  assoc_maxt_thread((void*)ulii);
	} else if (model_modifier & (MODEL_PDOM | MODEL_PREC)) {
	  if (spawn_threads(threads, &model_maxt_domrec_thread, assoc_thread_ct)) {
	    goto model_assoc_ret_THREAD_CREATE_FAIL;
	  }
	  model_maxt_domrec_thread((void*)ulii);
	} else if (model_modifier & MODEL_PTREND) {
	  if (spawn_threads(threads, &model_maxt_trend_thread, assoc_thread_ct)) {
	    goto model_assoc_ret_THREAD_CREATE_FAIL;
	  }
	  model_maxt_trend_thread((void*)ulii);
	} else if (model_modifier & MODEL_PGEN) {
	  if (spawn_threads(threads, &model_maxt_gen_thread, assoc_thread_ct)) {
	    goto model_assoc_ret_THREAD_CREATE_FAIL;
	  }
	  model_maxt_gen_thread((void*)ulii);
	} else {
	  if (spawn_threads(threads, &model_maxt_best_thread, assoc_thread_ct)) {
	    goto model_assoc_ret_THREAD_CREATE_FAIL;
	  }
...
THREAD_RET_TYPE testmiss_maxt_thread(void* arg) {
  ...
  uint32_t is_y = 0;
  uint32_t maxt_block_base = g_maxt_block_base;
  uint32_t marker_bidx_start = (((uint64_t)tidx) * g_block_diff) / g_assoc_thread_ct;
  uint32_t marker_bidx = marker_bidx_start;
  uintptr_t marker_idx = maxt_block_base + marker_bidx_start;
  uintptr_t pheno_nm_ctl = (pheno_nm_ct + (BITCT - 1)) / BITCT;
  uintptr_t pheno_nm_ctv = (pheno_nm_ctl + 1) & (~1);
  uintptr_t perm_vec_ctcl8m = (perm_vec_ct + (CACHELINE_DBL - 1)) & (~(CACHELINE_DBL - 1));
  uint32_t marker_bceil = (((uint64_t)tidx + 1) * g_block_diff) / g_assoc_thread_ct;
  ...
  uint32_t* __restrict__ perm_vecst = g_perm_vecst;
  uint32_t* __restrict__ perm_2success_ct = g_perm_2success_ct;
  uint32_t* precomp_ui = g_precomp_ui;
  uint32_t* __restrict__ missing_cts = g_missing_cts;
  double* __restrict__ results = &(g_maxt_thread_results[perm_vec_ctcl8m * tidx]);
  double* __restrict__ precomp_d = g_precomp_d;
  double* __restrict__ orig_1mpval = g_orig_1mpval;
