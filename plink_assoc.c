...
int32_t multcomp(char* outname, char* outname_end, uint32_t* marker_uidxs, uintptr_t chi_ct, char* marker_ids, uintptr_t max_marker_id_len, uint32_t plink_maxsnp, Chrom_info* chrom_info_ptr, double* chi, double pfilter, uint32_t mtest_adjust, double adjust_lambda, uint32_t non_chi, uint32_t* tcnt) {
  ...
  fputs("\b\b\b", stdout);
  LOGPRINTFWW("--adjust values (%" PRIuPTR " variant%s) written to %s .\n", chi_ct, (chi_ct == 1)? "" : "s", outname);
...
void transpose_perm1s(uintptr_t* perm_vecs, uint32_t perm_vec_ct, uint32_t pheno_nm_ct, uint32_t* perm_vecst) {
  ...
  for (; indiv_idx < pheno_nm_ct; indiv_idx++) {
    perm_idx = 0;
    pvptr = &(perm_vecs[indiv_idx / BITCT]);
    rshift = indiv_idx % BITCT;
...
uintptr_t rem_cost(uintptr_t indiv_ctv2, uintptr_t* loadbuf1, uintptr_t* loadbuf2) {
  ...
  //   B := A & (~detect_homcom)
  //   cost += popcount2(A + B)
  uintptr_t* lptr_end = &(loadbuf1[indiv_ctv2]);
  uintptr_t cost = 0;
  uintptr_t loader;
  uintptr_t loader2;
  uintptr_t xor_word;
...
#else
  uintptr_t* lptr_six_end = &(loadbuf1[indiv_ctv2 - (indiv_ctv2 % 6)]);
...
static uintptr_t* g_indiv_male_include2;
...
THREAD_RET_TYPE assoc_adapt_thread(void* arg) {
  ...
  double* __restrict__ orig_1mpval;
  ...
  while (1) {
	...
	if (uii < precomp_width) {
	  ...
	  if (model_fisher) {
	    dxx = 1.0 - fisher22(case_set_ct, uii - case_set_ct, col1_sum - case_set_ct, col2_sum + case_set_ct - uii, fisher_midp);
	  } else {
...
THREAD_RET_TYPE model_adapt_domrec_thread(void* arg) {
  ...
  while (1) {
	...
	if (uii < precomp_width) {
	  ...
	  if (model_fisher) {
	    dxx = 1.0 - fisher22(case_homx_ct, uii - case_homx_ct, col1_sum - case_homx_ct, col2_sum + case_homx_ct - uii, fisher_midp);
	  } else {
...
int32_t model_assoc_set_test(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, char* outname_end2, uint32_t model_modifier, uint32_t model_mperm_val, double pfilter, uint32_t mtest_adjust, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude_orig, uintptr_t marker_ct_orig, uintptr_t* marker_exclude_mid, uintptr_t marker_ct_mid, char* marker_ids, uintptr_t max_marker_id_len, uintptr_t* marker_reverse, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uintptr_t* sex_male,Aperm_info* apip, uint32_t pheno_nm_ct, uintptr_t* pheno_nm, uintptr_t* pheno_c, uintptr_t* founder_pnm, uint32_t gender_req, uint32_t ld_ignore_x, uint32_t hh_exists, Set_info* sip, uintptr_t* loadbuf_raw) {
  ...
  if (construct_ld_map(threads, bedfile, bed_offset, marker_exclude, marker_ct, marker_reverse, marker_idx_to_uidx, unfiltered_indiv_ct, founder_pnm, sip, set_incl, set_ct, setdefs, outname, outname_end, marker_ids, max_marker_id_len, sex_male, chrom_info_ptr, ld_ignore_x, hh_exists, &ld_map)) {
...
int32_t model_assoc(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uint32_t model_modifier, uint32_t model_cell_ct, uint32_t model_mperm_val, double ci_size, double ci_zt, double pfilter, uint32_t mtest_adjust, double adjust_lambda, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude_orig, uintptr_t marker_ct_orig, char* marker_ids, uintptr_t max_marker_id_len, uint32_t plink_maxsnp, uint32_t* marker_pos, char** marker_allele_ptrs, uintptr_t max_marker_allele_len, uintptr_t* marker_reverse, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uint32_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, Aperm_info* apip, uint32_t mperm_save, uint32_t pheno_nm_ct, uintptr_t* pheno_nm, uintptr_t* pheno_c, uintptr_t* founder_info, uintptr_t* sex_male, uint32_t hh_exists, uint32_t ld_ignore_x, Set_info* sip) {
  unsigned char* wkspace_mark = wkspace_base;
  uintptr_t unfiltered_indiv_ct4 = (unfiltered_indiv_ct + 3) / 4;
  ...
  if (wkspace_alloc_ul_checked(&loadbuf, MODEL_BLOCKSIZE * pheno_nm_ctv2 * sizeof(intptr_t)) ||
      wkspace_alloc_d_checked(&orig_1mpval, marker_ct * sizeof(double)) ||
      wkspace_alloc_ui_checked(&missing_cts, marker_ct * sizeof(int32_t))) {
    goto model_assoc_ret_NOMEM;
  }
  g_loadbuf = loadbuf;
  g_orig_1mpval = orig_1mpval;
  ...
  if (gender_req) {
	...
	wkspace_alloc_ul_checked(&indiv_male_include2, pheno_nm_ctv2 * sizeof(intptr_t))) {
      goto model_assoc_ret_NOMEM;
    }
    g_indiv_male_include2 = indiv_male_include2;
    vec_collapse_init(sex_male, unfiltered_indiv_ct, pheno_nm, pheno_nm_ct, indiv_male_include2);
    male_ct = popcount01_longs(indiv_male_include2, pheno_nm_ctv2);
    vec_init_invert(pheno_nm_ct, g_indiv_nonmale_include2, indiv_male_include2);
  ...
  if (model_perms) {
    if (cluster_starts) {
      retval = cluster_include_and_reindex(unfiltered_indiv_ct, pheno_nm, 1, pheno_c, pheno_nm_ct, 0, cluster_ct, cluster_map, cluster_starts, &g_cluster_ct, &g_cluster_map, &g_cluster_starts, &g_cluster_case_cts, &g_cluster_cc_perm_preimage);
      if (retval) {
	goto model_assoc_ret_1;
      }
      if (!g_cluster_ct) {
  ...
  if (wkspace_alloc_ul_checked(&indiv_ctrl_include2, pheno_nm_ctv2 * sizeof(intptr_t)) ||
      wkspace_alloc_ul_checked(&indiv_case_include2, pheno_nm_ctv2 * sizeof(intptr_t))) {
    goto model_assoc_ret_NOMEM;
  }
  vec_collapse_init(pheno_c, unfiltered_indiv_ct, pheno_nm, pheno_nm_ct, indiv_case_include2);
  case_ct = popcount01_longs(indiv_case_include2, pheno_nm_ctv2);
  g_case_ct = case_ct;
  ...
  if (gender_req) {
    ...
    // permutation tests
    if (wkspace_alloc_ul_checked(&indiv_nonmale_ctrl_include2, pheno_nm_ctv2 * sizeof(intptr_t)) ||
	wkspace_alloc_ul_checked(&indiv_nonmale_case_include2, pheno_nm_ctv2 * sizeof(intptr_t)) ||
	wkspace_alloc_ul_checked(&indiv_male_ctrl_include2, pheno_nm_ctv2 * sizeof(intptr_t)) ||
	wkspace_alloc_ul_checked(&indiv_male_case_include2, pheno_nm_ctv2 * sizeof(intptr_t))) {
      goto model_assoc_ret_NOMEM;
    }
    vec_collapse_init(sex_male, unfiltered_indiv_ct, pheno_nm, pheno_nm_ct, indiv_male_case_include2);
    bitfield_and(indiv_male_case_include2, indiv_case_include2, pheno_nm_ctv2);
    case_male_ct = popcount01_longs(indiv_male_case_include2, pheno_nm_ctv2);
    vec_init_andnot(pheno_nm_ctv2, indiv_male_ctrl_include2, indiv_male_include2, indiv_male_case_include2);
    vec_init_andnot(pheno_nm_ctv2, indiv_nonmale_case_include2, indiv_case_include2, indiv_male_case_include2);
    vec_init_andnot(pheno_nm_ctv2, indiv_nonmale_ctrl_include2, indiv_ctrl_include2, indiv_male_ctrl_include2);
 ...
 model_assoc_more_perms:
  if (model_perms_nst) {
    ...
    perm_vec_ctcl4m = (perm_vec_ct + (CACHELINE_INT32 - 1)) & (~(CACHELINE_INT32 - 1));
  ...
  do {
	...
	g_adapt_m_table[block_size] = marker_idx2++;
      ...
      if (is_x && (!model_assoc)) {
	force_missing((unsigned char*)(&(loadbuf[block_size * pheno_nm_ctv2])), indiv_male_include2, pheno_nm_ct);
      }
    ...
    if (!perm_pass_idx) {
      // basic --assoc/--model
      o1mpptr = &(orig_1mpval[marker_idx + g_block_start]);
	...
	for (marker_bidx = g_block_start; marker_bidx < block_size; marker_bidx++) {
	  ...
	  if (!is_haploid) {
	    if (model_maxt_nst) {
	      ...
	      single_marker_cc_freqs(pheno_nm_ctv2, &(loadbuf[marker_bidx * pheno_nm_ctv2]), indiv_ctrl_include2, indiv_case_include2, &uii, &ujj, &ukk, &umm);
	    }
	    *missp = ujj + umm;
	    *setp = uii + ukk;
	    ...
	    single_marker_cc_freqs(pheno_nm_ctv2, &(loadbuf[marker_bidx * pheno_nm_ctv2]), indiv_nonmale_ctrl_include2, indiv_nonmale_case_include2, &uii, &ujj, &ukk, &umm);
	    *missp = 2 * (ujj + umm);
	    *setp = uii + ukk;
	    ujj = 2 * (ctrl_nonmale_ct - ujj) - uii;
	    umm = 2 * (case_nonmale_ct - umm) - ukk;
	    haploid_single_marker_cc_freqs(pheno_nm_ctv2, &(loadbuf[marker_bidx * pheno_nm_ctv2]), indiv_male_ctrl_include2, indiv_male_case_include2, &unn, &uoo, &upp, &uqq);
	...
	for (marker_bidx = g_block_start; marker_bidx < block_size; marker_bidx++) {
	  marker_uidx2 = mu_table[marker_bidx];
	  single_marker_cc_3freqs(pheno_nm_ctv2, &(loadbuf[marker_bidx * pheno_nm_ctv2]), indiv_ctrl_include2, indiv_case_include2, &uii, &ujj, &ukk, &umm, &unn, &uoo);
	  *missp = ukk + uoo;
	  *setp = uii + umm;
  ...
  if (!perm_pass_idx) {
    ...
    if (!is_set_test) {
	  ...
	  goto model_assoc_ret_NOMEM;
        ...
        fill_idx_to_uidx(marker_exclude, unfiltered_marker_ct, marker_ct, marker_idx_to_uidx);
        retval = multcomp(outname, outname_end, marker_idx_to_uidx, marker_ct, marker_ids, max_marker_id_len, plink_maxsnp, chrom_info_ptr, model_fisher? orig_1mpval : orig_chisq, pfilter, mtest_adjust, adjust_lambda, model_fisher, NULL);
        if (retval) {
	  goto model_assoc_ret_1;
        }
  ...
  if (model_perms_nst) {
	...
	*wptr++ = '\n';
	...
	if (model_fisherx) {
	  for (marker_idx = 0; marker_idx < marker_ct; marker_idx++) {
	    if (orig_1mpval[marker_idx] > dxx) {
	      dxx = orig_1mpval[marker_idx];
	    }
	  }
	  dxx = 1 - dxx;
	} else {
    ...
    */
	...
	if (model_adapt_nst) {
	  ...
	  if ((!model_assoc) && ((model_adapt_nst && (!perm_attempt_ct[marker_idx])) || ((!model_adapt_nst) && ((model_fisherx && (orig_1mpval[marker_idx] == -9)) || ((!model_fisherx) && (orig_chisq[marker_idx] == -9)))))) {
	    // invalid
            wptr = memcpya(wptr, "          NA           NA", 25);
	  } else {
	    if (!model_perm_count) {
	    ...
	    if (model_adapt_nst) {
	      ...
	      if (model_fisherx) {
		// minimum p-value
		dzz = (int32_t)(doublearr_greater_than(maxt_extreme_stat, perms_total, 1.0 + EPSILON - orig_1mpval[marker_idx]) + 1);
	      } else {
		// maximum chisq
...
int32_t qassoc(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uint32_t model_modifier, uint32_t model_mperm_val, double pfilter, uint32_t mtest_adjust, double adjust_lambda, uintptr_t* marker_exclude, uintptr_t marker_ct, char* marker_ids, uintptr_t max_marker_id_len, uint32_t plink_maxsnp, uint32_t* marker_pos, char** marker_allele_ptrs, uintptr_t* marker_reverse, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uint32_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, Aperm_info* apip, uint32_t mperm_save, uint32_t pheno_nm_ct, uintptr_t* pheno_nm, double* pheno_d, uintptr_t* sex_male, uint32_t hh_exists, uint32_t perm_batch_size, Set_info* sip) {
  unsigned char* wkspace_mark = wkspace_base;
  uintptr_t unfiltered_indiv_ct4 = (unfiltered_indiv_ct + 3) / 4;
 ...
 qassoc_more_perms:
  ...
  if (!perm_pass_idx) {
	...
	for (uii = 0; uii < marker_ct; uii++) {
      ...
      retval = multcomp(outname, outname_end, marker_idx_to_uidx, marker_ct, marker_ids, max_marker_id_len, plink_maxsnp, chrom_info_ptr, g_orig_chisq, pfilter, mtest_adjust, adjust_lambda, 1, tcnt);
      if (retval) {
	goto qassoc_ret_1;
      }
    }
...
int32_t gxe_assoc(FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uintptr_t* marker_exclude, uintptr_t marker_ct, char* marker_ids, uintptr_t max_marker_id_len, uint32_t plink_maxsnp, uintptr_t* marker_reverse, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uintptr_t indiv_ct, uintptr_t* indiv_exclude, uintptr_t* pheno_nm, double* pheno_d, uintptr_t* gxe_covar_nm, uintptr_t* gxe_covar_c, uintptr_t* sex_male, uint32_t hh_exists) {
  unsigned char* wkspace_mark = wkspace_base;
  FILE* outfile = NULL;
  uintptr_t unfiltered_indiv_ct4 = (unfiltered_indiv_ct + 3) / 4;
  uintptr_t unfiltered_indiv_ctl = (unfiltered_indiv_ct + (BITCT - 1)) / BITCT;
...
int32_t testmiss(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uint32_t testmiss_mperm_val, uint32_t testmiss_modifier, double pfilter, uint32_t mtest_adjust, double adjust_lambda, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude_orig, uintptr_t marker_ct_orig, char* marker_ids, uintptr_t max_marker_id_len, uint32_t plink_maxsnp, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uint32_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, Aperm_info* apip, uint32_t mperm_save, uint32_t pheno_nm_ct, uintptr_t* pheno_nm, uintptr_t* pheno_c, uintptr_t* sex_male, uint32_t hh_exists) {
  ...
  if (mtest_adjust) {
    ...
    fill_idx_to_uidx(marker_exclude, unfiltered_marker_ct, marker_ct, marker_idx_to_uidx);
    retval = multcomp(outname, outname_end, marker_idx_to_uidx, marker_ct, marker_ids, max_marker_id_len, plink_maxsnp, chrom_info_ptr, g_orig_1mpval, pfilter, mtest_adjust, 0.0, 1, NULL);
    if (retval) {
      goto testmiss_ret_1;
    }
  }
  if (do_perms) {
	...
	collapse_copy_bitarr_incl(unfiltered_indiv_ct, sex_male, pheno_nm, pheno_nm_ct, sex_male_collapsed);
      }
    }
...
int32_t cmh_assoc(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uint32_t cmh_mperm_val, uint32_t cmh_modifier, double ci_size, double ci_zt, double pfilter, uint32_t mtest_adjust, double adjust_lambda, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude, uintptr_t marker_ct, char* marker_ids, uintptr_t max_marker_id_len, uint32_t plink_maxsnp, char** marker_allele_ptrs, uintptr_t* marker_reverse, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uint32_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, Aperm_info* apip, uint32_t mperm_save, uint32_t pheno_nm_ct, uintptr_t* pheno_nm, uintptr_t* pheno_c, uintptr_t* sex_male, uint32_t hh_exists, Set_info* sip) {
  ...
  uint32_t do_perms = cmh_modifier & (CLUSTER_CMH_PERM | CLUSTER_CMH_MPERM);
  uint32_t is_set_test = cmh_modifier & CLUSTER_CMH_SET_TEST;
  uint32_t perm_adapt_nst = (cmh_modifier & CLUSTER_CMH_PERM) && (!is_set_test);
  uint32_t perm_maxt_nst = (cmh_modifier & CLUSTER_CMH_MPERM) && (!is_set_test);
  uint32_t perms_nst = do_perms && (!is_set_test);
  uint32_t mperm_dump_all = 0;
  uint32_t perms_total = 0;
  uint32_t perms_done = 0;
  uint32_t perm_pass_idx = 0;
  int32_t retval = 0;
  uintptr_t pheno_nm_ctv2;
  uintptr_t* pheno_nm2;
  uintptr_t* pheno_c2;
  uintptr_t* loadbuf_raw;
  uintptr_t* loadbuf;
  ...
  // The best data structures for permutation testing are quite different from
  // those for the single-pass computation, so we separate the logic.
  ...
  if (wkspace_alloc_ul_checked(&pheno_nm2, unfiltered_indiv_ctl * sizeof(intptr_t)) ||
      wkspace_alloc_ul_checked(&pheno_c2, unfiltered_indiv_ctl * sizeof(intptr_t)) ||
      wkspace_alloc_ui_checked(&cluster_map2, cluster_starts[cluster_ct] * sizeof(int32_t))) {
    goto cmh_assoc_ret_NOMEM;
  }
  // temporary cluster_starts
  uiptr = (uint32_t*)top_alloc(&topsize, (cluster_ct + 1) * sizeof(int32_t));
  if (!uiptr) {
    goto cmh_assoc_ret_NOMEM;
  }
  ...
  for (cluster_idx = 0; cluster_idx < cluster_ct; cluster_idx++) {
    ...
    for (; uii < cluster_end; uii++) {
      ujj = cluster_map[uii];
      if (is_set(pheno_nm, ujj)) {
	if (is_set(pheno_c, ujj)) {
  ...
  pheno_nm_ct = popcount_longs(pheno_nm2, unfiltered_indiv_ctl);
  memcpy(pheno_c2, pheno_c, unfiltered_indiv_ctl * sizeof(intptr_t));
  bitfield_and(pheno_c2, pheno_nm2, unfiltered_indiv_ctl);
  loadbuf[unfiltered_indiv_ctl2 - 1] = 0;

  retval = cluster_include_and_reindex(unfiltered_indiv_ct, pheno_nm2, 1, pheno_c, pheno_nm_ct, 0, cluster_ct, cluster_map, cluster_starts, &cluster_ct2, &cluster_map2, &cluster_starts2, &g_cluster_case_cts, &g_cluster_cc_perm_preimage);
  if (retval) {
  ...
  if (cluster_ct2 < 2) {
    ...
    goto cmh_assoc_ret_INVALID_CMDLINE;
  } else if (breslow_day && (cluster_ct2 > 10)) {
    logprint("Warning: Breslow-Day statistics are unreliable with small clusters.\n");
  }
  pheno_nm_ctv2 = 2 * ((pheno_nm_ct + (BITCT - 1)) / BITCT);
  g_pheno_nm_ct = pheno_nm_ct;
  g_is_perm1 = 0;
  g_mperm_save_all = NULL;
  // if (is_set_test) {
  ...
  // * set of valid clusters (2+ nonmissing genotypes)
  ...
  //   then be determined via subtraction; but note that [case x A2] CANNOT be
  //   determined from [case x A1] because the number of cases with missing
  //   genotypes may vary)
  //
