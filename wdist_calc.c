...
void exclude_multi(uintptr_t* exclude_arr, int32_t* new_excl, uintptr_t unfiltered_indiv_ct, uintptr_t indiv_ct, uintptr_t* exclude_ct_ptr) {
  int32_t* new_excl_end = &(new_excl[indiv_ct]);
  uintptr_t exclude_ct = *exclude_ct_ptr;
  uintptr_t indiv_uidx = 0;
  int32_t* cur_stop;
  do {
    indiv_uidx = next_non_set_unsafe(exclude_arr, indiv_uidx);
    if (&(new_excl[unfiltered_indiv_ct - indiv_uidx]) < new_excl_end) {
      cur_stop = &(new_excl[next_set_unsafe(exclude_arr, indiv_uidx)]);
    } else {
      cur_stop = new_excl_end;
    }
    do {
      if (*new_excl++ == -1) {
...
int32_t ld_process_load(unsigned char* loadbuf, uintptr_t* geno_buf, uintptr_t* mask_buf, uintptr_t* missing_buf, double* marker_stdev_ptr, uintptr_t unfiltered_indiv_ct, uintptr_t* indiv_exclude, uintptr_t indiv_ct, uint32_t indiv_ctl, uint32_t indiv_trail_ct, uint32_t is_haploid, uint32_t is_x, uintptr_t* sex_male) {
...
int32_t ld_prune(FILE* bedfile, uintptr_t bed_offset, uint32_t marker_ct, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude, char* marker_ids, uintptr_t max_marker_id_len, Chrom_info* chrom_info_ptr, double* set_allele_freqs, uint32_t* marker_pos, uintptr_t unfiltered_indiv_ct, uintptr_t* indiv_exclude, uintptr_t* sex_male, uint32_t ld_window_size, uint32_t ld_window_kb, uint32_t ld_window_incr, double ld_last_param, char* outname, char* outname_end, uint64_t calculation_type) {
  ...
  do {
    ...
    putchar('\r');
    sprintf(logbuf, "Pruned %" PRIuPTR " markers from chromosome %d, leaving %" PRIuPTR ".\n", cur_exclude_ct, uii, chrom_info_ptr->chrom_end[uii] - chrom_info_ptr->chrom_start[uii] - cur_exclude_ct);
    logprintb();
...
int32_t do_rel_cutoff(uint64_t calculation_type, double rel_cutoff, double* rel_ibc, uintptr_t* indiv_exclude, uintptr_t* indiv_exclude_ct_ptr, char* outname, char* outname_end, uintptr_t unfiltered_indiv_ct, char* person_ids, uintptr_t max_person_id_len) {
  ...
  exclude_multi(indiv_exclude, rel_ct_arr, unfiltered_indiv_ct, g_indiv_ct, indiv_exclude_ct_ptr);
  if (indivs_excluded) {
...
int32_t calc_cluster_neighbor(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, uint32_t marker_ct, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude, Chrom_info* chrom_info_ptr, double* set_allele_freqs, uintptr_t unfiltered_indiv_ct, uintptr_t* indiv_exclude, char* person_ids, uint32_t plink_maxfid, uint32_t plink_maxiid, uintptr_t max_person_id_len, char* read_dists_fname, char* read_dists_id_fname, char* read_genome_fname, char* outname, char* outname_end, uint64_t calculation_type, uintptr_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, Cluster_info* cp, int32_t missing_pheno, uint32_t neighbor_n1, uint32_t neighbor_n2, uint32_t ppc_gap, uintptr_t* pheno_c, double* mds_plot_dmatrix_copy, uintptr_t* cluster_merge_prevented, double* cluster_sorted_ibs, unsigned char* wkspace_mark_precluster, unsigned char* wkspace_mark_postcluster) {
  ...
  if ((min_ppc != 0.0) || g_genome_main || read_genome_fname) {
	...
	for (indiv_idx2 = indiv_idx1 + 1; indiv_idx2 < indiv_ct; indiv_idx2++) {
	  ...
	  if (min_ppc != 0.0) {
		...
		ppc_fail_counts[indiv_idx2] += 1;
	      ...
	      if (!cluster_ct) {
		set_bit(cluster_merge_prevented, tri_coord_no_diag(indiv_idx1, indiv_idx2));
	      } else {
		if (clidx1 != clidx2) {
    ...
    // memory
    if (!g_dists) {
      retval = calc_distance(threads, 0, 1, bedfile, bed_offset, outname, outname_end, 0, DISTANCE_FLAT_MISSING | DISTANCE_CLUSTER, marker_exclude, marker_ct, set_allele_freqs, unfiltered_indiv_ct, indiv_exclude, person_ids, max_person_id_len, chrom_info_ptr, 0, NULL, 0.0);
      if (retval) {
        goto calc_cluster_neighbor_ret_1;
      }
      if (!cluster_ct) {
  ...
  if (cluster_missing || ibm_constraint) {
	...
	if (!cluster_ct) {
	  for (indiv_idx2 = 0; indiv_idx2 < indiv_idx1; indiv_idx2++) {
	    ...
	    if (dxx < min_ibm) {
	      set_bit(cluster_merge_prevented, tri_coord_no_diag(indiv_idx2, indiv_idx1));
	    }
	  }
	} else {
	  clidx1 = indiv_to_cluster[indiv_idx1];
	  for (indiv_idx2 = 0; indiv_idx2 < indiv_idx1; indiv_idx2++) {
		...
		*dptr++ = dxx;
	    ...
	    if (dxx < min_ibm) {
	      if (clidx1 < clidx2) {
		set_bit(cluster_merge_prevented, tri_coord_no_diag(clidx1, clidx2));
	      } else if (clidx1 > clidx2) {
		set_bit(cluster_merge_prevented, tri_coord_no_diag(clidx2, clidx1));
	      } else {
		ibm_warning = 1;
	      }
	    }
