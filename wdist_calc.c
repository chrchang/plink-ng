...
void collapse_copy_phenod(double* target, double* pheno_d, uintptr_t* indiv_exclude, uintptr_t unfiltered_indiv_ct, uintptr_t indiv_ct) {
  uintptr_t indiv_uidx = 0;
  double* target_end = &(target[indiv_ct]);
  uintptr_t delta;
  do {
    indiv_uidx = next_unset_ul_unsafe(indiv_exclude, indiv_uidx);
    delta = next_set_ul(indiv_exclude, indiv_uidx, unfiltered_indiv_ct) - indiv_uidx;
...
double fill_psbuf(uintptr_t block_size, double* dists, uintptr_t* col_uidxp, double* psbuf, double* ssq0p) {
  ...
  do {
    ...
    subtot = 0.0;
    do {
      if (!IS_SET(g_pheno_nm, col_uidx)) {
        col_uidx = next_set_ul_unsafe(g_pheno_nm, col_uidx);
      }
      if (g_load_dists) {
	dxx = dists[col_uidx];
      } else {
...
void ibs_test_range(uint32_t tidx, uintptr_t* perm_col_buf, double* perm_results) {
  ...
  for (row_idx = 0; row_idx < g_thread_start[tidx]; row_uidx++, row_idx++) {
    if (!IS_SET(g_pheno_nm, row_uidx)) {
      row_uidx = next_set_ul_unsafe(g_pheno_nm, row_uidx);
    }
  }
...
void reml_em_one_trait(double* wkbase, double* pheno, double* covg_ref, double* covr_ref, double tol, uint32_t is_strict) {
  ...
  do {
    ...
    dyy = 1.0;
    dzz = 0.0;
    col_major_matrix_multiply(indiv_ct_i32, indiv_ct_i32, indiv_ct_i32, wkbase, rel_dists, matrix_pvg);
    // blas_char = 'N';
...
int32_t calc_unrelated_herit(uint64_t calculation_type, int32_t ibc_type, uintptr_t unfiltered_indiv_ct, uintptr_t* indiv_exclude, uintptr_t indiv_ct, double* pheno_d, double* rel_ibc, uint32_t is_strict, double unrelated_herit_covg, double unrelated_herit_covr, double unrelated_herit_tol) {
  ...
  collapse_copy_phenod(pheno_ptr, pheno_d, indiv_exclude, unfiltered_indiv_ct, indiv_ct);
	...
	fpos = indiv_uidx * (indiv_uidx + 1) * (sizeof(float) / 2);
      ...
      for (uljj = 0; uljj <= ulii; uljj++) {
        if (!IS_SET(pheno_nm, indiv_uidx2)) {
          indiv_uidx2 = next_set_ul_unsafe(pheno_nm, indiv_uidx2);
...
void normalize_phenos(double* new_phenos, uint32_t indiv_ct, uintptr_t* indiv_exclude, uintptr_t* sex_nm, uintptr_t* sex_male, uint32_t sex_exclude) {
  ...
  uint32_t pheno_ct = 0;
  uint32_t indiv_idx = 0xffffffffU;
  double dxx;
  double mean;
  double stdev_recip;
  ...
  indiv_uidx = 0xffffffffU;
  indiv_idx = 0xffffffffU;
  while ((++indiv_idx) < indiv_ct) {
    indiv_uidx++;
    if (IS_SET(indiv_exclude, indiv_uidx)) {
      indiv_uidx = next_unset_unsafe(indiv_exclude, indiv_uidx);
...
int32_t calc_regress_pcs(char* evecname, uint32_t regress_pcs_modifier, uint32_t max_pcs, FILE* bedfile, uintptr_t bed_offset, uint32_t marker_ct, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude, char* marker_ids, uintptr_t max_marker_id_len, char* marker_alleles, uintptr_t max_marker_allele_len, uint32_t zero_extra_chroms, Chrom_info* chrom_info_ptr, uint32_t* marker_pos, uintptr_t indiv_ct, uintptr_t unfiltered_indiv_ct, uintptr_t* indiv_exclude, char* person_ids, uintptr_t max_person_id_len, uintptr_t* sex_nm, uintptr_t* sex_male, double* pheno_d, double missing_phenod, char* outname, char* outname_end) {
  ...
  if (regress_pcs_modifier & REGRESS_PCS_NORMALIZE_PHENO) {
    if (regress_pcs_sex_specific) {
      normalize_phenos(residual_vec, indiv_ct, indiv_exclude, sex_nm, sex_male, 1);
      normalize_phenos(residual_vec, indiv_ct, indiv_exclude, sex_nm, sex_male, 2);
      normalize_phenos(residual_vec, indiv_ct, indiv_exclude, sex_nm, sex_male, 4);
    } else {
      normalize_phenos(residual_vec, indiv_ct, indiv_exclude, sex_nm, sex_male, 7);
    }
  }
...
int32_t calc_genome(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, uint32_t marker_ct, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude, Chrom_info* chrom_info_ptr, uint32_t* marker_pos, double* set_allele_freqs, uintptr_t unfiltered_indiv_ct, uintptr_t* indiv_exclude, char* person_ids, uint32_t plink_maxfid, uint32_t plink_maxiid, uintptr_t max_person_id_len, char* paternal_ids, uintptr_t max_paternal_id_len, char* maternal_ids, uintptr_t max_maternal_id_len, uintptr_t* founder_info, uint32_t parallel_idx, uint32_t parallel_tot, char* outname, char* outname_end, int32_t nonfounders, uint64_t calculation_type, uint32_t genome_modifier, uint32_t ppc_gap, double min_pi_hat, double max_pi_hat, uintptr_t* pheno_nm, uintptr_t* pheno_c, Pedigree_rel_info pri, uint32_t skip_write) {
  ...
  for (uii = 0; uii <= g_thread_ct / 2; uii++) {
    ...
    g_thread_start[g_thread_ct - uii] = g_indiv_ct - ujj;
  }
  ...
  fill_uint_zero(g_missing_dbl_excluded, tot_cells);
  fill_uint_zero(g_indiv_missing_unwt, g_indiv_ct);
  fill_uint_zero(g_genome_main, tot_cells * 5);
  ...
  do {
    ...
    if (ukk < GENOME_MULTIPLEX) {
      memset(&(loadbuf[ukk * unfiltered_indiv_ct4]), 0, (GENOME_MULTIPLEX - ukk) * unfiltered_indiv_ct4);
	...
	*glptr3 = ulkk;
	...
	ulii = 0;
	ulkk = 0;
	gptr = &(loadbuf[indiv_uidx / 4 + (ujj + BITCT2) * unfiltered_indiv_ct4]);
	if (uqq) {
	  for (upp = 0; upp < BITCT2; upp++) {
	    uljj = (gptr[upp * unfiltered_indiv_ct4] >> uoo) & 3;
	    ulii |= uljj << (upp * 2);
	...
	*glptr3++ |= ulkk << BITCT2;
	...
	dqq = 1.0 - dpp;
	num_alleles = (double)(2 * (g_indiv_ct - missing_ct_buf[umm] - missing_ct_all));
...
uint32_t ld_process_load(uintptr_t* geno_buf, uintptr_t* mask_buf, uintptr_t* missing_buf, double* marker_stdev_ptr, uintptr_t indiv_ct, uint32_t indiv_ctl) {
  uintptr_t* geno_end = &(geno_buf[(indiv_ct + (BITCT2 - 1)) / BITCT2]);
...
int32_t ld_prune(FILE* bedfile, uintptr_t bed_offset, uint32_t marker_ct, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude, char* marker_ids, uintptr_t max_marker_id_len, Chrom_info* chrom_info_ptr, double* set_allele_freqs, uint32_t* marker_pos, uintptr_t unfiltered_indiv_ct, uintptr_t* indiv_exclude, uintptr_t* sex_male, uint32_t ld_window_size, uint32_t ld_window_kb, uint32_t ld_window_incr, double ld_last_param, char* outname, char* outname_end, uint64_t calculation_type, uint32_t hh_exists) {
  ...
  uint32_t indiv_ct_mld_m1 = ((uint32_t)indiv_ct_mld) - 1;
#ifdef __LP64__
  uint32_t indiv_ct_mld_rem = (MULTIPLEX_LD / 192) - (indiv_ct_mld * MULTIPLEX_LD - g_indiv_ct) / 192;
#else
  uint32_t indiv_ct_mld_rem = (MULTIPLEX_LD / 48) - (indiv_ct_mld * MULTIPLEX_LD - g_indiv_ct) / 48;
#endif
  uintptr_t indiv_ct_mld_long = indiv_ct_mld * (MULTIPLEX_LD / BITCT2);
  uint32_t indiv_trail_ct = indiv_ct_mld_long - indiv_ctl * 2;
  int32_t retval = 0;
  unsigned char* wkspace_mark = wkspace_base;
  ...
  if (alloc_collapsed_haploid_filters(unfiltered_indiv_ct, g_indiv_ct, hh_exists, 0, indiv_exclude, sex_male, &indiv_include2, &indiv_male_include2)) {
  ...
  if (indiv_trail_ct) {
    for (ulii = 1; ulii <= window_max; ulii++) {
      fill_ulong_zero(&(geno[ulii * indiv_ct_mld_long - indiv_trail_ct - 2]), indiv_trail_ct + 2);
      fill_ulong_zero(&(g_masks[ulii * indiv_ct_mld_long - indiv_trail_ct - 2]), indiv_trail_ct + 2);
    }
  }
  if (!pairwise) {
  ...
  do {
	...
	if (load_and_collapse(bedfile, loadbuf, unfiltered_indiv_ct, &(geno[ulii * indiv_ct_mld_long]), g_indiv_ct, indiv_exclude, 0)) {
	  goto ld_prune_ret_READ_FAIL;
	}
	if (is_haploid && hh_exists) {
	  haploid_fix(hh_exists, indiv_include2, indiv_male_include2, g_indiv_ct, is_x, is_y, (unsigned char*)(&(geno[ulii * indiv_ct_mld_long])));
	}
        missing_cts[ulii] = ld_process_load(&(geno[ulii * indiv_ct_mld_long]), &(g_masks[ulii * indiv_ct_mld_long]), &(g_mmasks[ulii * indiv_ctl]), &(marker_stdevs[ulii]), g_indiv_ct, indiv_ctl);
      }
    }
    pct = 1;
	...
	do {
	  ...
	  for (uii = 0; uii < cur_window_size - 1; uii++) {
	    ...
	    fixed_non_missing_ct = g_indiv_ct - missing_cts[uii];
#ifdef __LP64__
	    geno_fixed_vec_ptr = (__m128i*)(&(geno[uii * indiv_ct_mld_long]));
	    mask_fixed_vec_ptr = (__m128i*)(&(g_masks[uii * indiv_ct_mld_long]));
#else
	    geno_fixed_vec_ptr = &(geno[uii * indiv_ct_mld_long]);
	    mask_fixed_vec_ptr = &(g_masks[uii * indiv_ct_mld_long]);
#endif
	    ujj = uii + 1;
		...
		continue;
...
#ifdef __LP64__
	      geno_var_vec_ptr = (__m128i*)(&(geno[ujj * indiv_ct_mld_long]));
	      mask_var_vec_ptr = (__m128i*)(&(g_masks[ujj * indiv_ct_mld_long]));
#else
	      geno_var_vec_ptr = &(geno[ujj * indiv_ct_mld_long]);
	      mask_var_vec_ptr = &(g_masks[ujj * indiv_ct_mld_long]);
#endif

	      non_missing_ct = fixed_non_missing_ct - missing_cts[ujj] + sparse_intersection_ct(&(g_mmasks[uii * indiv_ctl]), &(g_mmasks[ujj * indiv_ctl]), indiv_ctl);
	      ...
	      dp_result[2] = missing_cts[ujj] - g_indiv_ct;
#ifdef __LP64__
	      for (ukk = 0; ukk < indiv_ct_mld_m1; ukk++) {
		...
		mask_var_vec_ptr = &(mask_var_vec_ptr[MULTIPLEX_LD / BITCT]);
	      ...
	      ld_dot_prod(geno_var_vec_ptr, &(geno_fixed_vec_ptr[ukk * (MULTIPLEX_LD / BITCT)]), mask_var_vec_ptr, &(mask_fixed_vec_ptr[ukk * (MULTIPLEX_LD / BITCT)]), dp_result, indiv_ct_mld_rem);
	      geno_var_vec_ptr = &(geno_var_vec_ptr[MULTIPLEX_LD / BITCT]);
	      mask_var_vec_ptr = &(mask_var_vec_ptr[MULTIPLEX_LD / BITCT]);
#else
	      for (ukk = 0; ukk < indiv_ct_mld_m1; ukk++) {
		...
		mask_var_vec_ptr = &(mask_var_vec_ptr[MULTIPLEX_LD / BITCT2]);
	      ...
	      ld_dot_prod(geno_var_vec_ptr, &(geno_fixed_vec_ptr[ukk * (MULTIPLEX_LD / BITCT2)]), mask_var_vec_ptr, &(mask_fixed_vec_ptr[ukk * (MULTIPLEX_LD / BITCT2)]), dp_result, indiv_ct_mld_rem);
	      geno_var_vec_ptr = &(geno_var_vec_ptr[MULTIPLEX_LD / BITCT2]);
	      mask_var_vec_ptr = &(mask_var_vec_ptr[MULTIPLEX_LD / BITCT2]);
#endif
	...
	memcpy(&(geno[uii * indiv_ct_mld_long]), &(geno[ujj * indiv_ct_mld_long]), indiv_ct_mld_long * sizeof(intptr_t));
	memcpy(&(g_masks[uii * indiv_ct_mld_long]), &(g_masks[ujj * indiv_ct_mld_long]), indiv_ct_mld_long * sizeof(intptr_t));
	memcpy(&(g_mmasks[uii * indiv_ctl]), &(g_mmasks[ujj * indiv_ctl]), indiv_ctl * sizeof(intptr_t));
	...
	if (window_unfiltered_end < chrom_end) {
	  ...
	  if (load_and_collapse(bedfile, loadbuf, unfiltered_indiv_ct, &(geno[cur_window_size * indiv_ct_mld_long]), g_indiv_ct, indiv_exclude, 0)) {
	    goto ld_prune_ret_READ_FAIL;
	  }
	  if (is_haploid && hh_exists) {
	    haploid_fix(hh_exists, indiv_include2, indiv_male_include2, g_indiv_ct, is_x, is_y, (unsigned char*)(&(geno[cur_window_size * indiv_ct_mld_long])));
	  }
	  missing_cts[cur_window_size] = ld_process_load(&(geno[cur_window_size * indiv_ct_mld_long]), &(g_masks[cur_window_size * indiv_ct_mld_long]), &(g_mmasks[cur_window_size * indiv_ctl]), &(marker_stdevs[cur_window_size]), g_indiv_ct, indiv_ctl);
	  cur_window_size++;
	  window_unfiltered_end++;
	}
      }
...
;
  ...
  if (calculation_type & CALC_IBC) {
	...
	indiv_uidx = next_unset_unsafe(indiv_exclude, indiv_uidx);
      ...
      fam_id = &(person_ids[indiv_uidx * max_person_id_len]);
  ...
  if (calculation_type & CALC_RELATIONSHIP) {
    ...
    if (calculation_type & CALC_IBC) {
      dptr2 = &(rel_ibc[((uint32_t)ibc_type) * g_indiv_ct + min_indiv]);
    } else {
...
int32_t calc_ibm(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, uintptr_t* marker_exclude, uint32_t marker_ct, uintptr_t unfiltered_indiv_ct, uintptr_t* indiv_exclude, Chrom_info* chrom_info_ptr) {
  uintptr_t unfiltered_indiv_ct4 = (unfiltered_indiv_ct + 3) / 4;
  uintptr_t marker_uidx = 0;
  uintptr_t marker_idx = 0;
  uint32_t chrom_fo_idx = 0;
...
int32_t calc_distance(pthread_t* threads, uint32_t parallel_idx, uint32_t parallel_tot, FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uint64_t calculation_type, uint32_t dist_calc_type, uintptr_t* marker_exclude, uint32_t marker_ct, double* set_allele_freqs, uintptr_t unfiltered_indiv_ct, uintptr_t* indiv_exclude, char* person_ids, uintptr_t max_person_id_len, Chrom_info* chrom_info_ptr, uint32_t wt_needed, uint32_t marker_weight_sum, uint32_t* marker_weights_i, double exponent) {
  ...
  if (wt_needed) {
    if (wkspace_alloc_ui_checked(&g_missing_tot_weights, llxx * sizeof(int32_t)) ||
        wkspace_alloc_ui_checked(&g_indiv_missing, g_indiv_ct * sizeof(int32_t))) {
      goto calc_distance_ret_NOMEM;
    }
    fill_uint_zero(g_missing_tot_weights, llxx);
    fill_uint_zero(g_indiv_missing, g_indiv_ct);
  }
...
int32_t calc_cluster_neighbor(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, uint32_t marker_ct, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude, Chrom_info* chrom_info_ptr, double* set_allele_freqs, uintptr_t unfiltered_indiv_ct, uintptr_t* indiv_exclude, char* person_ids, uint32_t plink_maxfid, uint32_t plink_maxiid, uintptr_t max_person_id_len, char* read_dists_fname, char* read_dists_id_fname, char* read_genome_fname, char* outname, char* outname_end, uint64_t calculation_type, uintptr_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, Cluster_info* cp, int32_t missing_pheno, uint32_t neighbor_n1, uint32_t neighbor_n2, uint32_t ppc_gap, uintptr_t* pheno_c, double* mds_plot_dmatrix_copy, uintptr_t* cluster_merge_prevented, double* cluster_sorted_ibs, unsigned char* wkspace_mark_precluster, unsigned char* wkspace_mark_postcluster) {
  ...
  if (cluster_ct) {
    ...
    indiv_to_cluster = (uint32_t*)malloc(indiv_ct * sizeof(int32_t));
    if (!indiv_to_cluster) {
  ...
  if ((min_ppc != 0.0) || g_genome_main || read_genome_fname) {
    if (do_neighbor && (min_ppc != 0.0)) {
      ...
      fill_uint_zero(ppc_fail_counts, indiv_ct);
    }
    if (read_genome_fname) {
      retval = read_genome(read_genome_fname, unfiltered_indiv_ct, indiv_exclude, indiv_ct, person_ids, max_person_id_len, cluster_merge_prevented, use_genome_dists? cluster_sorted_ibs : NULL, neighbor_n2, neighbor_quantiles, neighbor_qindices, ppc_fail_counts, min_ppc, cluster_ct && (!is_group_avg), cluster_ct, cluster_starts, indiv_to_cluster);
      if (retval) {
	goto calc_cluster_neighbor_ret_1;
      }
    } else {
	...
	for (indiv_idx2 = indiv_idx1 + 1; indiv_idx2 < indiv_ct; indiv_idx2++) {
	  if (cluster_ct) {
		...
		if (!is_group_avg) {
	      ...
	      cluster_sorted_ibs[tri_coord_no_diag(indiv_idx1, indiv_idx2)] = dxx;
	    }
	    if (mds_plot_dmatrix_copy) {
              mds_plot_dmatrix_copy[tri_coord_no_diag(indiv_idx1, indiv_idx2)] = dxx;
	    }
	  }
	  if (min_ppc != 0.0) {
    ...
    // memory
    if (!g_dists) {
      retval = calc_distance(threads, 0, 1, bedfile, bed_offset, outname, outname_end, 0, DISTANCE_FLAT_MISSING | DISTANCE_CLUSTER, marker_exclude, marker_ct, set_allele_freqs, unfiltered_indiv_ct, indiv_exclude, person_ids, max_person_id_len, chrom_info_ptr, 0, 0, NULL, 0.0);
      if (retval) {
        goto calc_cluster_neighbor_ret_1;
      }
      if (!cluster_ct) {
	...
	for (indiv_idx2 = 0; indiv_idx2 < indiv_idx1; indiv_idx2++) {
	  update_neighbor(indiv_ct, neighbor_n2, indiv_idx1, indiv_idx2, *dptr++, neighbor_quantiles, neighbor_qindices);
	}
      }
    }
    if (mds_fill_nonclust) {
      memcpy(mds_plot_dmatrix_copy, g_dists, (indiv_ct * (indiv_ct - 1)) * (sizeof(double) / 2));
    }
    if (cluster_ct) {
      dptr = g_dists;
      if (!is_group_avg) {
  ...
  if (read_dists_fname) {
    retval = read_dists(read_dists_fname, read_dists_id_fname, unfiltered_indiv_ct, indiv_exclude, indiv_ct, person_ids, max_person_id_len, cluster_ct, cluster_starts, indiv_to_cluster, (calculation_type & (CALC_IBS_TEST | CALC_GROUPDIST | CALC_REGRESS_DISTANCE))? 1 : 0, cluster_ct && (!is_group_avg), cluster_sorted_ibs, neighbor_n2, neighbor_quantiles, neighbor_qindices);
    if (retval) {
      goto calc_cluster_neighbor_ret_1;
    }
  }
  ...
  if (do_neighbor) {
    ...
    dptr = &(neighbor_quantiles[(neighbor_n1 - 1) * indiv_ct]);
    for (indiv_idx1 = neighbor_n1 - 1; indiv_idx1 < neighbor_n2; indiv_idx1++) {
      dxx = 0.0; // sum
      dxx1 = 0.0; // ssq
      for (indiv_idx2 = 0; indiv_idx2 < indiv_ct; indiv_idx2++) {
        dyy = *dptr++;
        dxx += dyy;
        dxx1 += dyy * dyy;
      }
      dyy = dxx * indiv_ct_recip;
      neighbor_quantile_means[indiv_idx1 + 1 - neighbor_n1] = dyy;
      neighbor_quantile_stdev_recips[indiv_idx1 + 1 - neighbor_n1] = sqrt(((double)((intptr_t)(indiv_ct - 1))) / (dxx1 - dxx * dyy));
    }
    if (min_ppc != 0.0) {
    ...
    for (indiv_idx1 = 0; indiv_idx1 < indiv_ct; indiv_idx1++) {
      ...
      *wptr_start++ = ' ';
      wptr_start = fw_strcpy(12, &(indiv_id[1]), wptr_start);
      *wptr_start++ = ' ';
      if (min_ppc != 0.0) {
        dyy = ((double)((int32_t)ppc_fail_counts[indiv_idx1])) * dxx1;
      }
      for (ulii = 0; ulii < neighbor_row_ct; ulii++) {
        wptr = uint32_writew6x(wptr_start, ulii + neighbor_n1, ' ');
	indiv_idx2 = indiv_idx1 + ulii * indiv_ct;
        dxx = neighbor_quantiles[indiv_idx2];
	...
	indiv_idx2 = neighbor_qindices[indiv_idx2];
        ...
        *wptr++ = ' ';
        wptr = fw_strcpy(12, &(indiv_id[1]), wptr);
        *wptr++ = ' ';
	if (min_ppc != 0.0) {
  ...
  if (is_group_avg) {
    cluster_dist_divide(indiv_ct, cluster_ct, cluster_starts, cluster_sorted_ibs);
  }
  ...
  if (cp->match_fname || cp->qmatch_fname) {
    retval = cluster_enforce_match(cp, missing_pheno, unfiltered_indiv_ct, indiv_exclude, indiv_ct, person_ids, max_person_id_len, cluster_ct, cluster_starts, indiv_to_cluster, cluster_merge_prevented);
    if (retval) {
      goto calc_cluster_neighbor_ret_1;
    }
  }
  ...
  if (cp->mds_dim_ct) {
	...
	fill_double_zero(mds_plot_dmatrix_copy, (ulii * (ulii - 1)) / 2);
      ...
      if (read_dists_fname) {
	retval = read_dists(read_dists_fname, read_dists_id_fname, unfiltered_indiv_ct, indiv_exclude, indiv_ct, person_ids, max_person_id_len, is_mds_cluster? cluster_ct : 0, is_mds_cluster? cluster_starts : NULL, is_mds_cluster? indiv_to_cluster : NULL, 2, 0, mds_plot_dmatrix_copy, 0, NULL, NULL);
      } else {
	retval = read_genome(read_genome_fname, unfiltered_indiv_ct, indiv_exclude, indiv_ct, person_ids, max_person_id_len, NULL, mds_plot_dmatrix_copy, 0, NULL, NULL, NULL, 0.0, 0, is_mds_cluster? cluster_ct : 0, is_mds_cluster? cluster_starts : NULL, is_mds_cluster? indiv_to_cluster : NULL);
      }
      if (retval) {
	goto calc_cluster_neighbor_ret_1;
      }
