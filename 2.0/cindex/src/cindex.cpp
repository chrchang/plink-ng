...
double CIndex(NumericVector yhat, NumericVector y, SEXP status) {
  ...
  //    constant multiplier is so small that the std::sort calls always
  //    dominate for R's N<2^31 domain.
  // More precisely, the new_yhat data structure consists of a bitarray, an
  // array of per-cacheline bitarray popcounts, and a second-level array of
  // first-array-cacheline partial sums.
  ...
  const uintptr_t new_yhat_2048b_popcounts_vec_ct = plink2::DivUp(new_yhat_64b_popcounts_cacheline_ct, plink2::kInt16PerVec);
  const uintptr_t vec_ct = recs_vec_ct + new_yhat_present_vec_ct + new_yhat_64b_popcounts_vec_ct + new_yhat_2048b_popcounts_vec_ct;
  ...
  for (uint32_t recs_idx = size; recs_idx; ) {
    ...
    const uint32_t idx_2048b = yhat_int / (plink2::kBitsPerCacheline * plink2::kInt16PerCacheline);
    uint32_t n_set_bits_before = 0;
    for (uint32_t uii = 0; uii != idx_2048b; ++uii) {
      n_set_bits_before += new_yhat_2048b_popcounts[uii];
    }

    // Each entry in the middle array is <= 512, and we're adding at most 16
    // of them even when compiling as 32-bit, so there's no overflow danger if
    // we add entire words at a time (effectively performing 2-4 uint16
    // additions in parallel).
    const uintptr_t* walias_64b_popcounts = reinterpret_cast<const uintptr_t*>(new_yhat_64b_popcounts);
    const uint32_t widx_64b = yhat_int / (plink2::kBitsPerCacheline * plink2::kInt16PerWord);
    uintptr_t acc = 0;
    for (uint32_t uii = idx_2048b * plink2::kWordsPerCacheline; uii != widx_64b; ++uii) {
      acc += walias_64b_popcounts[uii];
    }
    const uint32_t idx_64b = yhat_int / plink2::kBitsPerCacheline;
    uint32_t remainder = idx_64b % plink2::kInt16PerWord;
    if (remainder) {
      acc += plink2::bzhi(walias_64b_popcounts[widx_64b], remainder * 16);
    }
    n_set_bits_before += (acc * plink2::kMask0001) >> (plink2::kBitsPerWord - 16);

    uintptr_t widx = yhat_int / plink2::kBitsPerWord;
    for (uint32_t uii = idx_64b * plink2::kWordsPerCacheline; uii != widx; ++uii) {
      n_set_bits_before += plink2::PopcountWord(new_yhat_present[uii]);
    }
    remainder = yhat_int % plink2::kBitsPerWord;
    uintptr_t cur_word = new_yhat_present[widx];
