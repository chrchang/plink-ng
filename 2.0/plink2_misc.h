...
// PglErr HardyMultiallelic(const uintptr_t* founder_info, const uintptr_t* sex_nm, const uintptr_t* sex_male, const uintptr_t* variant_include, const ChrInfo* cip, const uintptr_t* allele_idx_offsets, const double* allele_freqs, uint32_t raw_sample_ct, uint32_t autosomal_variant_ct, uint32_t hwe_x_ct, uint32_t hwe_multiallelic_flags, PgenReader* simple_pgrp, STD_ARRAY_PTR_DECL(uint32_t, 3, hwe_geno_cts), STD_ARRAY_PTR_DECL(uint32_t, 6, hwe_triallelic_geno_cts), double* hwe_triallelic_pvals, STD_ARRAY_PTR_DECL(uint32_t, 3, hwe_x_male_geno_cts), STD_ARRAY_PTR_DECL(uint32_t, 3, hwe_x_nosex_geno_cts));
