...
pglerr_t vcf_sample_line(const char* preexisting_psamname, const char* const_fid, uint32_t double_id, fam_col_t fam_cols, char id_delim, char idspace_to, char flag_char, char* sample_line_first_id, char* outname, char* outname_end, uintptr_t* sample_ct_ptr) {
  ...
  pglerr_t reterr = kPglRetSuccess;
  {
	...
	while (((unsigned char)sample_line_iter[0]) >= ' ') {
	  ...
	  if (!token_end) {
	    token_end = sample_line_iter;
	    do {
	    } while ((unsigned char)(*(++token_end) >= 32));
	  }
	...
	if (!is_eoln_kns(*loadbuf_first_token)) {
	  ...
	  if (!sample_line_token_end) {
	    sample_line_token_end = sample_line_iter;
	    do {
	    } while ((unsigned char)(*(++sample_line_token_end)) >= 32);
	  }
...
pglerr_t vcf_to_pgen(const char* vcfname, const char* preexisting_psamname, const char* const_fid, misc_flags_t misc_flags, __attribute__((unused)) uint32_t dosage_erase_thresh, __attribute__((unused)) double import_dosage_certainty, char id_delim, char idspace_to, double vcf_min_gq, vcf_half_call_t vcf_half_call, __attribute__((unused)) uint32_t vcf_dosage_import_field, fam_col_t fam_cols, char* outname, char* outname_end, chr_info_t* cip) {
  ...
  spgw_preinit(&spgw);
  {
    ...
    if (vcf_dosage_import_field) {
      logerrprint("Error: VCF dosage import is currently under development.\n");
	...
	format_gt_present = 1;
      } else if (!memcmp(&(loadbuf_iter[2]), "FORMAT=<ID=GP,Number=", 21)) {
	if (format_gp_present) {
	  logerrprint("Error: Duplicate FORMAT:GP header line in --vcf file.\n");
	  goto vcf_to_pgen_ret_MALFORMED_INPUT;
	}
	if (memcmp(&(loadbuf_iter[23]), "1,Type=Float,Description=", 25)) {
	  sprintf(g_logbuf, "Warning: Header line %" PRIuPTR " of --vcf file does not have expected FORMAT:GP format. Ignoring.\n", line_idx);
	} else {
	  format_gp_present = 1;
	}
    ...
    if ((!format_gt_present) && require_gt) {
      ...
      logerrprint("Error: No GT field in --vcf file, when --vcf-require-gt was specified.\n");
    ...
    uintptr_t* phasing_flags = (uintptr_t*)bigstack_end_alloc_raw(round_up_pow2(max_variant_ctaw * sizeof(intptr_t), kEndAllocAlign));
    uintptr_t* phasing_flags_iter = phasing_flags;
    uintptr_t* nonref_flags = nullptr;
    if (info_pr_present) {
      nonref_flags = (uintptr_t*)bigstack_end_alloc_raw(round_up_pow2(max_variant_ctaw * sizeof(intptr_t), kEndAllocAlign));
    }
    ...
    uint32_t at_least_one_dosage = 0;
	...
	uint32_t gp_idx = 0; // if nonzero, zero-based index of GP field
	if (format_gp_present) {
	  char* format_end = strchr(loadbuf_iter, '\t');
	  if (!format_end) {
	    goto vcf_to_pgen_ret_MISSING_TOKENS;
	  }
	  gp_idx = get_vcf_format_position("GP", 2, loadbuf_iter, format_end);
	}

	// check if there's at least one phased het call, and/or at least one
	// relevant dosage
	if (alt_ct < 10) {
	  if ((vcf_min_gq == -1) && (!gp_idx)) {
	    // simplest phased het case
	    while (1) {
	      loadbuf_iter = strchr(&(loadbuf_iter[1]), '\t');
	      if (!loadbuf_iter) {
		break;
		...
		while (1) {
		  ...
		  if (loadbuf_iter[2] == '|') {
		    if (vcf_is_het_short(&(loadbuf_iter[1]), vcf_half_call)) {
	    ...
	    /*
	    if (gp_idx && (!at_least_one_dosage)) {
	      char* sample_gtext_start = strchr(&(loadbuf_iter[2]), '\t');
	      ++sample_gtext_start;
	      unsigned char old_postend_char = '\0';
	      while ((unsigned char)(*sample_gtext_start) >= 32) {
		char* sample_gtext_end = strchr(sample_gtext_start, '\t');
		if (!sample_gtext_end) {
		  sample_gtext_end = sample_gtext_start;
		  // skip spaces as well as regular characters
		  while ((unsigned char)(*sample_gtext_end) >= 32) {
		    ++sample_gtext_end;
		  }
		  old_postend_char = sample_gtext_end[1];
		  sample_gtext_end[1] = '\0';
		}
		char* token_start = sample_gtext_start;
		uint32_t bytes_left = (uintptr_t)(sample_gtext_end - sample_gtext_start);
		uint32_t token_idx = 0;
		while (1) {
		  char* token_end = (char*)memchr(token_start, ':', bytes_left);
		  if (!token_end) {
		    break;
		  }
		  bytes_left -= 1 + (uintptr_t)(token_end - token_start);
		  token_start = &(token_end[1]);
		  if (++token_idx == gp_idx) {
		    if ((token_start[0] == '.') && (((uint32_t)((unsigned char)token_start[1])) - 48 >= 10)) {
		      // missing field (dot followed by e.g. colon or comma)
		      // (should we also tolerate e.g. '?'?)
		      break;
		    }
		    double prob_0alt;
		    char* gp_iter = scanadv_double(token_start, &prob_0alt);
		    if ((!gp_iter) || (*gp_iter != ',')) {
		      goto vcf_to_pgen_ret_INVALID_GP;
		    }
		    double prob_1alt;
		    ...
		    double prob_2alt;
		    if (!scanadv_double(&(gp_iter[1]), &prob_2alt)) {
		      goto vcf_to_pgen_ret_INVALID_GP;
		    }
		    // todo: compute dosage
		  }
		}
		sample_gtext_start = &(sample_gtext_end[1]);
	      }
	      if (old_postend_char) {
		*sample_gtext_start = old_postend_char;
	      }
	    }
	    */
	    if (gp_idx) {
	      logerrprint("Error: FORMAT:GP import is currently under development.\n");
	      reterr = kPglRetNotYetSupported;
	      goto vcf_to_pgen_ret_1;
	    }
	    if (vcf_min_gq != -1) {
	      logerrprint("Error: --vcf-min-gq is not implemented yet.\n");
	      reterr = kPglRetNotYetSupported;
	      goto vcf_to_pgen_ret_1;
	    }
	  }
	...
	*nonref_flags_iter = nonref_word;
    ...
    if (multiallelic_skip_ct) {
      LOGPRINTF("%" PRIuPTR " multiallelic variant%s %sskipped (not yet supported).\n", multiallelic_skip_ct, (multiallelic_skip_ct == 1)? "" : "s", variant_skip_ct? "also " : "");
    }
	...
	// could also check for duplicates?
	const char old_contig_name_end_char = *contig_name_end;
	*contig_name_end = '\0';
	// can't use get_chr_code() since hash table may not be initialized
	int32_t chr_code_raw = get_chr_code_raw(contig_name_start);
	if (chr_code_raw == -1) {
	  if (!cip->name_ct) {
	    continue;
	  }
	  const uint32_t chr_code = id_htable_find(contig_name_start, cip->nonstd_names, cip->nonstd_id_htable, (uintptr_t)(contig_name_end - contig_name_start), kChrHtableSize);
	  if ((chr_code == 0xffffffffU) || (!IS_SET(cip->chr_mask, chr_code))) {
	    continue;
	  }
	} else {
	  if (chr_code_raw >= ((int32_t)kMaxContigs)) {
	    chr_code_raw = cip->xymt_codes[chr_code_raw - kMaxContigs];
	  }
	  if ((chr_code_raw < 0) || (!is_set(base_chr_present, chr_code_raw))) {
	    continue;
	  }
	}
	*contig_name_end = old_contig_name_end_char;
	...
	*/
	...
	uint32_t gp_idx = 0;
	if (format_gp_present) {
	  gp_idx = get_vcf_format_position("GP", 2, format_start, loadbuf_iter);
	}
	*/
	...
	if (!IS_SET(phasing_flags, vidx)) {
	  while (1) {
	    ...
	    for (uint32_t sample_idx_lowbits = 0; sample_idx_lowbits <= inner_loop_last; ++sample_idx_lowbits) {
	      char* cur_gdata_end = strchr(loadbuf_iter, '\t');
	      if (!cur_gdata_end) {
		if ((sample_idx_lowbits != inner_loop_last) || (widx != sample_ctl2_m1)) {
		  goto vcf_to_pgen_ret_MISSING_TOKENS;
		}
		cur_gdata_end = loadbuf_iter;
		do {
		} while ((unsigned char)(*(++cur_gdata_end)) >= 32);
	      }
	      uintptr_t cur_geno = (unsigned char)(*loadbuf_iter) - 48;
	      if (cur_geno <= 1) {
		// todo: GQ check
		char cc = loadbuf_iter[1];
		if ((cc != '/') && (cc != '|')) {
		  // haploid
		  cur_geno *= 2;
		} else {
  ...
  vcf_to_pgen_ret_INVALID_GT:
    ...
    break;
    /*
  vcf_to_pgen_ret_INVALID_GP:
    putc_unlocked('\n', stdout);
    LOGERRPRINTF("Error: Line %" PRIuPTR " of --vcf file has an invalid GP field.\n", line_idx);
    reterr = kPglRetMalformedInput;
    break;
    */
...
pglerr_t ox_sample_to_psam(const char* samplename, const char* ox_missing_code, misc_flags_t misc_flags, char* outname, char* outname_end, uint32_t* sample_ct_ptr) {
  ...
  uintptr_t line_idx = 0;
  {
    ...
    uintptr_t* col_is_categorical = (uintptr_t*)bigstack_alloc_raw(round_up_pow2(col_ctl * sizeof(intptr_t), kCacheline));
    uintptr_t* col_is_qt = (uintptr_t*)bigstack_alloc_raw(round_up_pow2(col_ctl * sizeof(intptr_t), kCacheline));
...
pglerr_t ox_gen_to_pgen(const char* genname, const char* samplename, const char* ox_single_chr_str, const char* ox_missing_code, misc_flags_t misc_flags, oxford_import_t oxford_import_flags, uint32_t hard_call_thresh, uint32_t dosage_erase_thresh, double import_dosage_certainty, char* outname, char* outname_end, chr_info_t* cip) {
  ...
  spgw_preinit(&spgw);
  {
    ...
    uint32_t import_dosage_certainty_int = kDosageMax - ((int32_t)((1.0 - import_dosage_certainty) * kDosageMax));
    if (!import_dosage_certainty_int) {
      import_dosage_certainty_int = 1;
    }
...
THREAD_FUNC_DECL bgen11_dosage_scan_thread(void* arg) {
  ...
  while (1) {
	...
	if ((dosage_int_sum > dosage_int_sum_thresh) || (dosage_int0 >= import_dosage_certainty_int) || (dosage_int1 >= import_dosage_certainty_int) || (dosage_int2 >= import_dosage_certainty_int)) {
	  ...
	  const uint32_t halfdist = biallelic_dosage_halfdist(write_dosage_int);
	  dosage_is_present = (halfdist < dosage_erase_halfdist);
	  if (dosage_is_present) {
	    break;
	  }
	}
...
pglerr_t ox_bgen_to_pgen(const char* bgenname, const char* samplename, const char* const_fid, const char* ox_missing_code, misc_flags_t misc_flags, oxford_import_t oxford_import_flags, uint32_t hard_call_thresh, uint32_t dosage_erase_thresh, double import_dosage_certainty, char id_delim, char idspace_to, uint32_t max_thread_ct, char* outname, char* outname_end, chr_info_t* cip) {
  ...
  spgw_preinit(&spgw);
  {
    ...
    //         number of variants which pass the filter.  (Fortunately, the
    //         header contains the variant count so we can usually early-bail
    //         when no chromosome filter is involved.)
    // Pass 2: Write .pvar and .pgen files.
    // Probable todo: Multithreaded load (unlike .gen.gz/.vcf.gz, there is no
    // reason to restrict ourselves to single-threaded decompression)
...
#endif
      // must have enough space for SNP/rs/chromosome IDs
      if (loadbuf_size < 3 * 65536) {
	goto ox_bgen_to_pgen_ret_NOMEM;
      }
      unsigned char* loadbuf = (unsigned char*)bigstack_alloc_raw(loadbuf_size);
      g_import_dosage_certainty_int = kDosageMax - ((int32_t)((1.0 - import_dosage_certainty) * kDosageMax));
      if (!g_import_dosage_certainty_int) {
	g_import_dosage_certainty_int = 1;
      }
...
pglerr_t plink1_sample_major_to_pgen(const char* pgenname, uintptr_t variant_ct, uintptr_t sample_ct, uint32_t real_ref_alleles, uint32_t max_thread_ct, FILE* infile) {
  ...
  pglerr_t reterr = kPglRetSuccess;
  {
    ...
    if (full_load_vecs_req > cachelines_avail * kVecsPerCacheline) {
      ...
      plink1_smaj_loadbuf = (uintptr_t*)bigstack_alloc_raw(round_up_pow2((cur_vidx_ct / 4) * ((uintptr_t)sample_ct), kCacheline));
    } else {
      load_multiplier = 1 + ((variant_ct - 1) / (calc_thread_ct * kPglVblockSize));
      cur_vidx_ct = variant_ct;
      plink1_smaj_loadbuf = (uintptr_t*)bigstack_alloc_raw(round_up_pow2(full_load_vecs_req * kBytesPerVec, kCacheline));
    }
...
THREAD_FUNC_DECL make_pgen_thread(void* arg) {
  ...
  const uint32_t hard_call_halfdist = g_hard_call_halfdist;
  const uint32_t dosage_erase_halfdist = g_dosage_erase_halfdist;
  const uintptr_t phaseraw_word_ct = kWordsPerVec + ((raw_sample_ct / kBitsPerWordD2) & (~(kWordsPerVec - 1)));
  ...
  while (1) {
    ...
    THREAD_BLOCK_FINISH(tidx);
    parity = 1 - parity;
    variant_idx_offset += calc_thread_ct * kPglVblockSize;
  ...
  pglerr_t reterr = kPglRetSuccess;
  {
	...
	while (1) {
	  ...
	  if (!is_last_block) {
		...
		join_threads2z(calc_thread_ct, 0, threads);
		g_cur_block_write_ct = 0;
		error_cleanup_threads2z(make_bedlike_thread, calc_thread_ct, threads);
	      }
	  ...
	  if (!is_last_block) {
	    ...
	    is_last_block = (variant_idx + cur_block_write_ct == variant_ct);
	    if (spawn_threads2z(make_bedlike_thread, calc_thread_ct, is_last_block, threads)) {
	...
	goto make_plink2_no_vsort_ret_1;
      ...
      const uint32_t input_biallelic = (!variant_allele_idxs) || (variant_allele_idxs[raw_variant_ct] == 2 * raw_variant_ct);
      // const_cast
      uintptr_t* write_allele_idx_offsets = (uintptr_t*)((uintptr_t)variant_allele_idxs);
      if (input_biallelic) {
	write_allele_idx_offsets = nullptr;
      } else if (variant_ct < raw_variant_ct) {
	if (bigstack_alloc_ul(variant_ct + 1, &write_allele_idx_offsets)) {
	  goto make_plink2_no_vsort_ret_NOMEM;
	}
	...
	read_phase_dosage_gflags = kfPgenGlobal0;
	uintptr_t* vrtypes_alias_iter = (uintptr_t*)pgfip->vrtypes;
	const uint32_t raw_variant_ctl = BITCT_TO_WORDCT(raw_variant_ct);
	uint32_t mask_multiply = ((pgfip->gflags & kfPgenGlobalHardcallPhasePresent)? 0x10 : 0) + ((pgfip->gflags & kfPgenGlobalDosagePresent)? 0x60 : 0) + ((pgfip->gflags & kfPgenGlobalDosagePhasePresent)? 0x80 : 0);
	...
	phaseraw_word_ct = kWordsPerVec + ((raw_sample_ct / kBitsPerWordD2) & (~(kWordsPerVec - 1)));
	load_vblock_cacheline_ct += WORDCT_TO_CLCT(((uint64_t)phaseraw_word_ct) * max_vblock_size);
      }
      uint32_t dosageraw_word_ct = 0;
	...
	dosageraw_word_ct = kWordsPerVec * (BITCT_TO_VECCT(raw_sample_ct) + DIV_UP(raw_sample_ct, (kBytesPerVec / sizeof(dosage_t))));
	...
	// todo: go to single-threaded fallback
	goto make_plink2_no_vsort_ret_NOMEM;
      }
#endif
      uint32_t calc_thread_ct = DIV_UP(variant_ct, kPglVblockSize);
	...
	goto make_plink2_no_vsort_ret_NOMEM;
      ...
      if (!pgfip->nonref_flags) {
	const pgen_global_flags_t gflags = pgfip->gflags;
	nonref_flags_storage = (gflags & kfPgenGlobalAllNonref)? 2 : 1;
      }
      g_pwcs = &(mpgwp->pwcs[0]);
	...
	if (cachelines_avail < alloc_base_cacheline_ct + mpgw_per_thread_cacheline_ct + other_per_thread_cacheline_ct) {
	  // todo: simple single-threaded low-memory fallback mode (especially
	  // important for large dosage datasets)
	  goto make_plink2_no_vsort_ret_NOMEM;
	}
	...
	if (read_batch_idx) {
	  ...
	  is_last_block = (write_idx_end == variant_ct);
	  if (spawn_threads2z(make_pgen_thread, calc_thread_ct, is_last_block, threads)) {
	    goto make_plink2_no_vsort_ret_THREAD_CREATE_FAIL;
	  }
	}
	if (!is_last_block) {
	  ...
	  for (uint32_t uii = 0; uii < cur_batch_size; ++uii, ++read_variant_uidx) {
		...
		join_threads2z(calc_thread_ct, 0, threads);
		g_cur_block_write_ct = 0;
		error_cleanup_threads2z(make_pgen_thread, calc_thread_ct, threads);
	      }
	      if (reterr == kPglRetMalformedInput) {
		logprint("\n");
		logerrprint("Error: Malformed .pgen file.\n");
    ...
    // todo: trim-alts interaction with set-hh-missing
...
pglerr_t export_ind_major_bed(const uintptr_t* orig_sample_include, const uintptr_t* variant_include, const uintptr_t* variant_allele_idxs, const alt_allele_ct_t* refalt1_select, uint32_t raw_sample_ct, uint32_t sample_ct, uint32_t raw_variant_ct, uint32_t variant_ct, uint32_t max_thread_ct, uintptr_t pgr_alloc_cacheline_ct, pgen_file_info_t* pgfip, char* outname, char* outname_end) {
  ...
  pglerr_t reterr = kPglRetSuccess;
  {
	...
	sample_batch_size = (writebuf_cachelines_avail / variant_cacheline_ct) & (~(kBitsPerWordD2 - 1));
	...
	read_sample_ct = read_sample_ctv2 * kQuatersPerVec;
      ...
      uintptr_t* vmaj_readbuf = (uintptr_t*)bigstack_alloc_raw(round_up_pow2(variant_ct * read_sample_ctaw2 * kBytesPerWord, kCacheline));      
...
pglerr_t export_ox_gen(const char* outname, const uintptr_t* sample_include, const uint32_t* sample_include_cumulative_popcounts, const uintptr_t* sex_male, const uintptr_t* variant_include, const chr_info_t* cip, const uint32_t* variant_bp, char** variant_ids, const uintptr_t* variant_allele_idxs, char** allele_storage, const alt_allele_ct_t* refalt1_select, uint32_t sample_ct, uint32_t raw_variant_ct, uint32_t variant_ct, uint32_t max_allele_slen, exportf_flags_t exportf_modifier, pgen_reader_t* simple_pgrp, uint32_t* sample_missing_geno_cts) {
  ...
  pglerr_t reterr = kPglRetSuccess;
  {
    ...
    if (simple_pgrp->fi.gflags & kfPgenGlobalDosagePresent) {
      const uint32_t multiallelic_present = variant_allele_idxs && (variant_allele_idxs[raw_variant_ct] != (2 * raw_variant_ct));
...
pglerr_t export_bgen11(const char* outname, const uintptr_t* sample_include, uint32_t* sample_include_cumulative_popcounts, const uintptr_t* sex_male, const uintptr_t* variant_include, const chr_info_t* cip, const uint32_t* variant_bp, char** variant_ids, const uintptr_t* variant_allele_idxs, char** allele_storage, const alt_allele_ct_t* refalt1_select, uint32_t sample_ct, uint32_t raw_variant_ct, uint32_t variant_ct, uint32_t max_allele_slen, uint32_t max_thread_ct, exportf_flags_t exportf_modifier, uintptr_t pgr_alloc_cacheline_ct, pgen_file_info_t* pgfip, uint32_t* sample_missing_geno_cts) {
  ...
  ZWRAP_useZSTDcompression(0);
  {
    ...
    const uintptr_t bgen_geno_cacheline_ct = DIV_UP(6 * sample_ct, kCacheline);
...
pglerr_t export_vcf(char* xheader, const uintptr_t* sample_include, const uint32_t* sample_include_cumulative_popcounts, const char* sample_ids, const char* sids, const uintptr_t* sex_male_collapsed, const uintptr_t* variant_include, const chr_info_t* cip, const uint32_t* variant_bp, char** variant_ids, const uintptr_t* variant_allele_idxs, char** allele_storage, const alt_allele_ct_t* refalt1_select, const uintptr_t* pvar_qual_present, const float* pvar_quals, const uintptr_t* pvar_filter_present, const uintptr_t* pvar_filter_npass, char** pvar_filter_storage, const char* pvar_info_reload, uintptr_t xheader_blen, uint32_t xheader_info_pr, uint32_t sample_ct, uintptr_t max_sample_id_blen, uintptr_t max_sid_blen, uint32_t raw_variant_ct, uint32_t variant_ct, uint32_t max_allele_slen, uint32_t max_filter_slen, uint32_t info_reload_slen, __maybe_unused uint32_t max_thread_ct, exportf_flags_t exportf_modifier, idpaste_t exportf_id_paste, char exportf_id_delim, pgen_file_info_t* pgfip, pgen_reader_t* simple_pgrp, char* outname, char* outname_end) {
  ...
  pglerr_t reterr = kPglRetSuccess;
  {
    ...
    if (writebuf_blen < 4 * sample_ct + 32 + max_filter_slen + info_reload_slen) {
      writebuf_blen = 4 * sample_ct + 32 + max_filter_slen + info_reload_slen;
    }
    ...
    // posible for both MAYBESID and SID to be set
    ...
    if (populate_strbox_htable(exported_sample_ids, sample_ct, max_exported_sample_id_blen, exported_id_htable_size, exported_id_htable)) {
      logerrprint("Warning: Duplicate ID(s) present in exported VCF file.\n");
    }
    for (uint32_t sample_idx = 0; sample_idx < sample_ct; ++sample_idx) {
      *write_iter++ = '\t';
...
pglerr_t exportf(char* xheader, const uintptr_t* sample_include, const char* sample_ids, const char* sids, const char* paternal_ids, const char* maternal_ids, const uintptr_t* sex_nm, const uintptr_t* sex_male, const pheno_col_t* pheno_cols, const char* pheno_names, const uintptr_t* variant_include, const chr_info_t* cip, const uint32_t* variant_bp, char** variant_ids, const uintptr_t* variant_allele_idxs, char** allele_storage, const alt_allele_ct_t* refalt1_select, const uintptr_t* pvar_qual_present, const float* pvar_quals, const uintptr_t* pvar_filter_present, const uintptr_t* pvar_filter_npass, char** pvar_filter_storage, const char* pvar_info_reload, const double* variant_cms, uintptr_t xheader_blen, uint32_t xheader_info_pr, uint32_t raw_sample_ct, uint32_t sample_ct, uintptr_t max_sample_id_blen, uintptr_t max_sid_blen, uintptr_t max_paternal_id_blen, uintptr_t max_maternal_id_blen, uint32_t pheno_ct, uintptr_t max_pheno_name_blen, uint32_t raw_variant_ct, uint32_t variant_ct, uint32_t max_allele_slen, uint32_t max_filter_slen, uint32_t info_reload_slen, uint32_t max_thread_ct, make_plink2_t make_plink2_modifier, exportf_flags_t exportf_modifier, idpaste_t exportf_id_paste, char exportf_id_delim, uintptr_t pgr_alloc_cacheline_ct, pgen_file_info_t* pgfip, pgen_reader_t* simple_pgrp, char* outname, char* outname_end) {
  ...
  pglerr_t reterr = kPglRetSuccess;
  {
    ...
    if (exportf_modifier & kfExportfOxGen) {
      ...
      pgr_clear_ld_cache(simple_pgrp);
      reterr = export_ox_gen(outname, sample_include, sample_include_cumulative_popcounts, sex_male, variant_include, cip, variant_bp, variant_ids, variant_allele_idxs, allele_storage, refalt1_select, sample_ct, raw_variant_ct, variant_ct, max_allele_slen, exportf_modifier, simple_pgrp, sample_missing_geno_cts);
      if (reterr) {
	goto exportf_ret_1;
      }
    }
