...
pglerr_t vcf_to_pgen(const char* vcfname, const char* preexisting_psamname, const char* const_fid, misc_flags_t misc_flags, uint32_t dosage_erase_thresh, double import_dosage_certainty, char id_delim, char idspace_to, double vcf_min_gq, vcf_half_call_t vcf_half_call, uint32_t vcf_dosage_import_field, fam_col_t fam_cols, char* outname, char* outname_end, chr_info_t* cip);
