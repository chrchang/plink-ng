...
pglerr_t glm_local_init(const char* local_covar_fname, const char* local_pvar_fname, const char* local_psam_fname, const char* sample_ids, const chr_info_t* cip, const uint32_t* variant_bp, char** variant_ids, const glm_info_t* glm_info_ptr, uint32_t raw_sample_ct, uintptr_t max_sample_id_blen, uint32_t raw_variant_ct, const uintptr_t** sample_include_ptr, const uintptr_t** sex_nm_ptr, const uintptr_t** sex_male_ptr, const uintptr_t** variant_include_ptr, uint32_t* sample_ct_ptr, uint32_t* variant_ct_ptr, gzFile* gz_local_covar_file_ptr, uint32_t** local_sample_uidx_order_ptr, uintptr_t** local_variant_include_ptr, uint32_t* local_sample_ct_ptr, uint32_t* local_variant_ctl_ptr, uint32_t* local_covar_ct_ptr) {
  ...
  pglerr_t reterr = kPglRetSuccess;
  {
    ...
    *local_variant_include_ptr = local_variant_include;
    ...
    uint32_t prev_bp = variant_bp[prev_variant_uidx];
	...
	}
	{
	  ...
	  if ((uint32_t)cur_chr_code != prev_chr_code) {
	    ...
	    prev_bp = variant_bp[prev_variant_uidx];
	  ...
	  if ((uint32_t)cur_bp > prev_bp) {
	    ...
	    } while ((prev_variant_uidx < chr_end) && ((uint32_t)cur_bp > variant_bp[prev_variant_uidx]));
...
boolerr_t glm_determine_covars(const uintptr_t* pheno_cc, const uintptr_t* initial_covar_include, const pheno_col_t* covar_cols, uint32_t raw_sample_ct, uint32_t raw_covar_ctl, uint32_t initial_covar_ct, uint32_t covar_max_nonnull_cat_ct, uint32_t is_sometimes_firth, uintptr_t* cur_sample_include, uintptr_t* covar_include, uint32_t* sample_ct_ptr, uint32_t* covar_ct_ptr, uint32_t* extra_cat_ct_ptr, uint32_t* separation_warning_ptr) {
  ...
  boolerr_t reterr = 0;
  {
	...
	covar_ct = popcount_longs(covar_include, raw_covar_ctl);
      ...
      // todo: in firth-fallback case, automatically switch to always-Firth if
      // separated covariate is detected
...
boolerr_t glm_fill_and_test_covars(const uintptr_t* sample_include, const uintptr_t* covar_include, const pheno_col_t* covar_cols, const char* covar_names, uintptr_t sample_ct, uintptr_t covar_ct, uint32_t local_covar_ct, uint32_t covar_max_nonnull_cat_ct, uintptr_t extra_cat_ct, uintptr_t max_covar_name_blen, uint32_t standard_beta, double vif_thresh, double max_corr, double* covars_smaj, double* covar_dotprod, double* covars_cmaj, char** cur_covar_names, vif_corr_err_t* vif_corr_check_result_ptr) {
  ...
  for (uintptr_t covar_read_idx = 0; covar_read_idx < covar_ct; ++covar_read_idx, ++covar_read_uidx) {
	...
	for (uintptr_t sample_idx = 0; sample_idx < sample_ct; ++sample_idx, ++sample_uidx) {
	  ...
	  *covar_write_iter++ = cur_covar_val;
      ...
      // do not have a column for the first remaining category
      uint32_t cat_uidx = next_set_unsafe(cat_covar_wkspace, 1) + 1;
	...
	*cur_covar_names_iter++ = (char*)new_covar_name_alloc;
	...
	if (standard_beta) {
	  for (uintptr_t sample_idx = 0; sample_idx < sample_ct; ++sample_idx, ++sample_uidx) {
	    next_set_unsafe_ck(sample_include, &sample_uidx);
	    cur_cat_obs_ct += (covar_vals[sample_uidx] == cat_uidx);
	  }
	  const double covar_sum_and_ssq = (double)((int32_t)cur_cat_obs_ct);
	  const double cur_mean = covar_sum_and_ssq * sample_ct_recip;
	  const double cur_stdev_recip = sqrt(sample_ct_m1_d / (covar_sum_and_ssq * (1.0 - cur_mean)));
	  double normalized_covar_vals[2];
	  normalized_covar_vals[0] = (-cur_mean) * cur_stdev_recip;
	  normalized_covar_vals[1] = (1.0 - cur_mean) * cur_stdev_recip;
	  sample_uidx = first_sample_uidx;
	  for (uintptr_t sample_idx = 0; sample_idx < sample_ct; ++sample_idx, ++sample_uidx) {
	    next_set_unsafe_ck(sample_include, &sample_uidx);
	    *covar_write_iter++ = normalized_covar_vals[(covar_vals[sample_uidx] == cat_uidx)];
	  }
	} else {
	  for (uintptr_t sample_idx = 0; sample_idx < sample_ct; ++sample_idx, ++sample_uidx) {
	    next_set_unsafe_ck(sample_include, &sample_uidx);
  ...
  if (standard_beta) {
    fill_double_zero(new_nonlocal_covar_ct, dbl_2d_buf); // all covariates zero-mean
  }
...
boolerr_t glm_alloc_fill_and_test_pheno_covars_qt(const uintptr_t* sample_include, const double* pheno_qt, const uintptr_t* covar_include, const pheno_col_t* covar_cols, const char* covar_names, uintptr_t sample_ct, uintptr_t covar_ct, uint32_t local_covar_ct, uint32_t covar_max_nonnull_cat_ct, uintptr_t extra_cat_ct, uintptr_t max_covar_name_blen, uint32_t standard_beta, double vif_thresh, double max_corr, double** pheno_d_ptr, double** covars_smaj_d_ptr, char*** cur_covar_names_ptr, vif_corr_err_t* vif_corr_check_result_ptr) {
  const uintptr_t new_covar_ct = covar_ct + extra_cat_ct;
  const uintptr_t new_nonlocal_covar_ct = new_covar_ct - local_covar_ct;
  ...
  if (bigstack_alloc_d(sample_ct, pheno_d_ptr) ||
      ...
      bigstack_alloc_d(new_nonlocal_covar_ct * sample_ct, covars_smaj_d_ptr) ||
      bigstack_alloc_d(new_nonlocal_covar_ct * new_nonlocal_covar_ct, &covar_dotprod) ||
      bigstack_alloc_d(new_nonlocal_covar_ct * sample_ct, &covars_cmaj)) {
    return 1;
  }
  ...
  if (glm_fill_and_test_covars(sample_include, covar_include, covar_cols, covar_names, sample_ct, covar_ct, local_covar_ct, covar_max_nonnull_cat_ct, extra_cat_ct, max_covar_name_blen, standard_beta, vif_thresh, max_corr, *covars_smaj_d_ptr, covar_dotprod, covars_cmaj, *cur_covar_names_ptr, vif_corr_check_result_ptr)) {
    return 1;
  }
...
boolerr_t glm_alloc_fill_and_test_pheno_covars_cc(const uintptr_t* sample_include, const uintptr_t* pheno_cc, const uintptr_t* covar_include, const pheno_col_t* covar_cols, const char* covar_names, uintptr_t sample_ct, uintptr_t covar_ct, uint32_t local_covar_ct, uint32_t covar_max_nonnull_cat_ct, uintptr_t extra_cat_ct, uintptr_t max_covar_name_blen, uint32_t standard_beta, double vif_thresh, double max_corr, uintptr_t** pheno_cc_collapsed_ptr, float** pheno_f_ptr, float** covars_cmaj_f_ptr, char*** cur_covar_names_ptr, vif_corr_err_t* vif_corr_check_result_ptr) {
  ...
  if (glm_fill_and_test_covars(sample_include, covar_include, covar_cols, covar_names, sample_ct, covar_ct, local_covar_ct, covar_max_nonnull_cat_ct, extra_cat_ct, max_covar_name_blen, standard_beta, vif_thresh, max_corr, covars_smaj_d, covar_dotprod, covars_cmaj_d, *cur_covar_names_ptr, vif_corr_check_result_ptr)) {
    return 1;
  }
...
static const double* g_covars_smaj_d = nullptr;
static double* g_covars_smaj_x_d = nullptr;
static double* g_covars_smaj_y_d = nullptr;
...
THREAD_FUNC_DECL glm_logistic_thread(void* arg) {
  ...
  while (1) {
    ...
    const float* local_covars_iter = &(g_local_covars_vcmaj_f[parity][variant_bidx * max_sample_ct * local_covar_ct]);
	...
	next_set_unsafe_ck(variant_include, &variant_uidx);
	{
	  ...
	  if (!missing_ct) {
		...
		nm_predictors_pmaj_iter[sample_idx] = kRecipDosage4thf * ((int32_t)((uint32_t)dosage_vals[dosage_idx]));
	      }
	    }
	  } else {
	    if (!dosage_ct) {
		...
		if (is_set(dosage_present, sample_midx)) {
		  cur_val = kRecipDosage4thf * ((int32_t)((uint32_t)dosage_vals[dosage_idx++]));
		} else {
	  ...
	  // alt1_dosage, case_allele_obs_ct, alt1_case_dosage, mach_r2 even
	  ...
	  for (uint32_t sample_idx = 0; sample_idx < nm_sample_ct; ++sample_idx) {
	    ...
	    alt1_case_dosage += cur_genotype_val * ((double)((int32_t)is_set(pheno_cc_nm, sample_idx)));
	  }
	  block_aux_iter->firth_fallback = 0;
	  block_aux_iter->alt1_dosage = dosage_sum;
	  block_aux_iter->alt1_case_dosage = alt1_case_dosage;
	  ...
	  if (genof_buffer_needed) {
	    ...
	    for (uint32_t sample_idx = 0; sample_idx < nm_sample_ct; ++sample_idx) {
	      float cur_geno = genotype_vals[sample_idx];
	      if (cur_geno > 1.0) {
		cur_geno = 2.0 - cur_geno;
	      }
	      nm_predictors_pmaj_iter[sample_idx] = cur_geno;
	    }
	    nm_predictors_pmaj_iter = &(nm_predictors_pmaj_iter[nm_sample_ct]);
	    append_float_zero(nm_sample_ct_rem, &nm_predictors_pmaj_iter);
	  }
	  if (model_dominant) {
	    for (uint32_t sample_idx = 0; sample_idx < nm_sample_ct; ++sample_idx) {
	      const float cur_geno = genotype_vals[sample_idx];
	      // 0..1..1
	      if (cur_geno > 1.0) {
		...
		genotype_vals[sample_idx] = cur_geno - 1.0;
	      }
	    }
	  }
	  ...
	  if (!is_always_firth) {
	    ...
	    const double ref_case_dosage = dosage_ceil * ((int32_t)nm_case_ct) - alt1_case_dosage;
	    const double alt1_ctrl_dosage = dosage_sum - alt1_case_dosage;
	    const double ref_ctrl_dosage = ref_sum - ref_case_dosage;
	    if ((alt1_case_dosage == 0.0) || (fabs(ref_case_dosage) < kBigEpsilon) || (fabs(alt1_ctrl_dosage) < kBigEpsilon) || (fabs(ref_ctrl_dosage) < kBigEpsilon)) {
	      if (is_sometimes_firth) {
  ...
  while (variant_bidx < cur_block_variant_ct) {
	...
	if (cur_sample_idx == 0xffffffffU) {
	  ...
	  continue;
        ...
        // todo: extend to save double-precision values for --linear
	if (local_cat_ct) {
	  uint32_t cat_idx;
	  ...
	  if (cat_idx != local_cat_ct) {
	    --cat_idx;
	    local_covars_vcmaj_f_iter[cat_idx * max_sample_ct + cur_sample_idx] = 1.0;
	  }
...
pglerr_t glm_logistic(const char* cur_pheno_name, char** test_names, char** test_names_x, char** test_names_y, const uint32_t* variant_bp, char** variant_ids, char** allele_storage, const glm_info_t* glm_info_ptr, const uint32_t* local_sample_uidx_order, const uintptr_t* local_variant_include, const char* outname, uint32_t raw_variant_ct, uint32_t max_chr_blen, double ci_size, double pfilter, double output_min_p, uint32_t max_thread_ct, uintptr_t pgr_alloc_cacheline_ct, uint32_t local_sample_ct, uint32_t local_loadbuf_size, pgen_file_info_t* pgfip, gzFile gz_local_covar_file, uintptr_t* valid_variants, double* orig_pvals, double* orig_chisq, unsigned char* overflow_buf, char* local_loadbuf) {
  ...
  cswrite_init_null(&css);
  {
    ...
    const uint32_t alt1_ct_col = glm_cols & kfGlmColAlt1count;
    const uint32_t tot_allele_col = glm_cols & kfGlmColTotallele;
    const uint32_t alt1_ct_cc_col = glm_cols & kfGlmColAlt1countcc;
    const uint32_t tot_allele_cc_col = glm_cols & kfGlmColTotallelecc;
    const uint32_t alt1_freq_col = glm_cols & kfGlmColAlt1freq;
    const uint32_t alt1_freq_cc_col = glm_cols & kfGlmColAlt1freqcc;
    ...
    *cswritep++ = '#';
    ...
    if (alt1_ct_cc_col) {
      cswritep = strcpya(cswritep, "\tALT1_CASE_CT\tALT1_CTRL_CT");
    }
    if (tot_allele_cc_col) {
      cswritep = strcpya(cswritep, "\tCASE_ALLELE_CT\tCTRL_ALLELE_CT");
    }
    if (alt1_freq_col) {
      cswritep = strcpya(cswritep, "\tALT1_FREQ");
    }
    if (alt1_freq_cc_col) {
      cswritep = strcpya(cswritep, "\tALT1_CASE_FREQ\tALT1_CTRL_FREQ");
    }
    if (mach_r2_col) {
	...
	for (uint32_t variant_bidx = 0; variant_bidx < prev_block_variant_ct; ++variant_bidx, ++write_variant_uidx) {
	  ...
	  for (uint32_t test_idx = 0; test_idx < cur_reported_test_ct; ++test_idx) {
	    ...
	    if (variant_bp) {
	      cswritep = uint32toa_x(variant_bp[write_variant_uidx], '\t', cswritep);
	    }
	    ...
	    if (alt1_ct_col) {
	      *cswritep++ = '\t';
	      cswritep = dtoa_g(auxp->alt1_dosage, cswritep);
	    }
	    if (tot_allele_col) {
	      *cswritep++ = '\t';
	    ...
	    if (alt1_ct_cc_col) {
	      *cswritep++ = '\t';
	      cswritep = dtoa_g(auxp->alt1_case_dosage, cswritep);
	      *cswritep++ = '\t';
	      cswritep = dtoa_g(auxp->alt1_dosage - auxp->alt1_case_dosage, cswritep);
	    }
	    if (tot_allele_cc_col) {
	      *cswritep++ = '\t';
	    ...
	    if (alt1_freq_col) {
	      *cswritep++ = '\t';
	      cswritep = dtoa_g(auxp->alt1_dosage / ((double)auxp->allele_obs_ct), cswritep);
	    }
	    if (alt1_freq_cc_col) {
	      *cswritep++ = '\t';
	      cswritep = dtoa_g(auxp->alt1_case_dosage / ((double)auxp->case_allele_obs_ct), cswritep);
	      *cswritep++ = '\t';
	      cswritep = dtoa_g((auxp->alt1_dosage - auxp->alt1_case_dosage) / ((double)(auxp->allele_obs_ct - auxp->case_allele_obs_ct)), cswritep);
	    }
	    if (mach_r2_col) {
	      *cswritep++ = '\t';
...
/*
pglerr_t glm_linear(const char* cur_pheno_name, char** test_names, char** test_names_x, char** test_names_y, const uint32_t* variant_bp, char** variant_ids, char** allele_storage, const glm_info_t* glm_info_ptr, const char* outname, uint32_t raw_variant_ct, uint32_t max_chr_blen, double ci_size, double pfilter, double output_min_p, uint32_t max_thread_ct, uintptr_t pgr_alloc_cacheline_ct, pgen_file_info_t* pgfip, uintptr_t* valid_variants, double* orig_pvals, double* orig_chisq, unsigned char* overflow_buf) {
  unsigned char* bigstack_mark = g_bigstack_base;
  char* cswritep = nullptr;
  compress_stream_state_t css;
  pglerr_t reterr = kPglRetSuccess;
...
pglerr_t glm_main(const uintptr_t* orig_sample_include, const char* sample_ids, const char* sids, const uintptr_t* sex_nm, const uintptr_t* sex_male, const pheno_col_t* pheno_cols, const char* pheno_names, const pheno_col_t* covar_cols, const char* covar_names, const uintptr_t* orig_variant_include, const chr_info_t* cip, const uint32_t* variant_bp, char** variant_ids, const uintptr_t* variant_allele_idxs, char** allele_storage, const glm_info_t* glm_info_ptr, const adjust_info_t* adjust_info_ptr, const aperm_t* aperm_ptr, const char* local_covar_fname, const char* local_pvar_fname, const char* local_psam_fname, uint32_t raw_sample_ct, uint32_t orig_sample_ct, uintptr_t max_sample_id_blen, uintptr_t max_sid_blen, uint32_t pheno_ct, uintptr_t max_pheno_name_blen, uint32_t orig_covar_ct, uintptr_t max_covar_name_blen, uint32_t raw_variant_ct, uint32_t variant_ct, uint32_t max_allele_slen, double ci_size, double vif_thresh, double pfilter, double output_min_p, uint32_t max_thread_ct, uintptr_t pgr_alloc_cacheline_ct, pgen_file_info_t* pgfip, char* outname, char* outname_end) {
  ...
  pglerr_t reterr = kPglRetSuccess;
  {
    ...
    if (local_covar_fname) {
      reterr = glm_local_init(local_covar_fname, local_pvar_fname, local_psam_fname, sample_ids, cip, variant_bp, variant_ids, glm_info_ptr, raw_sample_ct, max_sample_id_blen, raw_variant_ct, &orig_sample_include, &sex_nm, &sex_male, &orig_variant_include, &orig_sample_ct, &variant_ct, &gz_local_covar_file, &local_sample_uidx_order, &local_variant_include, &local_sample_ct, &local_variant_ctl, &local_covar_ct);
      if (reterr) {
	goto glm_main_ret_1;
      }
    ...
    *outname_end = '.';
	...
	if (is_set(cip->haploid_mask, chr_idx)) {
	  ...
	  const uint32_t cur_chr_variant_ct = popcount_bit_idx(orig_variant_include, variant_uidx_start, variant_uidx_end);
	  if (cur_chr_variant_ct) {
	    if (!removed_variant_ct) {
	      // no main-loop logic for excluding all haploid chromosomes, so
	      // make a full copy of orig_variant_include and throw away our
	      // reference to the original
	      if (bigstack_alloc_ul(raw_variant_ctl, &variant_include_nohap)) {
		goto glm_main_ret_NOMEM;
	      }
	      memcpy(variant_include_nohap, orig_variant_include, raw_variant_ctl * sizeof(intptr_t));
	    }
	...
	orig_variant_include = variant_include_nohap;
	max_variant_ct = variant_ct;
      }
    } else {
      if (xymt_exists(cip, kChrOffsetX, &x_code)) {
	variant_ct_x = count_chr_variants_unsafe(orig_variant_include, cip, (uint32_t)x_code);
	...
	variant_ct_y = count_chr_variants_unsafe(orig_variant_include, cip, (uint32_t)y_code);
	if (variant_ct_y) {
	  if (!male_ct) {
	...
	new_max_covar_name_blen = 6 + int_slen(local_covar_ct + 1);
      ...
      if (glm_info_ptr->condition_varname || glm_info_ptr->condition_list_fname) {
        // todo: insert --condition{-list} covariates in front (though not
	// before local covariates)
	// (--condition-list allocates at end of bigstack)
	logerrprint("Error: --condition and --condition-list are under development.\n");
	...
	if ((!new_covar_cols) ||
      ...
      memcpy(&(new_covar_cols[condition_ct + local_covar_ct]), covar_cols, (orig_covar_ct + local_covar_ct) * sizeof(pheno_col_t));
	...
	*name_end = '\0';
	...
	logerrprint("Error: --parameters/--tests cannot currently be used with categorical\ncovariates.  Manually expand them into binary covariates first.\n");
	goto glm_main_ret_INCONSISTENT_INPUT;
      }
    }
    const uint32_t domdev_present_p1 = domdev_present + 1;
    ...
    const uint32_t standard_beta = glm_flags & kfGlmStandardBeta;
	...
	LOGERRPRINTFWW("Warning: Skipping --glm regression on phenotype '%s' since there are fewer remaining samples than predictor columns.\n", cur_pheno_name);
	...
	if ((!y_samples_are_different) && (covar_ct == covar_ct_y) && are_all_words_identical(covar_include, covar_include_y, raw_covar_ctl)) {
      ...
      if (is_logistic) {
	if (glm_alloc_fill_and_test_pheno_covars_cc(cur_sample_include, cur_pheno_col->data.cc, covar_include, covar_cols, covar_names, sample_ct, covar_ct, local_covar_ct, covar_max_nonnull_cat_ct, extra_cat_ct, max_covar_name_blen, standard_beta, vif_thresh, glm_info_ptr->max_corr, &pheno_cc, &pheno_f, &covars_cmaj_f, &cur_covar_names, &vif_corr_check_result)) {
	  goto glm_main_ret_NOMEM;
	}
      } else {
	if (glm_alloc_fill_and_test_pheno_covars_qt(cur_sample_include, cur_pheno_col->data.qt, covar_include, covar_cols, covar_names, sample_ct, covar_ct, local_covar_ct, covar_max_nonnull_cat_ct, extra_cat_ct, max_covar_name_blen, standard_beta, vif_thresh, glm_info_ptr->max_corr, &pheno_d, &covars_smaj_d, &cur_covar_names, &vif_corr_check_result)) {
	  goto glm_main_ret_NOMEM;
	}
      }
	...
	continue;
      ...
      if (sample_ct_x) {
	if (is_logistic) {
	  if (glm_alloc_fill_and_test_pheno_covars_cc(cur_sample_include_x, cur_pheno_col->data.cc, covar_include_x, covar_cols, covar_names, sample_ct_x, covar_ct_x, local_covar_ct, extra_cat_ct_x, covar_max_nonnull_cat_ct, max_covar_name_blen, standard_beta, vif_thresh, glm_info_ptr->max_corr, &g_pheno_x_cc, &g_pheno_x_f, &g_covars_cmaj_x_f, &cur_covar_names_x, &vif_corr_check_result)) {
	    goto glm_main_ret_NOMEM;
	  }
	} else {
	  if (glm_alloc_fill_and_test_pheno_covars_qt(cur_sample_include_x, cur_pheno_col->data.qt, covar_include_x, covar_cols, covar_names, sample_ct_x, covar_ct_x, local_covar_ct, covar_max_nonnull_cat_ct, extra_cat_ct, max_covar_name_blen, standard_beta, vif_thresh, glm_info_ptr->max_corr, &g_pheno_x_d, &g_covars_smaj_x_d, &cur_covar_names_x, &vif_corr_check_result)) {
	    goto glm_main_ret_NOMEM;
	  }
	}
	if (vif_corr_check_result.errcode) {
      ...
      if (sample_ct_y) {
	if (is_logistic) {
	  if (glm_alloc_fill_and_test_pheno_covars_cc(cur_sample_include_y, cur_pheno_col->data.cc, covar_include_y, covar_cols, covar_names, sample_ct_y, covar_ct_y, local_covar_ct, extra_cat_ct_y, covar_max_nonnull_cat_ct, max_covar_name_blen, standard_beta, vif_thresh, glm_info_ptr->max_corr, &g_pheno_y_cc, &g_pheno_y_f, &g_covars_cmaj_y_f, &cur_covar_names_y, &vif_corr_check_result)) {
	    goto glm_main_ret_NOMEM;
	  }
	} else {
	  if (glm_alloc_fill_and_test_pheno_covars_qt(cur_sample_include_y, cur_pheno_col->data.qt, covar_include_y, covar_cols, covar_names, sample_ct_y, covar_ct_y, local_covar_ct, covar_max_nonnull_cat_ct, extra_cat_ct, max_covar_name_blen, standard_beta, vif_thresh, glm_info_ptr->max_corr, &g_pheno_y_d, &g_covars_smaj_y_d, &cur_covar_names_y, &vif_corr_check_result)) {
	    goto glm_main_ret_NOMEM;
	  }
	}
	if (vif_corr_check_result.errcode) {
      ...
      if (alloc_and_init_reported_test_names(g_parameter_subset, cur_covar_names, glm_flags, covar_ct, &cur_test_names)) {
	goto glm_main_ret_NOMEM;
      }
      if (sample_ct_x) {
	if (alloc_and_init_reported_test_names(g_parameter_subset_x, cur_covar_names_x, glm_flags, covar_ct_x, &cur_test_names_x)) {
	  goto glm_main_ret_NOMEM;
	}
      }
      if (sample_ct_y) {
	if (alloc_and_init_reported_test_names(g_parameter_subset_y, cur_covar_names_y, glm_flags, covar_ct_y, &cur_test_names_y)) {
	  goto glm_main_ret_NOMEM;
	}
      }
      if (joint_test) {
	...
	memcpy(tmp_variant_include, orig_variant_include, raw_variant_ctl * sizeof(intptr_t));
	...
	if (skip_x) {
	  if (local_variant_include) {
	    const uint32_t variant_ct_before_x = popcount_bit_idx(orig_variant_include, 0, x_start);
	...
	if (skip_y) {
	  if (local_variant_include) {
	    const uint32_t variant_ct_before_y = popcount_bit_idx(orig_variant_include, 0, y_start);
	...
	outname_end2 = strcpya(outname_end2, ".zst");
      ...
      *outname_end2 = '\0';
      ...
      if (is_logistic) {
	reterr = glm_logistic(cur_pheno_name, cur_test_names, cur_test_names_x, cur_test_names_y, glm_pos_col? variant_bp : nullptr, variant_ids, allele_storage, glm_info_ptr, local_sample_uidx_order, cur_local_variant_include, outname, raw_variant_ct, max_chr_blen, ci_size, pfilter, output_min_p, max_thread_ct, pgr_alloc_cacheline_ct, local_sample_ct, local_loadbuf_size, pgfip, gz_local_covar_file, valid_variants, orig_pvals, orig_chisq, overflow_buf, local_loadbuf);
      } else {
	// reterr = glm_linear(cur_pheno_name, cur_test_names, cur_test_names_x, cur_test_names_y, glm_pos_col? variant_bp : nullptr, variant_ids, allele_storage, glm_info_ptr, outname, raw_variant_ct, max_chr_blen, ci_size, pfilter, output_min_p, max_thread_ct, pgr_alloc_cacheline_ct, pgfip, valid_variants, orig_pvals, orig_chisq, overflow_buf);
      }
      if (reterr) {
	goto glm_main_ret_1;
      }
