...
PglErr ConcatPvariantPos(int32_t cur_bp, uintptr_t variant_ct, PvariantPosMergeContext* ppmcp, SamePosPvarRecord** same_pos_records, MergeReader* mrp, MergeWriter* mwp) {
  ...
  if (debug_print) {
    logprintf("Starting ConcatPvariantPos().\n");
  }
  ...
  for (uintptr_t rec_idx_start = 0; ; ) {
    if (debug_print) {
      logprintf("Entering loop, rec_idx_start=%" PRIuPTR ".\n", rec_idx_start);
    }
    ...
    if (debug_print) {
      logprintf("MergePvariant() returned, allele_ct=%u, reterr=%u.\n", allele_ct, S_CAST(uint32_t, reterr));
    }
    if (unlikely(reterr)) {
      return reterr;
    }
