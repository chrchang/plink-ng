...
uint32_t are_nonmajor_freqs_needed(command1_flags_t command_flags1, double min_maf, double max_maf) {
  return (command_flags1 & (kfCommand1AlleleFreq | kfCommand1LdPrune | kfCommand1Pca | kfCommand1MakeRel)) || (min_maf != 0.0) || (max_maf != 1.0);
}
...
pglerr_t apply_variant_bp_filters(const char* extract_fnames, const char* exclude_fnames, const chr_info_t* cip, const uint32_t* variant_bp, int32_t from_bp, int32_t to_bp, uint32_t raw_variant_ct, misc_flags_t misc_flags, unsorted_var_t vpos_sortstatus, uintptr_t* variant_include, uint32_t* variant_ct_ptr) {
  ...
  if ((from_bp != -1) || (to_bp != -1)) {
    ...
    if (from_bp != -1) {
      const uint32_t from_offset = uint32arr_greater_than(&(variant_bp[variant_uidx_start]), variant_uidx_end - variant_uidx_start, (uint32_t)from_bp);
      variant_uidx_start += from_offset;
    }
    if ((to_bp != -1) && (variant_uidx_start < variant_uidx_end)) {
      const uint32_t to_offset = uint32arr_greater_than(&(variant_bp[variant_uidx_start]), variant_uidx_end - variant_uidx_start, 1 + ((uint32_t)to_bp));
  ...
  if (extract_fnames && (misc_flags & kfMiscExtractRange)) {
    ...
    pglerr_t reterr = extract_exclude_range(extract_fnames, cip, variant_bp, raw_variant_ct, 0, variant_include, variant_ct_ptr);
    if (reterr) {
      return reterr;
    }
  }
  if (exclude_fnames && (misc_flags & kfMiscExcludeRange)) {
    ...
    pglerr_t reterr = extract_exclude_range(exclude_fnames, cip, variant_bp, raw_variant_ct, 1, variant_include, variant_ct_ptr);
    if (reterr) {
      return reterr;
    }
  }
...
pglerr_t plink2_core(char* var_filter_exceptions_flattened, const plink2_cmdline_t* pcp, make_plink2_t make_plink2_modifier, char* pgenname, char* psamname, char* pvarname, char* outname, char* outname_end, char* king_cutoff_fprefix, chr_info_t* cip) {
  ...
  pgr_preinit(&simple_pgr);
  {
    ...
    uint32_t max_variant_id_blen = 2;
    ...
    if (variant_ct && (!full_variant_id_htable_needed)) {
      reterr = apply_variant_bp_filters(pcp->extract_fnames, pcp->exclude_fnames, cip, variant_bp, pcp->from_bp, pcp->to_bp, raw_variant_ct, pcp->misc_flags, vpos_sortstatus, variant_include, &variant_ct);
      if (reterr) {
	goto plink2_ret_1;
      }
    }
    if (variant_ct && (full_variant_id_htable_needed || (pcp->extract_fnames && (!(pcp->misc_flags & kfMiscExtractRange))) || (pcp->exclude_fnames && (!(pcp->misc_flags & kfMiscExcludeRange))))) {
      ...
      // this needs to work under low memory conditions, but it gets a
      // substantial speedup from a smaller load factor, so we want to use a
      // smart sizing strategy
      uint32_t variant_id_htable_size = get_htable_fast_size(variant_ct);
      ...
      // the variant_ct * sizeof(int32_t) term here is a bit arbitrary
      if (nonhtable_alloc + variant_ct * sizeof(int32_t) > htable_max_alloc) {
	goto plink2_ret_NOMEM;
      }
	...
	if (variant_id_htable_size < min_htable_size) {
      ...
      reterr = populate_variant_id_htable_mt(variant_include, variant_ids, variant_ct, 1, variant_id_htable_size, pcp->max_thread_ct, variant_id_htable);
      if (reterr) {
	goto plink2_ret_1;
      }
      if (vpos_sortstatus & kfUnsortedVarBp) {
	...
	if (pcp->window_bp != -1) {
      ...
      if (pcp->varid_from || pcp->varid_to) {
	reterr = from_to_flag(variant_ids, variant_id_htable, pcp->varid_from, pcp->varid_to, raw_variant_ct, max_variant_id_blen, variant_id_htable_size, variant_include, cip, &variant_ct);
	if (reterr) {
	  goto plink2_ret_1;
	}
      }
      if (pcp->varid_snp) {
	reterr = snp_flag(variant_bp, variant_ids, variant_id_htable, pcp->varid_snp, raw_variant_ct, max_variant_id_blen, variant_id_htable_size, 0, pcp->window_bp, variant_include, cip, &variant_ct);
	if (reterr) {
	  goto plink2_ret_1;
	}
      }
      if (pcp->snps_range_list.name_ct) {
	reterr = snps_flag(variant_ids, variant_id_htable, &(pcp->snps_range_list), raw_variant_ct, max_variant_id_blen, variant_id_htable_size, 0, variant_include, &variant_ct);
	if (reterr) {
	  goto plink2_ret_1;
	}
      }
      if (pcp->varid_exclude_snp) {
	reterr = snp_flag(variant_bp, variant_ids, variant_id_htable, pcp->varid_exclude_snp, raw_variant_ct, max_variant_id_blen, variant_id_htable_size, 1, pcp->window_bp, variant_include, cip, &variant_ct);
	if (reterr) {
	  goto plink2_ret_1;
	}
      }
      if (pcp->exclude_snps_range_list.name_ct) {
	reterr = snps_flag(variant_ids, variant_id_htable, &(pcp->exclude_snps_range_list), raw_variant_ct, max_variant_id_blen, variant_id_htable_size, 1, variant_include, &variant_ct);
	if (reterr) {
	  goto plink2_ret_1;
	}
      }
	...
	reterr = extract_exclude_flag_norange(variant_ids, variant_id_htable, pcp->extract_fnames, raw_variant_ct, max_variant_id_blen, variant_id_htable_size, 0, variant_include, &variant_ct);
	if (reterr) {
	  goto plink2_ret_1;
	}
      }
      if (pcp->exclude_fnames && (!(pcp->misc_flags & kfMiscExcludeRange))) {
	reterr = extract_exclude_flag_norange(variant_ids, variant_id_htable, pcp->exclude_fnames, raw_variant_ct, max_variant_id_blen, variant_id_htable_size, 1, variant_include, &variant_ct);
	if (reterr) {
	  goto plink2_ret_1;
	}
      }
	...
	// their local phenotype copies on their own.  if we stop doing this,
	// it's appropriate to move this after final sample count is reported.)
      }
    }
	...
	if (!maj_alleles) {
      ...
      if (are_nonmajor_freqs_needed(pcp->command_flags1, pcp->min_maf, pcp->max_maf)) {
	if (bigstack_alloc_d(raw_variant_ct, &nonmajor_freqs)) {
	  goto plink2_ret_NOMEM;
	}
      }
      uint32_t x_start = 0;
	...
	if (x_len && ((pcp->command_flags1 & kfCommand1Hardy) || (pcp->hwe_thresh != 1.0)) && (!are_all_bits_zero(variant_include, x_start, x_end))) {
      ...
      if (are_allele_dosages_needed(pcp->misc_flags, make_plink2_modifier, maj_alleles || nonmajor_freqs, pcp->min_allele_dosage, pcp->max_allele_dosage)) {
	if (bigstack_alloc_ull(variant_allele_idxs? variant_allele_idxs[raw_variant_ct] : (2 * raw_variant_ct), &allele_dosages)) {
      ...
      if (are_founder_allele_dosages_needed(pcp->misc_flags, maj_alleles || nonmajor_freqs, pcp->min_allele_dosage, pcp->max_allele_dosage)) {
	...
	if (pcp->command_flags1 & kfCommand1GenotypingRate) {
      ...
      if (maj_alleles || nonmajor_freqs) {
	compute_maj_alleles_and_nonmajor_freqs(variant_include, variant_allele_idxs, nonfounders? allele_dosages : founder_allele_dosages, raw_variant_ct, variant_ct, (pcp->misc_flags / kfMiscMafSucc) & 1, maj_alleles, nonmajor_freqs);
	// todo: --read-freq
      }

      if (pcp->command_flags1 & kfCommand1AlleleFreq) {
	reterr = write_allele_freqs(variant_include, cip, variant_bp, variant_ids, variant_allele_idxs, allele_storage, nonfounders? allele_dosages : founder_allele_dosages, pcp->freq_ref_binstr, pcp->freq_alt1_binstr, variant_ct, pgfi.max_alt_allele_ct, max_allele_slen, pcp->allele_freq_modifier, nonfounders, outname, outname_end);
	if (reterr || (!(pcp->command_flags1 & (~(kfCommand1GenotypingRate | kfCommand1AlleleFreq))))) {
	  goto plink2_ret_1;
	}
      }
      if (pcp->command_flags1 & kfCommand1GenoCounts) {
	reterr = write_geno_counts(sample_include, sex_male, variant_include, cip, variant_bp, variant_ids, variant_allele_idxs, allele_storage, raw_geno_cts, x_male_geno_cts, raw_sample_ct, sample_ct, male_ct, variant_ct, x_start, max_allele_slen, pcp->geno_counts_modifier, &simple_pgr, outname, outname_end);
	...
	if (hwe_x_ct && ((pcp->hwe_thresh != 1.0) || (pcp->hardy_modifier & kfHardyColP))) {
      ...
      if (pcp->command_flags1 & kfCommand1Hardy) {
	reterr = hardy_report(variant_include, cip, variant_bp, variant_ids, variant_allele_idxs, allele_storage, nonfounders? raw_geno_cts : founder_raw_geno_cts, nonfounders? x_male_geno_cts : founder_x_male_geno_cts, nonfounders? x_nosex_geno_cts : founder_x_nosex_geno_cts, hwe_x_pvals, variant_ct, hwe_x_ct, max_allele_slen, pcp->output_min_p, pcp->hardy_modifier, nonfounders, outname, outname_end);
	...
	enforce_hwe_thresh(cip, nonfounders? raw_geno_cts : founder_raw_geno_cts, nonfounders? x_male_geno_cts : founder_x_male_geno_cts, nonfounders? x_nosex_geno_cts : founder_x_nosex_geno_cts, hwe_x_pvals, pcp->misc_flags, pcp->hwe_thresh, nonfounders, variant_include, &variant_ct);
      ...
      // raw_geno_cts/founder_raw_geno_cts/hwe_x_pvals no longer
      // needed
      bigstack_reset(bigstack_mark_geno_cts);

      if ((pcp->min_maf != 0.0) || (pcp->max_maf != 1.0) || pcp->min_allele_dosage || (pcp->max_allele_dosage != (~0LLU))) {
	// todo: --read-freq handling
	enforce_minor_freq_constraints(variant_allele_idxs, nonfounders? allele_dosages : founder_allele_dosages, nonmajor_freqs, raw_variant_ct, pcp->min_maf, pcp->max_maf, pcp->min_allele_dosage, pcp->max_allele_dosage, variant_include, &variant_ct);
      }

      if (pcp->mach_r2_max != 0.0) {
	// ok, time to implement this...
	logerrprint("Error: --mach-r2-filter is not implemented yet.\n");
        goto plink2_ret_1;
      }
    ...
    if ((pcp->command_flags1 & kfCommand1MakeRel) || keep_grm) {
      reterr = calc_grm(sample_include, sample_ids, sids, variant_include, cip, maj_alleles, nonmajor_freqs, raw_sample_ct, sample_ct, max_sample_id_blen, max_sid_blen, raw_variant_ct, variant_ct, pcp->grm_flags, pcp->parallel_idx, pcp->parallel_tot, pcp->max_thread_ct, &simple_pgr, outname, outname_end, keep_grm? (&grm) : nullptr);
      if (reterr) {
	goto plink2_ret_1;
      }
    ...
    if (pcp->command_flags1 & kfCommand1Pca) {
      ...
      reterr = calc_pca(sample_include, sample_ids, sids, variant_include, cip, variant_bp, variant_ids, variant_allele_idxs, allele_storage, maj_alleles, nonmajor_freqs, raw_sample_ct, sample_ct, max_sample_id_blen, max_sid_blen, raw_variant_ct, variant_ct, max_allele_slen, pcp->pca_ct, pcp->pca_flags, pcp->max_thread_ct, &simple_pgr, grm, outname, outname_end);
      if (reterr) {
	goto plink2_ret_1;
      }
    }
    ...
    if (pcp->command_flags1 & (kfCommand1MakePlink2 | kfCommand1Exportf)) {
	...
	goto plink2_ret_INCONSISTENT_INPUT;
      ...
      reterr = ld_prune(variant_include, cip, variant_bp, variant_ids, maj_alleles, nonmajor_freqs, founder_info, sex_male, &(pcp->ld_info), raw_variant_ct, variant_ct, raw_sample_ct, founder_ct, pcp->max_thread_ct, &simple_pgr, outname, outname_end);
      if (reterr) {
	goto plink2_ret_1;
      }
    }
...
pglerr_t alloc_and_flatten(char** sources, uint32_t param_ct, uint32_t max_blen, char** flattened_buf_ptr) {
  ...
  char* bufptr = (char*)malloc(totlen);
  if (!bufptr) {
    return kPglRetNomem;
  }
...
int main(int argc, char** argv) {
  ...
  
  {
	...
	if (!memcmp(flagname_p2, "req", 4)) {
	  ...
	  for (uint32_t param_idx = 1; param_idx <= param_ct; ++param_idx) {
	    ...
	    if ((cur_modif_slen == 2) && (!memcmp(cur_modif, "zs", 2))) {
	      ...
	      // er, filter-cases and filter-controls needs to be implemented
	      logerrprint("Error: --freq 'case-control' modifier has been retired.  Use\n--filter-cases/--filter-controls in conjunction with Unix text-processing\nutilities instead.\n");
	    } else if ((cur_modif_slen > 5) && (!memcmp(cur_modif, "cols=", 5))) {
	      if (pc.allele_freq_modifier & kfAlleleFreqColAll) {
		logerrprint("Error: Multiple --freq cols= modifiers.\n");
		goto main_ret_INVALID_CMDLINE;
	      }
	      reterr = parse_col_descriptor(&(cur_modif[5]), "chrom\0pos\0ref\0alt1\0alt\0reffreq\0alt1freq\0altfreq\0freq\0eq\0eqz\0alteq\0alteqz\0numeq\0altnumeq\0nobs\0", "freq", kfAlleleFreqColChrom, kfAlleleFreqColDefault, 1, &pc.allele_freq_modifier);
	      if (reterr) {
		goto main_ret_1;
	      }
	...
	if (!memcmp(flagname_p2, "eno", 4)) {
	  ...
	  for (uint32_t param_idx = 1; param_idx <= param_ct; ++param_idx) {
		...
		goto main_ret_INVALID_CMDLINE;
	      ...
	      reterr = parse_col_descriptor(&(cur_modif[5]), "chrom\0pos\0ref\0alt1\0alt\0alt1count\0totallele\0alt1countcc\0totallelecc\0alt1freq\0alt1freqcc\0machr2\0firth\0test\0nobs\0beta\0orbeta\0se\0ci\0t\0p\0", flagname_p, kfGlmColChrom, kfGlmColDefault, 1, &pc.glm_info.cols);
	      if (reterr) {
		goto main_ret_1;
	      }
	...
	if (!memcmp(flagname_p2, "ambda", 6)) {
	  ...
	  reterr = cmdline_alloc_string(chr_code, argv[arg_idx], kMaxIdSlen, &ox_single_chr_str);
	  if (reterr) {
	    goto main_ret_1;
	  }
	...
	if (!memcmp(flagname_p2, "utput-chr", 10)) {
	  ...
	  reterr = cmdline_alloc_string(cur_modif, argv[arg_idx], kMaxIdSlen, &ox_single_chr_str);
	  if (reterr) {
	    goto main_ret_1;
	  }
	...
	if (!memcmp(flagname_p2, "eed", 4)) {
	  ...
	  if (flagname_p2[3] == 'a') {
	    if (pc.missing_varid_match) {
	      logerrprint("Error: --set-all-var-ids cannot be used with --missing-var-code.\n");
	      goto main_ret_INVALID_CMDLINE_A;
	    }
	  } else {
	...
	if (!memcmp(flagname_p2, "chr-model", 10)) {
	  if (!(pc.command_flags1 & kfCommand1Glm)) {
	    logerrprint("Error: --xchr-model must be used with --glm.\n");
	    goto main_ret_INVALID_CMDLINE_A;
	  }
	  ...
	  const char* cur_modif = argv[arg_idx + 1];
	  pc.glm_info.xchr_model = ((uint32_t)extract_char_param(cur_modif)) - 48;
	  if (pc.glm_info.xchr_model > 2) {
    ...
    if ((pc.sample_sort_flags != kfSampleSort0) && (!(pc.command_flags1 & kfCommand1MakePlink2))) {
      // todo: permit merge
      logerrprint("Error: --indiv-sort must be used with --make-{b}pgen/--make-bed or dataset\nmerging.\n");
      goto main_ret_INVALID_CMDLINE_A;
    }
	...
	if (xload & kfXloadVcf) {
	  ...
	  reterr = ox_gen_to_pgen(pgenname, psamname, ox_single_chr_str, ox_missing_code, pc.misc_flags, oxford_import_flags, pc.hard_call_thresh, pc.dosage_erase_thresh, import_dosage_certainty, outname, convname_end, &chr_info);
	...
	*outname_end = '\0';
      ...
      const uint32_t calc_all_req = (pc.command_flags1 & (~(kfCommand1MakePlink2 | kfCommand1WriteSnplist))) || ((pc.command_flags1 & kfCommand1MakePlink2) && (make_plink2_modifier & (kfMakeBed | kfMakePgen)));
	...
	const uint32_t calc_pvar_req = (pc.command_flags1 & (~kfCommand1MakePlink2)) || ((pc.command_flags1 & kfCommand1MakePlink2) && (make_plink2_modifier & (kfMakeBed | kfMakeBim | kfMakePgen | kfMakePvar)));
	...
	if (calc_psam_req || (pc.filter_flags & kfFilterPsamReq)) {
      ...
      // logistic/Firth test code
      /*
      const uint32_t sample_ct = 499999;
      const uint32_t predictor_ct = 7;
      
      const uint32_t sample_stride = round_up_pow2(sample_ct, 4);
      const uint32_t predictor_stride = round_up_pow2(predictor_ct, 4);
      float* yy;
      float* xx;
      float* coef;
      float* pp;
      matrix_finvert_buf1_t* inv_1d_buf;
      float* flt_2d_buf;
      float* vv;
      float* hh;
      float* grad;
      ...
      float* ww;
      float* tmpnxk_buf;
      bigstack_alloc_f(sample_stride, &yy);
      bigstack_alloc_f(predictor_ct * sample_stride, &xx);
      fill_float_zero(sample_stride, yy);
      fill_float_zero(predictor_ct * sample_stride, xx);
      for (uint32_t uii = 0; uii < 10 * predictor_ct; ++uii) {
	yy[uii] = 1.0;
      }
      for (uint32_t uii = 0; uii < sample_ct; ++uii) {
	xx[uii] = 1.0;
      }
      xx[sample_stride] = 1.0;
      xx[sample_stride + 1] = 1.0;
      xx[sample_stride + 2] = 1.0;
      for (uint32_t uii = 0; uii < predictor_ct - 2; ++uii) {
	const uint32_t skip = 1 + (predictor_ct - 2) * (uii + 1);
	for (uint32_t ujj = 0; ujj < sample_ct; ujj += skip) {
	  xx[(uii + 2) * sample_stride + ujj] = 1.0;
	}
      }
      if (0) {
	for (uint32_t uii = 0; uii < sample_ct; ++uii) {
	  printf("%g", yy[uii]);
	  for (uint32_t ujj = 1; ujj < predictor_ct; ++ujj) {
	    printf("\t%g", xx[ujj * sample_stride + uii]);
	  }
	  printf("\n");
	}
	exit(1);
      ...
      bigstack_alloc_f(predictor_stride, &coef);
      bigstack_alloc_f(sample_ct, &pp);
      inv_1d_buf = (matrix_finvert_buf1_t*)bigstack_alloc(kMatrixFinvertBuf1CheckedAlloc * predictor_ct);
      bigstack_alloc_f(predictor_ct * predictor_stride, &flt_2d_buf);
      bigstack_alloc_f(sample_ct, &vv);
      bigstack_alloc_f(predictor_ct * predictor_stride, &hh);
      bigstack_alloc_f(predictor_ct, &grad);
      bigstack_alloc_f(predictor_ct, &dcoef);
      bigstack_alloc_f(sample_ct, &ww);
      bigstack_alloc_f(predictor_ct * sample_stride, &tmpnxk_buf);

      // non-Firth only
      float* ll;
      bigstack_alloc_f(predictor_ct * predictor_stride, &ll);
      
      for (uint32_t uii = 0; uii < 20; ++uii) {
        fill_float_zero(predictor_stride, coef);
	if (0) {
	  if (logistic_regression(yy, xx, sample_ct, predictor_ct, coef, ll, pp, vv, hh, grad, dcoef)) {
	    if (!uii) {
	      printf("regression failed\n");
	     }
	   }
	} else {
	  if (firth_regression(yy, xx, sample_ct, predictor_ct, coef, hh, inv_1d_buf, flt_2d_buf, pp, vv, grad, dcoef, ww, tmpnxk_buf)) {
	    if (!uii) {
	      printf("Firth regression failed\n");
	    }
	  }
	}
      }
      for (uint32_t predictor_idx = 0; predictor_idx < predictor_ct; ++predictor_idx) {
	printf("%g ", coef[predictor_idx]);
      }
      printf("\n");
      exit(1);
      */
      BLAS_SET_NUM_THREADS(1);
      reterr = plink2_core(var_filter_exceptions_flattened, &pc, make_plink2_modifier, pgenname, psamname, pvarname, outname, outname_end, king_cutoff_fprefix, &chr_info);
    }    
  }
  while (0) {
  main_ret_NOMEM:
