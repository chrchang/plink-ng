...
ZSTD_selectAddr(U32 indx, U32 lowLimit, const BYTE* candidate, const BYTE* backup)
{
    ...
    __asm__ (
        ...
        "cmova %3, %0\n"
        : "+r"(candidate)
        : "r"(indx), "r"(lowLimit), "r"(backup)
        );
    return candidate;
#else
    return indx >= lowLimit ? candidate : backup;
#endif
}
...
MEM_STATIC void ZSTD_writeTaggedIndex(U32* const hashTable, size_t hashAndTag, U32 indx) {
    size_t const hash = hashAndTag >> ZSTD_SHORT_CACHE_TAG_BITS;
    U32 const tag = (U32)(hashAndTag & ZSTD_SHORT_CACHE_TAG_MASK);
    assert(indx >> (32 - ZSTD_SHORT_CACHE_TAG_BITS) == 0);
    hashTable[hash] = (indx << ZSTD_SHORT_CACHE_TAG_BITS) | tag;
}
