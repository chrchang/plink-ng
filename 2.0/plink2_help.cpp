...
"      (must be VCFv4.3-style 0..1) if it's present, add 'dosage=GP' (not\n"
"      implemented yet).\n\n"
	       );
...
"    * 'bgen-1.1', 'bgen-1.2', 'bgen-1.3': Oxford-format .bgen + .sample.  For\n"
"      v1.2/v1.3, sample identifiers are stored in the .bgen (with id-delim and\n"
"      id-paste settings applied), and default precision is 16-bit (use the\n"
"      'bits' modifier to change this).\n"
...
"      By default, dosages are not exported; use 'vcf-dosage=GP' to export them\n"
"      as genotype posterior probabilities.\n"
...
"      Alternatively, with 'local-cats=[C]', the local-covar file is expected to\n"
"      have n columns with integer-valued entries in [1, C].  These category\n"
"      assignments are expanded into (C-1) local covariates in the usual manner.\n"
...
"  --geno {val} <hh-missing> : Exclude variants with missing call frequencies\n"
