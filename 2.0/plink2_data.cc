...
void CopyAndResortDosage(const uintptr_t* __restrict read_dosagepresent, const Dosage* read_dosagevals, const uint32_t* new_sample_idx_to_old, uint32_t raw_sample_ct, uint32_t sample_ct, uintptr_t* __restrict write_dosagepresent, Dosage* write_dosagevals, uint32_t* write_dosage_ct_ptr, uint32_t* cumulative_popcount_buf) {
  const uint32_t raw_sample_ctl = BitCtToWordCt(raw_sample_ct);
  FillCumulativePopcounts(read_dosagepresent, raw_sample_ctl, cumulative_popcount_buf);
  const uint32_t sample_ctl = BitCtToWordCt(sample_ct);
  ZeroWArr(sample_ctl, write_dosagepresent);
  Dosage* write_dosagevals_iter = write_dosagevals;
    ...
    for (uint32_t subset_idx = 0; ; ) {
      ...
      for (; cur_idx != cur_idx_end; ++cur_idx) {
        ...
        if (!allele_idx_offsets) {
          ...
          cur_allele_ct = allele_idx_offsets[variant_uidx + 1] - cur_allele_idx_offset;
        }
        if ((cur_allele_ct == 2) || no_multiallelic_branch) {
          ...
          // todo: multiallelic; also, variant_hethap_cts, allele_presents,
          // allele dosages, and/or mach-r2 needed
          // reterr = PgrGetDCounts(sample_include, sample_include_interleaved_vec, sample_include_cumulative_popcounts, sample_ct, variant_uidx, pgrp, mach_r2_vals? (&(mach_r2_vals[variant_uidx])) : nullptr, genocounts, cur_dosages);
          // need PgrGetMDCounts() and PgrGetMD()
          exit(63);
        }
        // some common code can probably be moved here
      }
...
PglErr LoadAlleleAndGenoCounts(const uintptr_t* sample_include, const uintptr_t* founder_info, const uintptr_t* sex_nm, const uintptr_t* sex_male, const uintptr_t* variant_include, const ChrInfo* cip, const uintptr_t* allele_idx_offsets, uint32_t raw_sample_ct, uint32_t sample_ct, uint32_t founder_ct, uint32_t male_ct, uint32_t nosex_ct, uint32_t raw_variant_ct, uint32_t variant_ct, uint32_t first_hap_uidx, uint32_t max_thread_ct, uintptr_t pgr_alloc_cacheline_ct, PgenFileInfo* pgfip, uintptr_t* allele_presents, uint64_t* allele_dosages, uint64_t* founder_allele_dosages, uint32_t* variant_missing_hc_cts, uint32_t* variant_missing_dosage_cts, uint32_t* variant_hethap_cts, STD_ARRAY_PTR_DECL(uint32_t, 3, raw_geno_cts), STD_ARRAY_PTR_DECL(uint32_t, 3, founder_raw_geno_cts), STD_ARRAY_PTR_DECL(uint32_t, 3, x_male_geno_cts), STD_ARRAY_PTR_DECL(uint32_t, 3, founder_x_male_geno_cts), STD_ARRAY_PTR_DECL(uint32_t, 3, x_nosex_geno_cts), STD_ARRAY_PTR_DECL(uint32_t, 3, founder_x_nosex_geno_cts), double* mach_r2_vals) {
  ...
  PglErr reterr = kPglRetSuccess;
  {
    ...
    if (allele_idx_offsets && (variant_hethap_cts || allele_presents || allele_dosages || founder_allele_dosages || mach_r2_vals)) {
      logerrputs("Error: LoadAlleleAndGenoCounts() doesn't fully support multiallelic variants yet.\n");
    ...
    const uint32_t xy_dosages_needed = (pgfip->gflags & kfPgenGlobalDosagePresent) && (((allele_presents || allele_dosages || founder_allele_dosages || variant_missing_dosage_cts) && XymtExists(cip, kChrOffsetX, &unused_chr_code)) || (allele_presents && (sample_ct != male_ct) && XymtExists(cip, kChrOffsetY, &unused_chr_code2)));
...
static AlleleCode* g_loaded_allele_cts[2] = {nullptr, nullptr};
static uintptr_t* g_loaded_allele_presents[2] = {nullptr, nullptr};

static uintptr_t** g_thread_write_genovecs = nullptr;
static uintptr_t** g_thread_write_patch_01_sets = nullptr;
static AlleleCode** g_thread_write_patch_01_vals = nullptr;
static uintptr_t** g_thread_write_patch_10_sets = nullptr;
static AlleleCode** g_thread_write_patch_10_vals = nullptr;
...
THREAD_FUNC_DECL MakePgenThread(void* arg) {
  ...
  __maybe_unused const uintptr_t* write_allele_idx_offsets = g_allele_idx_offsets;
  ...
  /*
  const uintptr_t allele_codes_word_ct = DivUp(2 * raw_sample_ct, kBytesPerWord / sizeof(AlleleCode));
  uintptr_t* write_patch_01_sets = nullptr;
  AlleleCode* write_patch_01_vals = nullptr;
  uintptr_t* write_patch_10_sets = nullptr;
  AlleleCode* write_patch_10_vals = nullptr;
  if (g_thread_write_patch_01_sets) {
    write_patch_01_sets = g_thread_write_patch_01_sets[tidx];
    write_patch_01_vals = g_thread_write_patch_01_vals[tidx];
    write_patch_10_sets = g_thread_write_patch_10_sets[tidx];
    write_patch_10_vals = g_thread_write_patch_10_vals[tidx];
  }
  */
  ...
  __maybe_unused uint32_t cur_allele_ct = 2;
  uint32_t parity = 0;
  while (1) {
    const uint32_t is_last_block = g_is_last_thread_block;
    const uintptr_t cur_block_write_ct = g_cur_block_write_ct;
    ...
    for (; write_idx < write_idx_end; ++write_idx) {
      ...
      if (1) {
        ...
        if (new_sample_idx_to_old) {
          ...
          if (is_dosage) {
            CopyAndResortDosage(cur_dosagepresent, cur_dosagevals, new_sample_idx_to_old, raw_sample_ct, sample_ct, write_dosagepresent, write_dosagevals, &write_dosage_ct, cumulative_popcount_buf);
            if (is_dphase) {
              // structure is isomorphic to dosageraw
              CopyAndResortDosage(cur_dphasepresent, R_CAST(Dosage*, cur_dphasedelta), new_sample_idx_to_old, raw_sample_ct, sample_ct, write_dphasepresent, R_CAST(Dosage*, write_dphasedeltas), &write_dphase_ct, cumulative_popcount_buf);
            }
          }
          ...
          if (is_dosage) {
            CopyDosageSubset(cur_dosagepresent, cur_dosagevals, sample_include, sample_ct, read_dosage_ct, write_dosagepresent, write_dosagevals, &write_dosage_ct);
            if (is_dphase) {
              CopyDosageSubset(cur_dphasepresent, R_CAST(Dosage*, cur_dphasedelta), sample_include, sample_ct, read_dphase_ct, write_dphasepresent, R_CAST(Dosage*, write_dphasedeltas), &write_dphase_ct);
            }
          }
        } else {
        ...
        if (refalt1_select_iter && (refalt1_select_iter[write_idx][0] == 1)) {
        ...
        // need to perform trim-alts erasure here
        // spgwp should always be non-null here
        /*
        uintptr_t* read_patch_01_set = cur_genovec_end;
        cur_genovec_end = &(cur_genovec_end[raw_sample_ctaw]);
        AlleleCode* read_patch_01_vals = R_CAST(AlleleCode*, cur_genovec_end);
        cur_genovec_end = &(cur_genovec_end[allele_codes_word_ct]);
        uintptr_t* read_patch_10_set = cur_genovec_end;
        cur_genovec_end = &(cur_genovec_end[raw_sample_ctaw]);
        AlleleCode* read_patch_10_vals = R_CAST(AlleleCode*, cur_genovec_end);
        cur_genovec_end = &(cur_genovec_end[allele_codes_word_ct]);
        if (is_hphase) {
...
#  ifdef USE_AVX2
        ...
        // todo: try to apply InverseMovemask functions here
...
PglErr MakePgenRobust(const uintptr_t* sample_include, const uint32_t* new_sample_idx_to_old, const uintptr_t* variant_include, const uintptr_t* allele_idx_offsets, const uintptr_t* allele_presents, const STD_ARRAY_PTR_DECL(AlleleCode, 2, refalt1_select), const uint32_t* new_variant_idx_to_old, uint32_t raw_sample_ct, uint32_t sample_ct, uint32_t raw_variant_ct, uint32_t variant_ct, uint32_t hard_call_thresh, uint32_t dosage_erase_thresh, MakePlink2Flags make_plink2_flags, PgenReader* simple_pgrp, char* outname, char* outname_end) {
  ...
  PreinitSpgw(&spgw);
  {
    ...
    if ((make_plink2_flags & kfMakeBed) || ((make_plink2_flags & (kfMakePgen | (kfMakePgenFormatBase * 3))) == (kfMakePgen | kfMakePgenFormatBase))) {
      ...
      const uint32_t read_dosage_present = (read_phase_dosage_gflags / kfPgenGlobalDosagePresent) & 1;
      ...
      const uint32_t read_dphase_present = (read_phase_dosage_gflags / kfPgenGlobalDosagePhasePresent) & 1;
      ...
      if (g_hard_call_halfdist && (read_hphase_present || read_or_write_dphase_present)) {
        ...
        write_phase_dosage_gflags |= kfPgenGlobalHardcallPhasePresent | kfPgenGlobalDosagePhasePresent;
      ...
      if (new_sample_idx_to_old || subsetting_required) {
        ...
        if (input_biallelic) {
          ...
          if (unlikely(bigstack_alloc_w(DivUp(2 * sample_ct * sizeof(AlleleCode), kBytesPerWord), &(g_thread_write_genovecs[0])))) {
            goto MakePgenRobust_ret_NOMEM;
          }
        }
      } else {
      ...
      const uint32_t sample_ctl = BitCtToWordCt(sample_ct);
      g_thread_write_patch_01_sets = nullptr;
      g_thread_write_patch_01_vals = nullptr;
      g_thread_write_patch_10_sets = nullptr;
      g_thread_write_patch_10_vals = nullptr;
      if (write_allele_idx_offsets) {
        // todo: allocate g_thread_write_{01,10}_{sets,vals} here
        logputs("Error: Multiallelic .pgen write is under development.\n");
        reterr = kPglRetNotYetSupported;
        goto MakePgenRobust_ret_1;
      }
      ...
      if (write_allele_idx_offsets) {
        ...
        g_loaded_allele_cts[0] = nullptr;
        g_loaded_allele_cts[1] = nullptr;
        g_loaded_allele_presents[0] = nullptr;
        g_loaded_allele_presents[1] = nullptr;
      }
...
PglErr MakePlink2NoVsort(const char* xheader, const uintptr_t* sample_include, const PedigreeIdInfo* piip, const uintptr_t* sex_nm, const uintptr_t* sex_male, const PhenoCol* pheno_cols, const char* pheno_names, const uint32_t* new_sample_idx_to_old, const uintptr_t* variant_include, const ChrInfo* cip, const uint32_t* variant_bps, const char* const* variant_ids, const uintptr_t* allele_idx_offsets, const char* const* allele_storage, const uintptr_t* allele_presents, const STD_ARRAY_PTR_DECL(AlleleCode, 2, refalt1_select), const uintptr_t* pvar_qual_present, const float* pvar_quals, const uintptr_t* pvar_filter_present, const uintptr_t* pvar_filter_npass, const char* const* pvar_filter_storage, const char* pvar_info_reload, const double* variant_cms, uintptr_t xheader_blen, InfoFlags info_flags, uint32_t raw_sample_ct, uint32_t sample_ct, uint32_t pheno_ct, uintptr_t max_pheno_name_blen, uint32_t raw_variant_ct, uint32_t variant_ct, uint32_t max_allele_slen, uint32_t max_filter_slen, uint32_t info_reload_slen, uint32_t max_thread_ct, uint32_t hard_call_thresh, uint32_t dosage_erase_thresh, MakePlink2Flags make_plink2_flags, PvarPsamFlags pvar_psam_flags, uintptr_t pgr_alloc_cacheline_ct, PgenFileInfo* pgfip, PgenReader* simple_pgrp, char* outname, char* outname_end) {
  ...
  PglErr reterr = kPglRetSuccess;
  {
    ...
    if ((make_plink2_flags & kfMakeBed) || ((make_plink2_flags & (kfMakePgen | (kfMakePgenFormatBase * 3))) == (kfMakePgen | kfMakePgenFormatBase))) {
      ...
      // PgrGetRaw() tends to be the bottleneck when dosage data is present.
      // Also rather not duplicate complicated multiallelic variant handling
      // code here.
      // To simplify things a bit, we also use the fallback when
      // --set-hh-missing/--set-mixed-mt-missing may generate dosages from
      // scratch.
      if (allele_idx_offsets || (pgfip->gflags & kfPgenGlobalDosagePresent) ||
          g_plink2_write_flags & (kfPlink2WriteSetHhMissingKeepDosage | kfPlink2WriteSetMixedMtMissingKeepDosage)) {
        goto MakePlink2NoVsort_fallback;
      }
      assert(variant_ct);
      assert(sample_ct);
      g_allele_idx_offsets = nullptr;
      if (variant_ct == raw_variant_ct) {
      ...
      PgenGlobalFlags read_phase_dosage_gflags = pgfip->gflags & kfPgenGlobalHardcallPhasePresent;
      if (make_plink2_flags & kfMakePgenErasePhase) {
        read_phase_dosage_gflags &= ~kfPgenGlobalHardcallPhasePresent;
      }
      if (read_phase_dosage_gflags && (variant_ct < raw_variant_ct)) {
      ...
      g_dosage_erase_halfdist = kDosage4th - dosage_erase_thresh;
      const uint32_t read_hphase_present = (read_phase_dosage_gflags / kfPgenGlobalHardcallPhasePresent) & 1;
      const PgenGlobalFlags write_phase_dosage_gflags = read_phase_dosage_gflags;
      uint32_t read_or_write_hphase_present = read_hphase_present;
      ...
      MpgwInitPhase1(nullptr, variant_ct, sample_ct, write_phase_dosage_gflags, &alloc_base_cacheline_ct, &mpgw_per_thread_cacheline_ct, &vrec_len_byte_ct, &vblock_cacheline_ct);
      ...
      if (new_sample_idx_to_old || subsetting_required) {
        uintptr_t writebuf_byte_ct = QuaterCtToByteCt(sample_ct);
      ...
      reterr = MpgwInitPhase2(outname, nullptr, pgfip->nonref_flags, variant_ct, sample_ct, write_phase_dosage_gflags, nonref_flags_storage, vrec_len_byte_ct, vblock_cacheline_ct, calc_thread_ct, mpgw_alloc, mpgwp);
      if (unlikely(reterr)) {
        goto MakePlink2NoVsort_ret_1;
      }
      g_sample_include = subsetting_required? sample_include : nullptr;
