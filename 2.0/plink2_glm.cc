  ...
  PreinitRLstream(&rls);
  {
    ...
    while (1) {
      if (!IsEolnKns(*linebuf_first_token)) {
        ...
        const char* read_ptr = linebuf_first_token;
    ...
    *local_sample_ct_ptr = local_sample_ct;
    ...
    if (new_sample_ct < orig_sample_ct) {
      ...
      *sample_include_ptr = sample_include_copy;
      BigstackEndReset(RLstreamMemStart(&rls));
    ...
    reterr = RetargetRLstreamRaw(local_pvar_fname, &rls, &line_iter);
    if (unlikely(reterr)) {
      goto GlmLocalOpen_ret_READ_RLSTREAM_PSAM;
    }
    line_idx = 0;
    ...
    if (is_header_line) {
      ...
      line_iter = NextTokenMult(linebuf_first_token, 4);
    ...
    *local_variant_include_ptr = local_variant_include;
    ...
    while (1) {
      if (!IsEolnKns(*linebuf_first_token)) {
        ...
        line_iter = CurTokenEnd(linebuf_first_token);
        // #CHROM
        ...
        }
        {
          ...
          line_iter = TokenLex(line_iter, col_types, col_skips, 2, token_ptrs, token_slens);
    ...
    *local_variant_ctl_ptr = BitCtToWordCt(local_variant_ct);
    ...
    if (!glm_info_ptr->local_cat_ct) {
      ...
      line_idx = 1;
      if (unlikely(!gzgets(local_covar_rlsp->gz_infile, loadbuf, loadbuf_size))) {
    ...
    if (cur_covar_col->type_code == kPhenoDtypeOther) {
      ...
      *sum_iter++ = covar_sum;
      ...
      for (uint32_t cat_idx = 1; cat_idx != remaining_cat_ct; ++cat_idx) {
        ...
        *sum_iter++ = u31tod(cur_cat_obs_ct);
      }
    }
  }
...
BoolErr GlmAllocFillAndTestPhenoCovarsQt(const uintptr_t* sample_include, const double* pheno_qt, const uintptr_t* covar_include, const PhenoCol* covar_cols, const char* covar_names, uintptr_t sample_ct, uintptr_t covar_ct, uint32_t local_covar_ct, uint32_t covar_max_nonnull_cat_ct, uintptr_t extra_cat_ct, uintptr_t max_covar_name_blen, double max_corr, double vif_thresh, uintptr_t xtx_state, double** pheno_d_ptr, RegressionNmPrecomp** nm_precomp_ptr, double** covars_cmaj_d_ptr, const char*** cur_covar_names_ptr, VifCorrErr* vif_corr_check_result_ptr) {
  ...
  *nm_precomp_ptr = nullptr;
  if (xtx_state) {
    ...
    // x^2 + (2*xtx_state + 3)x + (6*xtx_state)
    // 2x^2 + (2*xtx_state + 5)x + (6*xtx_state + 1)
    // 3x^2 + (2*xtx_state + 5)x + (6*xtx_state + 1)
    // 4x^2 + (4*xtx_state + 5)x + (9*xtx_state - 1)
    // 4x^2 + (4*xtx_state + 6)x + (9*xtx_state - 1)
    if (unlikely(
            bigstack_alloc_d(9 * xtx_state - 1 +
                             new_covar_ct * (6 + 4 * xtx_state + 4 * new_covar_ct),
                             &((*nm_precomp_ptr)->xtx_image)) ||
            bigstack_alloc_d(new_covar_ct * (new_covar_ct + 1), &corr_buf))) {
      return 1;
    }
    (*nm_precomp_ptr)->xt_y_image = &((*nm_precomp_ptr)->xtx_image[(new_covar_ct + xtx_state + 1) * (new_covar_ct + xtx_state + 1)]);
    (*nm_precomp_ptr)->covarx_dotprod_inv = &((*nm_precomp_ptr)->xt_y_image[new_covar_ct + xtx_state + 1]);
...
static inline void GenoarrToFloats(const uintptr_t* genoarr, uint32_t sample_ct, float* floatbuf) {
  GenoarrLookup16x4bx2(genoarr, kSmallFloatPairs, sample_ct, floatbuf);
}
...
uintptr_t GetLogisticWorkspaceSize(uint32_t sample_ct, uint32_t biallelic_predictor_ct, uint32_t max_extra_allele_ct, uint32_t constraint_ct, uint32_t genof_buffer_needed, uint32_t is_sometimes_firth) {
  ...
  // xx = (max_predictor_ct + genof_buffer_needed) * sample_ctav floats
  workspace_size += RoundUpPow2((max_predictor_ct + genof_buffer_needed) * sample_ctav * sizeof(float), kCacheline);
...
static uintptr_t** g_thread_mhc = nullptr;
...
static const AlleleCode* g_omitted_alleles = nullptr;
...
static uintptr_t* g_pheno_cc = nullptr;
static uintptr_t* g_pheno_x_cc = nullptr;
static uintptr_t* g_pheno_y_cc = nullptr;
static uintptr_t* g_gcount_case_interleaved_vec = nullptr;
static uintptr_t* g_gcount_case_interleaved_vec_x = nullptr;
static uintptr_t* g_gcount_case_interleaved_vec_y = nullptr;
...
static double* g_pheno_d = nullptr;
...
static uint32_t g_max_extra_allele_ct = 0;
...
THREAD_FUNC_DECL GlmLogisticThread(void* arg) {
  ...
  const uint32_t max_extra_allele_ct = g_max_extra_allele_ct;
  uint32_t cur_extra_allele_ct = 0;  // temporary
  ...
  while (1) {
    ...
    double* beta_se_iter = g_block_beta_se;
    if (1) {
      beta_se_iter = &(beta_se_iter[2 * max_reported_test_ct * variant_bidx]);
    } else {
      // todo: make this allele-based
    }
    ...
    while (variant_bidx < variant_bidx_end) {
      ...
      if (is_y && g_sample_include_y) {
        ...
        cur_gcount_case_interleaved_vec = g_gcount_case_interleaved_vec_y;
        cur_pheno = g_pheno_y_f;
        nm_precomp = g_nm_precomp_y;
        ...
        cur_gcount_case_interleaved_vec = g_gcount_case_interleaved_vec_x;
        cur_pheno = g_pheno_x_f;
        nm_precomp = g_nm_precomp_x;
        ...
        cur_gcount_case_interleaved_vec = g_gcount_case_interleaved_vec;
        cur_pheno = g_pheno_f;
        nm_precomp = g_nm_precomp;
      ...
      float* nm_predictors_pmaj_buf = S_CAST(float*, arena_alloc_raw_rd((max_predictor_ct + genof_buffer_needed) * sample_ctav * sizeof(float), &workspace_iter));
      ...
      if (cur_constraint_ct) {
        ...
        cur_constraints_con_major = S_CAST(float*, arena_alloc_raw_rd(cur_constraint_ct * max_predictor_ct, &workspace_iter));
        ZeroFArr(cur_constraint_ct * max_predictor_ct, cur_constraints_con_major);
        cur_constraints_con_major[1] = 1;  // [0][1]
        // [1][2] initialization must be deferred
      }
      // assert((uintptr_t)(workspace_iter - workspace_buf) == GetLogisticWorkspaceSize(cur_sample_ct, cur_biallelic_predictor_ct, max_extra_allele_ct, cur_constraint_ct, genof_buffer_needed, is_sometimes_firth));
      ...
      for (; variant_bidx != cur_variant_bidx_end; ++variant_bidx) {
        const uintptr_t variant_uidx = BitIter1(variant_include, &variant_uidx_base, &variant_include_bits);
        {
          ...
          //
          // todo: only compute mach_r2 once (or not at all, if not requested?)
          // per variant in multiallelic case
          ...
          // too few samples, or remaining samples are all-case/all-control
          // probable todo: exclude linearly-dependent later alleles from
          // regression (this could be considered a generalization of the
          // nonzero-variance check)
          ...
          if (missing_ct || (!prev_nm)) {
            ...
            nm_predictors_pmaj_iter = &(nm_predictors_pmaj_iter[literal_covar_ct * nm_sample_ctav]);
          }
          ...
          if (corr_inv && prev_nm && (!cur_extra_allele_ct)) {
            uintptr_t start_pred_idx = 0;
            if (!(model_dominant || model_recessive || joint_hethom)) {
              start_pred_idx = domdev_present + 2;
              semicomputed_biallelic_xtx[cur_predictor_ct] = dosage_sum;
              semicomputed_biallelic_xtx[cur_predictor_ct + 1] = dosage_ssq;
            }
          ...
          if (!is_always_firth) {
            ...
            const double ref_case_dosage = dosage_ceil * u31tod(nm_case_ct) - alt_case_dosage;
          ...
          if (cur_constraint_ct) {
            ...
            // later
            *beta_se_iter2++ = 0.0;
            cur_constraints_con_major[cur_predictor_ct + 2] = 1.0;  // [1][2]
            double chisq;
        ...
        // todo: if domdev, --tests, and/or interactions are included, this
        // must advance once per tested allele
        // otherwise, one entry per variant is enough
...
PglErr ReadLocalCovarBlock(const uintptr_t* sample_include, const uintptr_t* sample_include_x, const uintptr_t* sample_include_y, const uint32_t* sample_include_cumulative_popcounts, const uint32_t* sample_include_x_cumulative_popcounts, const uint32_t* sample_include_y_cumulative_popcounts, const ChrInfo* cip, const uintptr_t* variant_include, const uint32_t* local_sample_uidx_order, const uintptr_t* local_variant_include, uint32_t sample_ct, uint32_t sample_ct_x, uint32_t sample_ct_y, uint32_t variant_uidx_start, uint32_t variant_uidx_end, uint32_t cur_block_variant_ct, uint32_t local_sample_ct, uint32_t local_covar_ct, uint32_t omit_last, uint32_t local_cat_ct, ReadLineStream* local_covar_rlsp, char** local_covar_line_iterp, uint32_t* local_line_idx_ptr, uint32_t* local_xy_ptr, float* local_covars_vcmaj_f_iter, double* local_covars_vcmaj_d_iter, uint32_t* local_sample_idx_order) {
  ...
  while (variant_bidx < cur_block_variant_ct) {
    ...
    for (; variant_bidx != cur_variant_bidx_end; ++variant_bidx) {
      ...
      local_covar_line_iter = K_CAST(char*, linebuf_iter);
      if (local_covars_vcmaj_f_iter) {
  ...
  *local_covar_line_iterp = local_covar_line_iter;
...
PglErr GlmLogistic(const char* cur_pheno_name, const char* const* test_names, const char* const* test_names_x, const char* const* test_names_y, const uint32_t* variant_bps, const char* const* variant_ids, const char* const* allele_storage, const GlmInfo* glm_info_ptr, const uint32_t* local_sample_uidx_order, const uintptr_t* local_variant_include, const char* outname, uint32_t raw_variant_ct, uint32_t max_chr_blen, double ci_size, double ln_pfilter, double output_min_ln, uint32_t max_thread_ct, uintptr_t pgr_alloc_cacheline_ct, uintptr_t overflow_buf_size, uint32_t local_sample_ct, PgenFileInfo* pgfip, ReadLineStream* local_covar_rlsp, uintptr_t* valid_variants, uintptr_t* valid_alleles, double* orig_ln_pvals, double* orig_permstat, uintptr_t* valid_allele_ct_ptr) {
  ...
  PreinitCstream(&css);
  {
    ...
    char* local_covar_line_iter = nullptr;
    ...
    uintptr_t workspace_alloc = GetLogisticWorkspaceSize(sample_ct, biallelic_predictor_ct, max_extra_allele_ct, constraint_ct, genof_buffer_needed, is_sometimes_firth);
    if (sample_ct_x) {
      const uintptr_t workspace_alloc_x = GetLogisticWorkspaceSize(sample_ct_x, biallelic_predictor_ct_x, max_extra_allele_ct, constraint_ct_x, genof_buffer_needed, is_sometimes_firth);
      if (workspace_alloc_x > workspace_alloc) {
        workspace_alloc = workspace_alloc_x;
      }
    }
    if (sample_ct_y) {
      const uintptr_t workspace_alloc_y = GetLogisticWorkspaceSize(sample_ct_y, biallelic_predictor_ct_y, max_extra_allele_ct, constraint_ct_y, genof_buffer_needed, is_sometimes_firth);
    ...
    for (uint32_t uii = 0; uii != 2; ++uii) {
      ...
      if (local_covar_ct) {
        ...
        if (unlikely(bigstack_alloc_f(read_block_size * max_sample_ct * local_covar_ct, &(g_local_covars_vcmaj_f[uii])))) {
          goto GlmLogistic_ret_NOMEM;
        }
      } else {
    ...
    for (uint32_t variant_idx = 0; ; ) {
      ...
      if (variant_idx) {
        ...
        for (uint32_t variant_bidx = 0; variant_bidx != prev_block_variant_ct; ++variant_bidx) {
          ...
          const uint32_t cur_extra_allele_ct = cur_allele_ct - 2;
          if (omitted_alleles) {
            ...
            variant_is_valid |= allele_is_valid;
            {
              ...
              if (cur_extra_allele_ct) {
                if (beta_se_multiallelic_fused) {
                  ...
                  } else if (nonomitted_allele_idx == cur_extra_allele_ct) {
                    inner_reported_test_ct -= include_intercept;
                  } else {
                    inner_reported_test_ct = 1;
                  }
                } else if (!hide_covar) {
                  inner_reported_test_ct += cur_extra_allele_ct;
                }
              }
...
uint32_t GenoarrToDoublesRemoveMissing(const uintptr_t* genoarr, const double* __restrict table, uint32_t sample_ct, double* dst) {
  assert(sample_ct);
  const uint32_t sample_ctl2m1 = (sample_ct - 1) / kBitsPerWordD2;
  uint32_t subgroup_len = kBitsPerWordD2;
  double* dst_iter = dst;
...
THREAD_FUNC_DECL GlmLinearThread(void* arg) {
  ...
  if (g_thread_mhc) {
    ...
    patch_01_set = g_thread_mhc[tidx];
    patch_01_vals = R_CAST(AlleleCode*, &(patch_01_set[max_sample_ctl]));
    AlleleCode* patch_01_vals_end = &(patch_01_vals[max_sample_ct]);
    VecAlignUp(&patch_01_vals_end);
    patch_10_set = R_CAST(uintptr_t*, patch_01_vals_end);
    patch_10_vals = R_CAST(AlleleCode*, &(patch_10_set[max_sample_ctl]));
  }
  uint32_t variant_idx_offset = 0;
  ...
  while (1) {
    ...
    if (max_extra_allele_ct) {
      allele_bidx = variant_bidx + CountExtraAlleles(variant_include, allele_idx_offsets, g_read_variant_uidx_starts[0], g_read_variant_uidx_starts[tidx], 0);;
    }
    double* beta_se_iter = g_block_beta_se;
    if (beta_se_multiallelic_fused) {
      beta_se_iter = &(beta_se_iter[2 * max_reported_test_ct * variant_bidx]);
    ...
    LinearAuxResult* block_aux_iter = &(g_linear_block_aux[allele_bidx]);
    ...
    while (variant_bidx < variant_bidx_end) {
      ...
      for (; variant_bidx != cur_variant_bidx_end; ++variant_bidx) {
        ...
        uint32_t multidosage_sample_ct = 0;
        if (!allele_ct_m2) {
          PglErr reterr = PgrGetD(cur_sample_include, cur_sample_include_cumulative_popcounts, cur_sample_ct, variant_uidx, pgrp, genovec, dosage_present, dosage_main, &dosage_ct);
          if (unlikely(reterr)) {
            g_error_ret = reterr;
          ...
          PglErr reterr = PgrGetMD(cur_sample_include, cur_sample_include_cumulative_popcounts, cur_sample_ct, variant_uidx, pgrp, genovec, patch_01_set, patch_01_vals, patch_10_set, patch_10_vals, &patch_01_ct, &patch_10_ct, dosage_present, dosage_main, &dosage_ct, nullptr, nullptr, nullptr, nullptr, &multidosage_sample_ct);
        ...
        if (!allele_ct_m2) {
          ...
          const_allele_present = (!dosage_ct) && ((genocounts[0] == nm_sample_ct) || (genocounts[1] == nm_sample_ct) || (genocounts[2] == nm_sample_ct));
          if (!missing_ct) {
            ...
            if (!sparse_optimization) {
              ...
              if (dosage_ct) {
                ...
                for (uint32_t dosage_idx = 0; dosage_idx != dosage_ct; ++dosage_idx) {
                  const uintptr_t sample_idx = BitIter1(dosage_present, &sample_idx_base, &dosage_present_bits);
                  const uintptr_t dosage_val = dosage_main[dosage_idx];
                  // 32768 -> 2, 16384 -> 1, 0 -> 0
                  genotype_vals[sample_idx] = kRecipDosageMid * swtod(dosage_val);
                  const uint64_t prev_val = kDosageMid * GetQuaterarrEntry(genovec, sample_idx);
                  dosage_sum += dosage_val - prev_val;
                  dosage_ssq += dosage_val * dosage_val - prev_val * prev_val;
                }
              }
            }
          } else {
            if (!dosage_ct) {
              ...
              for (uint32_t sample_idx = 0; sample_idx != nm_sample_ct; ++sample_idx) {
                const uintptr_t sample_midx = BitIter1(sample_nm, &sample_midx_base, &sample_nm_bits);
                const uintptr_t cur_geno = GetQuaterarrEntry(genovec, sample_midx);
                double cur_val;
                if (IsSet(dosage_present, sample_midx)) {
                  const uintptr_t dosage_val = dosage_main[dosage_idx++];
          ...
          // multiallelic.
          // We don't filter out all-zero columns; it's the caller's
          // responsibility to use trim-alts first when necessary.
          // Punt on sparse_optimization for now; may be worth revisiting after
          // multiallelic dosage implemented.
          // For whatever it's worth, dosage_ct == 0 is guaranteed for now if
          // we reach here.
          assert(!dosage_ct);
          ...
          if (omitted_allele_idx) {
            ...
            if (!missing_ct) {
              GenoarrLookup16x8bx2(genovec, kSmallInvDoublePairs, nm_sample_ct, genotype_vals);
            } else {
              GenoarrToDoublesRemoveMissing(genovec, kSmallInvDoubles, cur_sample_ct, genotype_vals);
            }
          }
          ...
          if (omitted_allele_idx != 1) {
            ...
            if (!missing_ct) {
              GenoarrLookup16x8bx2(genovec, kSmallDoublePairs, nm_sample_ct, alt1_start);
            } else {
              GenoarrToDoublesRemoveMissing(genovec, kSmallDoubles, cur_sample_ct, alt1_start);
            }
          }
          ...
          if (patch_01_ct) {
            const uintptr_t* patch_set_nm = patch_01_set;
            if (missing_ct) {
              CopyBitarrSubset(patch_01_set, sample_nm, nm_sample_ct, tmp_nm);
              patch_set_nm = tmp_nm;
            }
            uintptr_t sample_idx_base = 0;
            ...
            if (!omitted_allele_idx) {
              for (uint32_t uii = 0; uii != patch_01_ct; ++uii) {
                ...
                const uint32_t allele_code = patch_01_vals[uii];
          ...
          uintptr_t alt1_het_ct = genocounts[1] - patch_01_ct;
          if (patch_10_ct) {
            const uintptr_t* patch_set_nm = patch_10_set;
            if (missing_ct) {
              CopyBitarrSubset(patch_10_set, sample_nm, nm_sample_ct, tmp_nm);
              patch_set_nm = tmp_nm;
            }
            uintptr_t sample_idx_base = 0;
            uintptr_t cur_bits = patch_set_nm[0];
            if (!omitted_allele_idx) {
              ...
              for (uint32_t uii = 0; uii != patch_10_ct; ++uii) {
                const uintptr_t sample_idx = BitIter1(patch_set_nm, &sample_idx_base, &cur_bits);
                const uint32_t ac0 = patch_10_vals[2 * uii];
                const uint32_t ac1 = patch_10_vals[2 * uii + 1];
                if (ac0 == ac1) {
                  machr2_dosage_ssqs[ac0] += 1;
                  alt1_start[sample_idx] = 0.0;
          ...
          const uintptr_t alt1_hom_ct = genocounts[2] - patch_10_ct;
        ...
        for (uint32_t allele_idx = 0; allele_idx != allele_ct; ++allele_idx) {
          ...
          if (is_xchr_model_1) {
            ...
            if (allele_idx > 1 - omitted_allele_idx) {
              geno_col = &(multi_start[(cur_predictor_ct - (allele_ct - allele_idx) + (allele_idx < omitted_allele_idx)) * nm_sample_ct]);
            }
            a1_dosage = 0.0;
            for (uint32_t sample_idx = 0; sample_idx != nm_sample_ct; ++sample_idx) {
        ...
        if ((nm_sample_ct <= cur_predictor_ct) || const_allele_present) {
          ...
          if (!sparse_optimization) {
            ...
            if (missing_ct || (!prev_nm)) {
              ...
              // bugfix (15 Aug 2018): this was not handling --parameters
              // correctly whne a covariate was only needed as part of an
              // interaction
              parameter_uidx += cur_covar_ct;
  ...
  PreinitCstream(&css);
  {
    ...
    for (uint32_t uii = 0; uii != 2; ++uii) {
      ...
      if (local_covar_ct) {
        ...
        if (unlikely(bigstack_alloc_d(read_block_size * max_sample_ct * local_covar_ct, &(g_local_covars_vcmaj_d[uii])))) {
          goto GlmLinear_ret_NOMEM;
        }
      } else {
    ...
    *cswritep++ = '#';
    ...
    for (uint32_t variant_idx = 0; ; ) {
      ...
      if (!ts.is_last_block) {
        ...
        if (local_covar_line_iter) {
          reterr = ReadLocalCovarBlock(g_sample_include, g_sample_include_x, g_sample_include_y, g_sample_include_cumulative_popcounts, g_sample_include_x_cumulative_popcounts, g_sample_include_y_cumulative_popcounts, cip, variant_include, local_sample_uidx_order, local_variant_include, sample_ct, sample_ct_x, sample_ct_y, read_block_idx * read_block_size, read_block_idx * read_block_size + cur_read_block_size, cur_block_variant_ct, local_sample_ct, local_covar_ct, (glm_info_ptr->flags / kfGlmLocalOmitLast) & 1, glm_info_ptr->local_cat_ct, local_covar_rlsp, &local_covar_line_iter, &local_line_idx, &local_xy, nullptr, g_local_covars_vcmaj_d[parity], local_sample_idx_order);
          if (unlikely(reterr)) {
            goto GlmLinear_ret_1;
          }
        }
              ...
              if (ln_pfilter <= 0.0) {
                ...
                if (!cur_constraint_ct) {
                  if (primary_beta == 0.0) {
                    ...
                    primary_ln_pval = TstatToLnP(primary_tstat, auxp->sample_obs_ct - cur_biallelic_predictor_ct - cur_extra_allele_ct);
                  }
                } else {
                  // possible todo: support for F-distribution p-values instead
                  // of asymptotic chi-square p-values
              ...
              for (uint32_t allele_test_idx = 0; allele_test_idx != inner_reported_test_ct; ++allele_test_idx) {
                ...
                *cswritep++ = '\t';
                ...
                if ((!cur_constraint_ct) || (test_idx != primary_reported_test_idx)) {
                  ...
                  if (allele_is_valid) {
                    tstat = beta / se;
                    ln_pval = TstatToLnP(tstat, auxp->sample_obs_ct - cur_biallelic_predictor_ct - cur_extra_allele_ct);
                  }
                  if (beta_col) {
                    *cswritep++ = '\t';
                    if (allele_is_valid) {
...
PglErr GlmMain(const uintptr_t* orig_sample_include, const SampleIdInfo* siip, const uintptr_t* sex_nm, const uintptr_t* sex_male, const PhenoCol* pheno_cols, const char* pheno_names, const PhenoCol* covar_cols, const char* covar_names, const uintptr_t* orig_variant_include, const ChrInfo* cip, const uint32_t* variant_bps, const char* const* variant_ids, const uintptr_t* allele_idx_offsets, const AlleleCode* maj_alleles, const char* const* allele_storage, const GlmInfo* glm_info_ptr, const AdjustInfo* adjust_info_ptr, const APerm* aperm_ptr, const char* local_covar_fname, const char* local_pvar_fname, const char* local_psam_fname, uint32_t raw_sample_ct, uint32_t orig_sample_ct, uint32_t pheno_ct, uintptr_t max_pheno_name_blen, uint32_t orig_covar_ct, uintptr_t max_covar_name_blen, uint32_t raw_variant_ct, uint32_t orig_variant_ct, uint32_t max_variant_id_slen, uint32_t max_allele_slen, uint32_t xchr_model, double ci_size, double vif_thresh, double ln_pfilter, double output_min_ln, uint32_t max_thread_ct, uintptr_t pgr_alloc_cacheline_ct, PgenFileInfo* pgfip, PgenReader* simple_pgrp, char* outname, char* outname_end) {
  ...
  PreinitGzTokenStream(&gts);
  {
    ...
    if (local_covar_fname) {
      reterr = GlmLocalOpen(local_covar_fname, local_pvar_fname, local_psam_fname, siip->sample_ids, cip, variant_bps, variant_ids, glm_info_ptr, raw_sample_ct, siip->max_sample_id_blen, raw_variant_ct, &orig_sample_include, &sex_nm, &sex_male, &early_variant_include, &orig_sample_ct, &variant_ct, &local_covar_rls, &local_sample_uidx_order, &local_variant_include, &local_sample_ct, &local_variant_ctl, &local_covar_ct);
      if (unlikely(reterr)) {
        goto GlmMain_ret_1;
      }
    }
    ...
    *outname_end = '.';
    ...
    g_omitted_alleles = (glm_flags & kfGlmOmitRef)? nullptr : maj_alleles;
    ...
    if (glm_info_ptr->parameters_range_list.name_ct) {
      if (unlikely(
              ...
              bigstack_alloc_w(biallelic_raw_predictor_ctl, &g_parameter_subset) ||
              bigstack_alloc_w(biallelic_raw_predictor_ctl, &g_parameter_subset_x) ||
              bigstack_alloc_w(biallelic_raw_predictor_ctl, &g_parameter_subset_y))) {
        goto GlmMain_ret_NOMEM;
      }
      ...
      if (add_sex_covar && first_interaction_pred_uidx) {
        ...
        if (unlikely(bigstack_calloc_w(biallelic_raw_predictor_ctl, &parameter_subset_reshuffle_buf))) {
          goto GlmMain_ret_NOMEM;
        }
        ...
        memcpy(raw_parameter_subset, parameter_subset_reshuffle_buf, biallelic_raw_predictor_ctl * sizeof(intptr_t));
    ...
    g_max_extra_allele_ct = max_extra_allele_ct;
    ...
    for (uint32_t pheno_idx = 0; pheno_idx != pheno_ct; ++pheno_idx) {
      ...
      if (raw_parameter_subset) {
        CollapseParameterSubset(covar_include, raw_parameter_subset, domdev_present, raw_covar_ct, covar_ct, add_interactions, g_parameter_subset, &biallelic_predictor_ct);
      }
      ...
      if (cur_sample_include_x) {
        ...
        if ((!x_samples_are_different) && (covar_ct == covar_ct_x) && wordsequal(covar_include, covar_include_x, raw_covar_ctl)) {
          ...
          if (!sample_ct_x) {
            ...
            if (raw_parameter_subset) {
              CollapseParameterSubset(covar_include, raw_parameter_subset, domdev_present, raw_covar_ct, covar_ct_x, add_interactions, g_parameter_subset_x, &biallelic_predictor_ct_x);
            }
      ...
      if (cur_sample_include_y) {
        ...
        if ((!y_samples_are_different) && (covar_ct == covar_ct_y) && wordsequal(covar_include, covar_include_y, raw_covar_ctl)) {
          ...
          if (!sample_ct_y) {
            ...
            if (raw_parameter_subset) {
              CollapseParameterSubset(covar_include, raw_parameter_subset, domdev_present, raw_covar_ct, covar_ct_y, add_interactions, g_parameter_subset_y, &biallelic_predictor_ct_y);
            }
      ...
      if (unlikely(AllocAndInitReportedTestNames(g_parameter_subset, cur_covar_names, glm_flags, covar_ct + extra_cat_ct, &cur_test_names))) {
        goto GlmMain_ret_NOMEM;
      }
      if (sample_ct_x) {
        if (unlikely(AllocAndInitReportedTestNames(g_parameter_subset_x, cur_covar_names_x, glm_flags, covar_ct_x + extra_cat_ct_x, &cur_test_names_x))) {
          goto GlmMain_ret_NOMEM;
        }
      }
      if (sample_ct_y) {
        if (unlikely(AllocAndInitReportedTestNames(g_parameter_subset_y, cur_covar_names_y, glm_flags, covar_ct_y + extra_cat_ct_y, &cur_test_names_y))) {
          goto GlmMain_ret_NOMEM;
        }
      }
      if (joint_test) {
      ...
      if (sample_ct_x) {
        if (unlikely(bigstack_alloc_u32(raw_sample_ctl, &g_sample_include_x_cumulative_popcounts))) {
          goto GlmMain_ret_NOMEM;
        }
      ...
      if (sample_ct_y) {
        if (unlikely(bigstack_alloc_u32(raw_sample_ctl, &g_sample_include_y_cumulative_popcounts))) {
          goto GlmMain_ret_NOMEM;
        }
      ...
      if (report_adjust || perms_total) {
        if (unlikely(bigstack_alloc_d(cur_variant_ct, &orig_ln_pvals))) {
          goto GlmMain_ret_NOMEM;
        }
        ...
        if (perms_total) {
          if (is_logistic) {
            if (unlikely(bigstack_alloc_d(cur_variant_ct, &orig_permstat))) {
              goto GlmMain_ret_NOMEM;
            }
          } else {
      ...
      if (unlikely(AllocAndFillSubsetChrFoVidxStart(cur_variant_include, cip, &g_subset_chr_fo_vidx_start))) {
        goto GlmMain_ret_NOMEM;
      }
      ...
      if (is_logistic) {
        if (max_extra_allele_ct) {
          logerrputs("Error: --glm logistic-regression multiallelic variant support is currently under development.\n");
          reterr = kPglRetNotYetSupported;
          goto GlmMain_ret_1;
        }
        reterr = GlmLogistic(cur_pheno_name, cur_test_names, cur_test_names_x, cur_test_names_y, glm_pos_col? variant_bps : nullptr, variant_ids, allele_storage, glm_info_ptr, local_sample_uidx_order, cur_local_variant_include, outname, raw_variant_ct, max_chr_blen, ci_size, ln_pfilter, output_min_ln, max_thread_ct, pgr_alloc_cacheline_ct, overflow_buf_size, local_sample_ct, pgfip, &local_covar_rls, valid_variants, valid_alleles, orig_ln_pvals, orig_permstat, &valid_allele_ct);
      } else {
        reterr = GlmLinear(cur_pheno_name, cur_test_names, cur_test_names_x, cur_test_names_y, glm_pos_col? variant_bps : nullptr, variant_ids, allele_storage, glm_info_ptr, local_sample_uidx_order, cur_local_variant_include, outname, raw_variant_ct, max_chr_blen, ci_size, ln_pfilter, output_min_ln, max_thread_ct, pgr_alloc_cacheline_ct, overflow_buf_size, local_sample_ct, pgfip, &local_covar_rls, valid_variants, valid_alleles, orig_ln_pvals, &valid_allele_ct);
      }
      if (unlikely(reterr)) {
        goto GlmMain_ret_1;
      }
