...
PglErr GlmLocalOpen(const char* local_covar_fname, const char* local_pvar_fname, const char* local_psam_fname, const char* sample_ids, const ChrInfo* cip, const uint32_t* variant_bps, const char* const* variant_ids, const GlmInfo* glm_info_ptr, uint32_t raw_sample_ct, uintptr_t max_sample_id_blen, uint32_t raw_variant_ct, const uintptr_t** sample_include_ptr, const uintptr_t** sex_nm_ptr, const uintptr_t** sex_male_ptr, const uintptr_t** variant_include_ptr, uint32_t* sample_ct_ptr, uint32_t* variant_ct_ptr, TextRstream* local_covar_trsp, uint32_t** local_sample_uidx_order_ptr, uintptr_t** local_variant_include_ptr, uint32_t* local_sample_ct_ptr, uint32_t* local_variant_ctl_ptr, uint32_t* local_covar_ct_ptr) {
  ...
  PreinitTextRstream(&trs);
  {
    ...
    if (unlikely(reterr)) {
      goto GlmLocalOpen_ret_PSAM_RSTREAM_FAIL;
    }
    ...
    *local_sample_ct_ptr = local_sample_ct;
    ...
    if (new_sample_ct < orig_sample_ct) {
      ...
      *sample_include_ptr = sample_include_copy;
      BigstackEndReset(TextRstreamMemStart(&trs));
    ...
    reterr = TextRetarget(local_pvar_fname, &trs);
    if (unlikely(reterr)) {
      goto GlmLocalOpen_ret_PSAM_RSTREAM_FAIL;
    }
    line_idx = 0;
    do {
      ...
      if (unlikely(reterr)) {
        ...
        goto GlmLocalOpen_ret_PVAR_RSTREAM_FAIL;
      }
      is_header_line = (line_start[0] == '#');
    ...
    *local_variant_ctl_ptr = BitCtToWordCt(local_variant_ct);
    ...
    if (CleanupTextRstream(&trs, &reterr)) {
      logerrprintfww(kErrprintfFread, local_pvar_fname, strerror(errno));
      goto GlmLocalOpen_ret_1;
    }
    ...
    } else if (unlikely(loadbuf_size < 2 * kDecompressChunkSize)) {
      // this shouldn't be possible
      goto GlmLocalOpen_ret_NOMEM;
    }
    // don't formally allocate
    char* dst = R_CAST(char*, g_bigstack_base);
    reterr = TextRfileOpenEx(local_covar_fname, kMaxLongLine, loadbuf_size, dst, &local_covar_trf);
    if (unlikely(reterr)) {
    ...
    if (!glm_info_ptr->local_cat_ct) {
      line_idx = 1;
      reterr = TextRfileNextLine(&local_covar_trf, &line_start);
      if (unlikely(reterr)) {
        if (reterr == kPglRetEof) {
          snprintf(g_logbuf, kLogbufSize, "Error: %s is empty.\n", local_covar_fname);
          goto GlmLocalOpen_ret_MALFORMED_INPUT_WW;
  ...
  GlmLocalOpen_ret_NOMEM:
    reterr = kPglRetNomem;
    break;
  GlmLocalOpen_ret_PSAM_RSTREAM_FAIL:
    TextRstreamErrPrint(local_psam_fname, &trs);
    break;
  GlmLocalOpen_ret_PVAR_RSTREAM_FAIL:
    TextRstreamErrPrint(local_pvar_fname, &trs);
    break;
  GlmLocalOpen_ret_RFILE_FAIL:
    TextRfileErrPrint(local_covar_fname, &local_covar_trf);
    break;
 ...
 GlmLocalOpen_ret_1:
  ...
  CleanupTextRfile(&local_covar_trf, &reterr);
#else
  assert(!CleanupTextRstream(&trs, &reterr));
  assert(!CleanupTextRfile(&local_covar_trf, &reterr));
#endif
...
static uintptr_t* g_joint_test_params = nullptr;
static uintptr_t* g_joint_test_params_x = nullptr;
static uintptr_t* g_joint_test_params_y = nullptr;
...
static uint32_t g_tests_flag = 0;
...
THREAD_FUNC_DECL GlmLogisticThread(void* arg) {
  ...
  if (g_thread_mhc) {
    ...
    pgv.patch_01_set = g_thread_mhc[tidx];
  ...
  while (1) {
    ...
    while (variant_bidx < variant_bidx_end) {
      ...
      if (is_y && g_sample_include_y) {
        ...
        cur_joint_test_params = g_joint_test_params_y;
        ...
        cur_joint_test_params = g_joint_test_params_x;
        ...
        cur_joint_test_params = g_joint_test_params;
  ...
  while (variant_bidx < cur_block_variant_ct) {
    ...
    for (; variant_bidx != cur_variant_bidx_end; ++variant_bidx) {
      ...
      if (!IsSet(local_variant_include, local_line_idx)) {
        ...
        PglErr reterr = TextSkipNz(local_line_idx_target_m1 - local_line_idx, local_covar_trsp);
        if (unlikely(reterr)) {
          if (reterr == kPglRetEof) {
            logputs("\n");
          ...
          TextRstreamErrPrint("--glm local-covar= file", local_covar_trsp);
          return reterr;
        }
      ...
      PglErr reterr = TextNextLineLstrip(local_covar_trsp, &local_covar_line_start);
      if (unlikely(reterr)) {
        if (reterr == kPglRetEof) {
          logputs("\n");
          logerrputs("Error: --glm local-covar= file has fewer lines than local-pvar= file.\n");
...
PglErr GlmLogistic(const char* cur_pheno_name, const char* const* test_names, const char* const* test_names_x, const char* const* test_names_y, const uint32_t* variant_bps, const char* const* variant_ids, const char* const* allele_storage, const GlmInfo* glm_info_ptr, const uint32_t* local_sample_uidx_order, const uintptr_t* local_variant_include, const char* outname, uint32_t raw_variant_ct, uint32_t max_chr_blen, double ci_size, double ln_pfilter, double output_min_ln, uint32_t max_thread_ct, uintptr_t pgr_alloc_cacheline_ct, uintptr_t overflow_buf_size, uint32_t local_sample_ct, PgenFileInfo* pgfip, TextRstream* local_covar_trsp, uintptr_t* valid_variants, uintptr_t* valid_alleles, double* orig_ln_pvals, double* orig_permstat, uintptr_t* valid_allele_ct_ptr) {
  ...
  PreinitCstream(&css);
  {
    ...
    if (local_covar_ct) {
      reterr = TextRewind(local_covar_trsp);
      if (unlikely(reterr)) {
        goto GlmLogistic_ret_RSTREAM_FAIL;
      }
...
PglErr GlmLinear(const char* cur_pheno_name, const char* const* test_names, const char* const* test_names_x, const char* const* test_names_y, const uint32_t* variant_bps, const char* const* variant_ids, const char* const* allele_storage, const GlmInfo* glm_info_ptr, const uint32_t* local_sample_uidx_order, const uintptr_t* local_variant_include, const char* outname, uint32_t raw_variant_ct, uint32_t max_chr_blen, double ci_size, double ln_pfilter, double output_min_ln, uint32_t max_thread_ct, uintptr_t pgr_alloc_cacheline_ct, uintptr_t overflow_buf_size, uint32_t local_sample_ct, PgenFileInfo* pgfip, TextRstream* local_covar_trsp, uintptr_t* valid_variants, uintptr_t* valid_alleles, double* orig_ln_pvals, uintptr_t* valid_allele_ct_ptr) {
  ...
  PreinitCstream(&css);
  {
    ...
    if (local_covar_ct) {
      reterr = TextRewind(local_covar_trsp);
      if (unlikely(reterr)) {
        goto GlmLinear_ret_RSTREAM_FAIL;
      }
      if (unlikely(bigstack_alloc_u32(local_sample_ct, &local_sample_idx_order))) {
    ...
    *cswritep++ = '#';
    ...
    for (uint32_t variant_idx = 0; ; ) {
      ...
      if (!ts.is_last_block) {
        ...
        if (local_covar_ct) {
          reterr = ReadLocalCovarBlock(g_sample_include, g_sample_include_x, g_sample_include_y, g_sample_include_cumulative_popcounts, g_sample_include_x_cumulative_popcounts, g_sample_include_y_cumulative_popcounts, cip, variant_include, local_sample_uidx_order, local_variant_include, sample_ct, sample_ct_x, sample_ct_y, read_block_idx * read_block_size, read_block_idx * read_block_size + cur_read_block_size, cur_block_variant_ct, local_sample_ct, local_covar_ct, (glm_info_ptr->flags / kfGlmLocalOmitLast) & 1, glm_info_ptr->local_cat_ct, local_covar_trsp, &local_line_idx, &local_xy, nullptr, g_local_covars_vcmaj_d[parity], local_sample_idx_order);
          if (unlikely(reterr)) {
            goto GlmLinear_ret_1;
          }
        }
...
static uint32_t g_subbatch_size = 0;

THREAD_FUNC_DECL GlmLinearSubbatchThread(void* arg) {
  const uintptr_t tidx = R_CAST(uintptr_t, arg);
  PgenReader* pgrp = g_pgr_ptrs[tidx];
  PgenVariant pgv;
  pgv.genovec = g_genovecs[tidx];
  ...
  const uint32_t max_extra_allele_ct = g_max_extra_allele_ct;
  const uint32_t beta_se_multiallelic_fused = (!domdev_present) && (!g_tests_flag) && (!add_interactions);
  const uint32_t subbatch_size = g_subbatch_size;
  uintptr_t max_sample_ct = MAXV(g_sample_ct, g_sample_ct_x);
  if (max_sample_ct < g_sample_ct_y) {
    max_sample_ct = g_sample_ct_y;
  }
...
PglErr GlmLinearBatch(const uintptr_t* pheno_batch, const PhenoCol* pheno_cols, const char* pheno_names, const char* const* test_names, const char* const* test_names_x, const char* const* test_names_y, const uint32_t* variant_bps, const char* const* variant_ids, const char* const* allele_storage, const GlmInfo* glm_info_ptr, const uint32_t* local_sample_uidx_order, const uintptr_t* local_variant_include, uint32_t raw_variant_ct, uint32_t completed_pheno_ct, uint32_t batch_size, uintptr_t max_pheno_name_blen, uint32_t max_chr_blen, double ci_size, double ln_pfilter, double output_min_ln, uint32_t max_thread_ct, uintptr_t pgr_alloc_cacheline_ct, uintptr_t overflow_buf_size, uint32_t local_sample_ct, PgenFileInfo* pgfip, TextRstream* local_covar_trsp, char* outname, char* outname_end) {
  ...
  PglErr reterr = kPglRetSuccess;
  {
    ...
    if (local_covar_ct) {
      reterr = TextRewind(local_covar_trsp);
      if (unlikely(reterr)) {
        goto GlmLinearBatch_ret_RSTREAM_FAIL;
      }
      if (unlikely(bigstack_alloc_u32(local_sample_ct, &local_sample_idx_order))) {
      ...
      for (uint32_t uii = 0; uii != local_sample_ct; ++uii) {
        ...
        if ((cur_uidx != UINT32_MAX) && IsSet(sample_include, cur_uidx)) {
          cur_idx = RawToSubsettedPos(sample_include, g_sample_include_cumulative_popcounts, cur_uidx);
        }
        local_sample_idx_order[uii] = cur_idx;
      }
    }
    ...
    for (uint32_t subbatch_idx = 0; subbatch_idx != subbatch_ct; ++subbatch_idx) {
      ...
      for (uint32_t fidx = 0; fidx != subbatch_size; ++fidx, ++pheno_uidx) {
        ...
        *cswritep++ = '#';
        ...
        FillPhenoAndXtY(sample_include, pheno_cols[pheno_uidx].data.qt, g_covars_cmaj_d, sample_ct, domdev_present_p1, covar_ct, g_nm_precomp? (&(g_nm_precomp->xt_y_image[fidx * (1 + domdev_present_p1 + covar_ct)])) : nullptr, &(g_pheno_d[fidx * sample_ct]));
        if (sample_ct_x) {
          FillPhenoAndXtY(g_sample_include_x, pheno_cols[pheno_uidx].data.qt, g_covars_cmaj_x_d, sample_ct_x, domdev_present_p1, covar_ct_x, g_nm_precomp_x? (&(g_nm_precomp_x->xt_y_image[fidx * (1 + domdev_present_p1 + covar_ct_x)])) : nullptr, &(g_pheno_x_d[fidx * sample_ct_x]));
        }
        if (sample_ct_y) {
          FillPhenoAndXtY(g_sample_include_y, pheno_cols[pheno_uidx].data.qt, g_covars_cmaj_y_d, sample_ct_y, domdev_present_p1, covar_ct_y, g_nm_precomp_y? (&(g_nm_precomp_y->xt_y_image[fidx * (1 + domdev_present_p1 + covar_ct_y)])) : nullptr, &(g_pheno_y_d[fidx * sample_ct_y]));
        }
      }
      ...
      for (uint32_t variant_idx = 0; ; ) {
        ...
        if (!ts.is_last_block) {
          ...
          g_block_beta_se = block_beta_se_bufs[parity];
          ts.is_last_block = (variant_idx + cur_block_variant_ct == variant_ct);
          ts.thread_func_ptr = GlmLinearSubbatchThread;
          if (unlikely(SpawnThreads3z(variant_idx, &ts))) {
  ...
  GlmLinearBatch_ret_NOMEM:
    reterr = kPglRetNomem;
    break;
  GlmLinearBatch_ret_RSTREAM_FAIL:
    TextRstreamErrPrint("--glm local-covar= file", local_covar_trsp);
    break;
...
PglErr GlmMain(const uintptr_t* orig_sample_include, const SampleIdInfo* siip, const uintptr_t* sex_nm, const uintptr_t* sex_male, const PhenoCol* pheno_cols, const char* pheno_names, const PhenoCol* covar_cols, const char* covar_names, const uintptr_t* orig_variant_include, const ChrInfo* cip, const uint32_t* variant_bps, const char* const* variant_ids, const uintptr_t* allele_idx_offsets, const AlleleCode* maj_alleles, const char* const* allele_storage, const GlmInfo* glm_info_ptr, const AdjustInfo* adjust_info_ptr, const APerm* aperm_ptr, const char* local_covar_fname, const char* local_pvar_fname, const char* local_psam_fname, uint32_t raw_sample_ct, uint32_t orig_sample_ct, uint32_t pheno_ct, uintptr_t max_pheno_name_blen, uint32_t orig_covar_ct, uintptr_t max_covar_name_blen, uint32_t raw_variant_ct, uint32_t orig_variant_ct, uint32_t max_variant_id_slen, uint32_t max_allele_slen, uint32_t xchr_model, double ci_size, double vif_thresh, double ln_pfilter, double output_min_ln, uint32_t max_thread_ct, uintptr_t pgr_alloc_cacheline_ct, PgenFileInfo* pgfip, PgenReader* simple_pgrp, char* outname, char* outname_end) {
  ...
  unsigned char* bigstack_end_mark = g_bigstack_end;
  PglErr reterr = kPglRetSuccess;
  TextRstream local_covar_trs;
  TokenBatchStream tbs;
  PreinitTextRstream(&local_covar_trs);
  PreinitTBstream(&tbs);
  {
    ...
    if (local_covar_fname) {
      reterr = GlmLocalOpen(local_covar_fname, local_pvar_fname, local_psam_fname, siip->sample_ids, cip, variant_bps, variant_ids, glm_info_ptr, raw_sample_ct, siip->max_sample_id_blen, raw_variant_ct, &orig_sample_include, &sex_nm, &sex_male, &early_variant_include, &orig_sample_ct, &variant_ct, &local_covar_trs, &local_sample_uidx_order, &local_variant_include, &local_sample_ct, &local_variant_ctl, &local_covar_ct);
      if (unlikely(reterr)) {
        goto GlmMain_ret_1;
      }
    }
    ...
    *outname_end = '.';
    ...
    g_tests_flag = glm_info_ptr->tests_range_list.name_ct || (glm_flags & kfGlmTestsAll);
    const uint32_t joint_test = domdev_present || g_tests_flag;
    ...
    if (joint_test) {
      if (unlikely(
              ...
              bigstack_alloc_w(biallelic_raw_predictor_ctl, &g_joint_test_params) ||
              bigstack_alloc_w(biallelic_raw_predictor_ctl, &g_joint_test_params_x))) {
        goto GlmMain_ret_NOMEM;
      }
      ...
      if (!g_tests_flag) {
        ...
        uintptr_t* tests_buf;
        if (unlikely(
                bigstack_alloc_w(biallelic_raw_predictor_ctl, &g_joint_test_params_y) ||
    ...
    if (report_adjust || perms_total) {
      ...
      for (uint32_t pheno_uidx = 0; pheno_uidx != pheno_ct; ++pheno_uidx) {
        ...
        if (raw_parameter_subset) {
          biallelic_predictor_ct = CollapseParamOrTestSubset(covar_include, raw_parameter_subset, domdev_present, raw_covar_ct, covar_ct, add_interactions, g_parameter_subset);
        }
...
#endif
        if (g_tests_flag && (!g_constraint_ct)) {
        ...
        if (cur_sample_include_x) {
          ...
          if ((!x_samples_are_different) && (covar_ct == covar_ct_x) && wordsequal(covar_include, covar_include_x, raw_covar_ctl)) {
            ...
            if (!sample_ct_x) {
              ...
              if (raw_parameter_subset) {
                biallelic_predictor_ct_x = CollapseParamOrTestSubset(covar_include, raw_parameter_subset, domdev_present, raw_covar_ct, covar_ct_x, add_interactions, g_parameter_subset_x);
              }
              if (raw_joint_test_params) {
              ...
              if (sample_ct_x && g_tests_flag && (!g_constraint_ct_x)) {
        ...
        if (cur_sample_include_y) {
          ...
          if ((!y_samples_are_different) && (covar_ct == covar_ct_y) && wordsequal(covar_include, covar_include_y, raw_covar_ctl)) {
            ...
            if (!sample_ct_y) {
              ...
              if (raw_parameter_subset) {
                biallelic_predictor_ct_y = CollapseParamOrTestSubset(covar_include, raw_parameter_subset, domdev_present, raw_covar_ct, covar_ct_y, add_interactions, g_parameter_subset_y);
              }
              if (raw_joint_test_params) {
                assert(g_tests_flag);
              ...
              if (sample_ct_y && g_tests_flag && (!g_constraint_ct_y)) {
        ...
        if (unlikely(AllocAndInitReportedTestNames(g_parameter_subset, cur_covar_names, glm_flags, covar_ct + extra_cat_ct, g_tests_flag? g_constraint_ct : 0, &cur_test_names))) {
          goto GlmMain_ret_NOMEM;
        }
        if (sample_ct_x) {
          if (unlikely(AllocAndInitReportedTestNames(g_parameter_subset_x, cur_covar_names_x, glm_flags, covar_ct_x + extra_cat_ct_x, g_tests_flag? g_constraint_ct_x : 0, &cur_test_names_x))) {
            goto GlmMain_ret_NOMEM;
          }
        }
        if (sample_ct_y) {
          if (unlikely(AllocAndInitReportedTestNames(g_parameter_subset_y, cur_covar_names_y, glm_flags, covar_ct_y + extra_cat_ct_y, g_tests_flag? g_constraint_ct_y : 0, &cur_test_names_y))) {
            goto GlmMain_ret_NOMEM;
          }
        }
 ...
 GlmMain_ret_1:
  CleanupTBstream(&tbs);
  CleanupTextRstream2(local_covar_fname, &local_covar_trs, &reterr);
  BigstackDoubleReset(bigstack_mark, bigstack_end_mark);
  return reterr;
}
