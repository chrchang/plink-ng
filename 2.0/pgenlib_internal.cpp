...
void transpose_quaterblock_internal(const uintptr_t* read_iter, uint32_t read_ul_stride, uint32_t write_ul_stride, uint32_t read_batch_size, uint32_t write_batch_size, uintptr_t* write_iter, unsigned char* __restrict buf0, unsigned char* __restrict buf1) {
  ...
  const uint32_t write_dword_ct = BITCT_TO_WORDCT(read_batch_size);
  const uint32_t penult_inner_loop_iter_ct = 2 * write_dword_ct;
  const uint32_t cur_write_skip = kWordsPerCacheline - penult_inner_loop_iter_ct;
...
pglerr_t read_refalt1_genovec_hphase_subset_unsafe(const uintptr_t* __restrict sample_include, const uint32_t* __restrict sample_include_cumulative_popcounts, uint32_t sample_ct, uint32_t vidx, pgen_reader_t* pgrp, const unsigned char** fread_pp, const unsigned char** fread_endp, uintptr_t* __restrict genovec, uintptr_t* __restrict phasepresent, uintptr_t* __restrict phaseinfo, uint32_t* phasepresent_ct_ptr) {
      ...
      *phasepresent_ct_ptr = het_ct;
      while (1) {
        ...
        if (phaseinfo_widx >= het_ctdl) {
          ...
          phaseinfo_word = partial_word_load(&(aux2_raw_phaseinfo_iter[phaseinfo_widx]), DIV_UP(loop_len, CHAR_BIT));
        } else {
#ifdef __arm__
  ...
  while (1) {
    ...
    if (phaseinfo_widx >= raw_phasepresent_ctl_m1) {
      ...
      // avoid possible segfault
      phaseinfo_word = partial_word_load(&(aux2_second_half[phaseinfo_widx]), DIV_UP(loop_len, CHAR_BIT));
    } else {
  ...
  while (1) {
    ...
    if (xor_word) {
      ...
      while (1) {
        ...
        if (!(difflist_idx % kBitsPerWordD2)) {
          if (!(difflist_idx % kPglDifflistGroupSize)) {
            partial_uint_store_adv(new_sample_idx, sample_id_byte_ct, &group_first_sample_ids_iter);
            if (difflist_idx) {
              *extra_byte_cts_iter++ = ((uintptr_t)(fwrite_bufp - last_group_vint_start)) - (kPglDifflistGroupSize - 1);
              *raregeno_iter++ = raregeno_word;
              raregeno_word = 0;
  ...
  *vrtype_ptr += 16;
  ...
  if (het_ct == phasepresent_ct) {
    ...
    if (phasepresent_write_idx_lowbits) {
      ...
      partial_word_store_adv(phasepresent_write_word, cur_byte_ct, &fwrite_bufp_final);
    }
  ...
  if (phaseinfo_write_idx_lowbits) {
    ...
    partial_word_store_adv(phaseinfo_write_word, cur_byte_ct, &fwrite_bufp_final);
  }
#ifdef __LP64__
