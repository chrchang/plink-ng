#!/usr/bin/env python3
"""Write a --qfam fixture exercising all four kinds of group.

  * nuclear families,
  * sibships, i.e. samples sharing FID/PAT/MAT whose parents are not in the
    dataset,
  * unrelated founders (singletons).
...
def main():
    ...
    with open('qfam.psam', 'w') as f:
        f.write('#FID\tIID\tPAT\tMAT\tSEX\tQT\n')
        for row in samples:
            f.write('%s\t%s\t%s\t%s\t%d\t%.4f\n' % (row[0], row[1], row[2], row[3], row[4], rand.gauss(0.0, 1.0)))
    ...
    write_vcf('qfam.vcf', False)
    write_vcf('qfam_flipped.vcf', True)
