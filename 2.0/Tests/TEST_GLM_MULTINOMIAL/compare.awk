# Compares a --glm multinomial report against an oracle report of the same
# variants, row by row, keyed on ID, TEST and CATEGORY.
#
...
# printed digit for the two fits having started from different points.
#
...
}
{
    ...
    for (i = 1; i <= NF; ++i) {
        if (!same(o[i], $i)) { fail("field " i " differs on " k ": " o[i] " vs " $i) }
    }
    seen[k] = 1
    ++matched
}
