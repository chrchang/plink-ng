...
# 8. Merge conflicts.  Only the cases whose answer is well-defined today are
...
# 8a. Hardcall, dosage and phase conflicts, same samples and variants in both
...
# 8b. Same, with multiallelic hardcalls.  ALTx/ALTy genotypes have their own
...
# 8c. QUAL/FILTER/INFO conflicts.  'first' keeps the first value even when it
