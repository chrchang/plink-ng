...
cdef extern from "../pgenlib_python_support.h" namespace "plink2":
    ...
    PglErr SpgwAppendBiallelicGenovecDosage16(const uintptr_t* genovec, const uintptr_t* dosage_present, const uint16_t* dosage_vals, uint32_t dosage_ct, STPgenWriter* spgwp)
...
cdef class PgenReader:
    ...
    def __cinit__(self, bytes filename, object raw_sample_ct = None,
        ...
        cdef uintptr_t dosage_vals_byte_ct = DivUp(file_sample_ct, (2 * kInt32PerVec)) * kBytesPerVec
    ...
    cpdef read(self, uint32_t variant_idx, np.ndarray geno_int_out, uint32_t allele_idx = 1):
        ...
        # instead
        cdef PglErr reterr = PgrReadAlleleCountvecSubsetUnsafe(self._subset_include_vec, self._subset_cumulative_popcounts, self._subset_size, variant_idx, allele_idx, self._state_ptr, self._genovec)
        if reterr != kPglRetSuccess:
    ...
    cpdef read_dosages(self, uint32_t variant_idx, np.ndarray floatarr_out, uint32_t allele_idx = 1):
        ...
        if allele_idx == 0:
            ...
            BiallelicDosage16Invert(dosage_ct, self._dosage_vals)
        ...
        if floatarr_out.dtype == np.float32:
            ...
            Dosage16ToFloatsMinus9(self._genovec, self._dosage_present, self._dosage_vals, self._subset_size, dosage_ct, data32_ptr)
        elif floatarr_out.dtype == np.float64:
            data64_ptr = <double*>floatarr_out.data
            Dosage16ToDoublesMinus9(self._genovec, self._dosage_present, self._dosage_vals, self._subset_size, dosage_ct, data64_ptr)
        else:
    ...
    cdef read_range_internal8(self, uint32_t variant_idx_start, uint32_t variant_idx_end, np.ndarray[np.int8_t,mode="c",ndim=2] geno_int8_out, uint32_t allele_idx = 1, bint sample_maj = 0):
        ...
        if sample_maj == 0:
            ...
            for variant_idx in range(variant_idx_start, variant_idx_end):
                reterr = PgrReadAlleleCountvecSubsetUnsafe(subset_include_vec, subset_cumulative_popcounts, subset_size, variant_idx, allele_idx, pgrp, genovec)
                if reterr != kPglRetSuccess:
        ...
        cdef uint32_t variant_batch_ct = DivUp(variant_idx_ct, kPglQuaterTransposeBatch)
        ...
        for variant_batch_idx in range(variant_batch_ct):
            ...
            for uii in range(variant_batch_size):
                reterr = PgrReadAlleleCountvecSubsetUnsafe(subset_include_vec, subset_cumulative_popcounts, subset_size, uii + variant_idx_offset, allele_idx, pgrp, vmaj_iter)
                if reterr != kPglRetSuccess:
                    raise RuntimeError("read_range() error " + str(reterr))
            ...
            for sample_batch_idx in range(sample_batch_ct):
                ...
                TransposeQuaterblock(vmaj_iter, sample_ctaw2, kPglQuaterTransposeWords, variant_batch_size, sample_batch_size, smaj_iter, transpose_batch_buf)
    ...
    cdef read_list_internal8(self, np.ndarray[np.uint32_t] variant_idxs, np.ndarray[np.int8_t,mode="c",ndim=2] geno_int8_out, uint32_t allele_idx = 1, bint sample_maj = 0):
        ...
        for variant_batch_idx in range(variant_batch_ct):
            ...
            for uii in range(variant_batch_size):
                ...
                if variant_idx >= raw_variant_ct:
                    raise RuntimeError("read_list() variant index too large (" + str(variant_idx) + "; only " + str(raw_variant_ct) + " in file)")
                reterr = PgrReadAlleleCountvecSubsetUnsafe(subset_include_vec, subset_cumulative_popcounts, subset_size, variant_idx, allele_idx, pgrp, vmaj_iter)
                if reterr != kPglRetSuccess:
...
cdef class PgenWriter:
    ...
    def __cinit__(self, bytes filename, uint32_t sample_ct,
        ...
        cdef uint32_t genovec_cacheline_ct = DivUp(sample_ct, kQuatersPerCacheline)
        cdef uint32_t dosage_vals_cacheline_ct = DivUp(sample_ct, (2 * kInt32PerCacheline))
    ...
    cpdef append_biallelic(self, np.ndarray[np.int8_t,mode="c"] geno_int8):
        ...
        BytesToGenoarrUnsafe(genobytes, self._state_ptr[0].pwc.sample_ct, self._genovec)
    ...
    cpdef append_alleles(self, np.ndarray[np.int32_t,mode="c"] allele_int32, bint all_phased = False):
        ...
        cdef PglErr reterr
        if not all_phased:
            AlleleCodesToGenoarrUnsafe(allele_codes, NULL, self._state_ptr[0].pwc.sample_ct, genovec, NULL, NULL)
            reterr = SpgwAppendBiallelicGenovec(genovec, self._state_ptr)
        else:
            AlleleCodesToGenoarrUnsafe(allele_codes, NULL, self._state_ptr[0].pwc.sample_ct, genovec, self._phasepresent, self._phaseinfo)
    ...
    cpdef append_partially_phased(self, np.ndarray[np.int32_t,mode="c"] allele_int32, np.ndarray[np.uint8_t,cast=True] phasepresent):
        ...
        cdef uintptr_t* phaseinfo = self._phaseinfo
        AlleleCodesToGenoarrUnsafe(allele_codes, phasepresent_bytes, self._state_ptr[0].pwc.sample_ct, genovec, phasepresent_buf, phaseinfo)
    ...
    cdef append_dosages_internal32(self, np.ndarray[np.float32_t,mode="c"] floatarr):
        ...
        cdef uint32_t dosage_ct
        FloatsToDosage16(<float*>(&(floatarr[0])), self._state_ptr[0].pwc.sample_ct, 6554, genovec, dosage_present, dosage_vals, &dosage_ct)
        cdef PglErr reterr = SpgwAppendBiallelicGenovecDosage16(genovec, dosage_present, dosage_vals, dosage_ct, self._state_ptr)
        if reterr != kPglRetSuccess:
    ...
    cdef append_dosages_internal64(self, np.ndarray[np.float64_t,mode="c"] doublearr):
        ...
        cdef uint16_t* dosage_vals = self._dosage_vals
        cdef uint32_t dosage_ct
        DoublesToDosage16(<double*>(&(doublearr[0])), self._state_ptr[0].pwc.sample_ct, 6554, genovec, dosage_present, dosage_vals, &dosage_ct)
        cdef PglErr reterr = SpgwAppendBiallelicGenovecDosage16(genovec, dosage_present, dosage_vals, dosage_ct, self._state_ptr)
        if reterr != kPglRetSuccess:
            raise RuntimeError("append_dosages() error " + str(reterr))
        return
    ...
    cdef append_dosages_batch_internal32(self, np.ndarray[np.float32_t,mode="c",ndim=2] floatarr_batch):
        ...
        cdef PglErr reterr
        for uii in range(batch_size):
            FloatsToDosage16(<float*>(&(floatarr_batch[uii, 0])), self._state_ptr[0].pwc.sample_ct, 6554, genovec, dosage_present, dosage_vals, &dosage_ct)
            reterr = SpgwAppendBiallelicGenovecDosage16(genovec, dosage_present, dosage_vals, dosage_ct, self._state_ptr)
            if reterr != kPglRetSuccess:
    ...
    cdef append_dosages_batch_internal64(self, np.ndarray[np.float64_t,mode="c",ndim=2] doublearr_batch):
        ...
        cdef PglErr reterr
        for uii in range(batch_size):
            DoublesToDosage16(<double*>(&(doublearr_batch[uii, 0])), self._state_ptr[0].pwc.sample_ct, 6554, genovec, dosage_present, dosage_vals, &dosage_ct)
            reterr = SpgwAppendBiallelicGenovecDosage16(genovec, dosage_present, dosage_vals, dosage_ct, self._state_ptr)
            if reterr != kPglRetSuccess:
                raise RuntimeError("append_dosages_batch() error " + str(reterr))
        return
