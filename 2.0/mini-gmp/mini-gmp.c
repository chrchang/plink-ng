...
#define gmp_clz(count, x) do {						\
    mp_limb_t __clz_x = (x);						\
    unsigned __clz_c = 0;						\
    unsigned int LOCAL_SHIFT_BITS = 8;			       		\
...
static void *
gmp_default_realloc (void *old, __attribute__((unused)) size_t unused_old_size, size_t new_size)
{
  void * p;
...
static void
gmp_default_free (void *p, __attribute__((unused)) size_t unused_size)
{
  free (p);
}
  ...
  if (GMP_LIMB_BITS < GMP_ULONG_BITS)
    {
      ...
      n = GMP_MIN ((unsigned long)n, 1 + (unsigned long) (GMP_ULONG_BITS - 1) / GMP_LIMB_BITS);
      while (--n >= 0)
...
gmp_popcount_limb (mp_limb_t x)
{
  ...
  unsigned int LOCAL_SHIFT_BITS = 16;
  for (c = 0; x > 0;)
    {
