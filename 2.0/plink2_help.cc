...
"    both, and 'NA', '-9', '?', '?""?' and '.' all mark a missing call; any other\n"
"    dosage outside [0, 2] is an error.\n"
...
"    * --psam/--fam supplies real sample IDs (and sex, and phenotypes) instead.\n"
"      Since the .mgf has no IDs of its own to match against, the file just has\n"
"      to have one row per sample, in the .mgf's column order.\n"
...
"    chrom/pos/ref/alt to the .lmendel[.zst] output file.\n\n"
    ...
    HelpPrint("homozyg\0homozyg-snp\0homozyg-kb\0homozyg-density\0homozyg-gap\0homozyg-het\0homozyg-maf\0homozyg-min-af\0homozyg-window-snp\0homozyg-window-het\0homozyg-window-missing\0homozyg-window-threshold\0", &help_ctrl, 1,
"  --homozyg ['zs'] ['subtract-1-from-lengths'] ['cols='<column set descriptor>]\n"
"    Run-of-homozygosity reports, written to <output prefix>.hom, .hom.indiv and\n"
"    .hom.summary.\n"
"    * Fully-haploid chromosomes and chrM are skipped, and males are skipped\n"
"      on chrX.\n"
"    * This analysis is most informative on a LD-pruned variant set: linkage\n"
"      disequilibrium extends runs that are not evidence of autozygosity.  Use\n"
"      e.g. --indep-pairwise first.\n"
"    * Relatedly, rare variants are homozygous in nearly everyone and pad out\n"
"      runs the same way.  The default --homozyg-... parameter values make sense\n"
"      for human data filtered to minimum MAF ~0.05.  So, as a sanity check,\n"
"      --homozyg now errors out when a MAF<0.05 variant is still present and\n"
"      --homozyg-maf was not specified.  (But if you're intentionally including\n"
"      rarer variants in your analysis, it's fine to specify \"--homozyg-maf 0\".)\n"
...
"      for females.\n"
"    * For consistency with other commands, you can no longer generate these\n"
"      reports by specifying --homozyg-... settings without --homozyg.\n"
"    Supported column sets are:\n"
...
"    With either output type, the computation can be subdivided with --parallel.\n\n"
    ...
    HelpPrint("epistasis\0", &help_ctrl, 1,
"  --epistasis ['zs'] ['ref-based'] ['log10'] ['cols='<col set descriptor>]\n"
"    Scan every pair of autosomal variants for an interaction with quantitative\n"
"    phenotype(s), by least-squares regression of the phenotype(s) on the two\n"
"    genotypes, their product, and any --covar covariates.  Potentially\n"
"    significant interactions are reported to <output prefix>.<pheno>.epi.qt ;\n"
"    .epi.qt.summary files provide per-variant summaries.\n"
"    * The current implementation is limited to 1 phenotype; this will be\n"
"      addressed soon.\n"
"    * Logistic/Firth regression for case/control phenotypes will be implemented\n"
"      later.\n"
...
"    is written to <output prefix>.<pheno>.epi.cc, with a per-variant summary\n"
"    alongside it in .epi.cc.summary.\n"
"    * The current implementation errors out if more than one case/control\n"
"      phenotype is defined.  This limitation may be removed later.\n"
"    * This is a genotypic test, which trades off a bit of power in the\n"
"      linear-interaction case for more power to detect other types of\n"
"      interactions.  Contact us if you prefer the basic 'allelic' form of this\n"
"      test.\n"
"    * An empty genotype row or column costs degree(s) of freedom instead of\n"
"      dropping the pair, so the report has a DF column.\n"
...
"      covariates.  STAT/DF/P then come from that fit.  BEST_CHISQ and N_SIG\n""      stay unadjusted, since they cover pairs that were never refit.  A sample\n"
"      missing a covariate is left out of the whole scan, and a pair whose fit\n"
"      does not converge is left out of the report.\n"
...
"    Supported column sets are:\n"
"      Shared between .epi.cc and .epi.cc.summary:\n"
"        chrom: Chromosome IDs.\n"
"        pos: Base-pair coordinates.\n"
"        (Variant IDs are always present.)\n"
"        maybea1: A1 alleles, if at least 1 multiallelic variant is present.\n"
...
"        (BEST_CHISQ and BEST_ID are always present, and positioned here, with\n"
"        BEST_CHROM in between when chrom is present.)\n"
"    The default is chrom,maybea1,stat,df,p,nsig,ntot,prop.\n\n"
               );
...
"    * Without a phenotype name, the report covers all samples in one ALL\n"
"      group.  PLINK 1.x always splits on the case/control phenotype, but\n"
"      several phenotypes may be loaded here, so the one to split on is named\n"
"      explicitly.\n"
"    * With a name, one table per category follows the ALL table: CASE and\n"
"      CONTROL for a binary phenotype, one per observed category otherwise.\n"
"      Samples with no value for that phenotype are counted in ALL only.\n"
"    * A quantitative phenotype is an error, since it has no categories.\n"
...
"      HETERO row reports flanking heterozygosity instead of a haplotype.\n\n"
    ...
    HelpPrint("ld-score\0", &help_ctrl, 1,
...
"    * When nonfounders are present, you must explicitly specify either\n"
"      --ld-score-founders or --nonfounders.\n"
...
"      formats default to a square matrix, and can be combined with 'square0' or\n"
"      'triangle'.\n"
...
#endif
    HelpPrint("neighbour\0neighbor\0pca\0", &help_ctrl, 1,
"  --neighbour [neighbor count] ['zs'] ['cols='<col set descriptor>]\n"
"    Flag samples that sit in a sparse part of principal-component space, as an\n"
"    outlier check on a set of PCs.  Requires --pca, or --read-eigvec to load\n"
"    coordinates computed earlier.\n"
"    * For each sample, this reports the mean distance to its K nearest\n"
"      neighbors in PC space (DIST_SELF), the mean of those neighbors' own\n"
"      DIST_SELF values (DIST_NN), and sqrt(DIST_SELF / DIST_NN) (STAT).  A\n"
"      sample in empty space has a high STAT; one merely sitting next to a\n"
"      dense cluster does not, which is what the square root is for.  This is\n"
"      the statistic from section 3.4 of Prive F, Luu K, Blum MGB, McGrath JJ,\n"
"      Vilhjalmsson BJ (2020) Efficient toolkit implementing best practices for\n"
"      principal component analysis of population genetic data.\n"
"    * K defaults to 5; you can adjust this by passing a numeric argument.  All\n"
"      of the available PCs are used.\n"
"    * No outlier threshold is applied, since a sensible one depends on the\n"
"      dataset; plot a histogram of STAT, pick a cutoff, and feed the sample\n"
"      IDs above it to --remove.\n"
"    * The nearest-neighbor search runs over a k-d tree, so its cost grows\n"
"      roughly as sample_ct * log(sample_ct).\n"
"    * Supported column sets are:\n"
"        maybefid: FID, if that column was in the input.\n"
"        fid: Force FID column.\n"
"        (IID is always present, and positioned here.)\n"
"        maybesid: SID, if that column was in the input.\n"
"        sid: Force SID column.\n"
"        distself: Mean distance to the K nearest neighbors.\n"
"        distnn: Mean DIST_SELF over those K neighbors.\n"
...
"    * Categorical phenotypes are skipped unless --mnl-ref names their reference\n"
"      category, in which case they are analyzed with multinomial logistic\n"
"      regression (see --mnl-ref).  The three modes above apply to them too.\n"
...
"          (Multinomial reports: see --mnl-ref.)\n"
"      p: Asymptotic p-value (or -log10(p)) for T/Z-statistic.\n"
...
"    construction only, leaving every other command's variant set alone.  The\n"
...
"                               value (default 0.95).\n"
    ...
    HelpPrint("ld-score-annot\0ld-score-founders\0ld-score-window\0ld-score-window-kb\0ld-score-window-cm\0ld-score\0", &help_ctrl, 0,
"  --ld-score-annot <filename> :\n"
"    Partitions LD Score: given a file with a variant ID column and one column\n"
"    per annotation, each window variant contributes its r^2 weighted by its\n"
"    value there, and one L2 column is written per annotation, named after it.\n"
"    This is what stratified LD Score regression takes as input.  The file must\n"
"    cover every variant being scored; values are usually 0 or 1, but any finite\n"
"    weight works.\n"
"  --ld-score-founders                  : Specify that only founders should be\n"
"                                         considered in LD Score computation.\n"
"  --ld-score-window <max varct radius> : Limit --ld-score variant-ct distance.\n"
...
"                                   samples in the given file.\n"
    ...
    HelpPrint("homozyg-snp\0homozyg-kb\0homozyg-density\0homozyg-gap\0homozyg-het\0homozyg-maf\0homozyg-min-af\0homozyg-window-snp\0homozyg-window-het\0homozyg-window-missing\0homozyg-window-threshold\0homozyg\0", &help_ctrl, 0,
"  --homozyg-snp <min var count>  : Set minimum ROH variant count (default 100).\n"
"  --homozyg-kb <min length>      : Set minimum ROH kb span (default 1000).\n"
"  --homozyg-density <max kb/var> : Set maximum ROH inverse-density (def. 50).\n"
"  --homozyg-gap <max int gap kb> : Set maximum ROH internal gap (def. 1000).\n"
"  --homozyg-het <max hets>       : Set maximum ROH het count (def. unlimited).\n"
"  --homozyg-maf <min nonmaj AF>  : MAF minimum for --homozyg.  Must be\n"
"    (alias: --homozyg-min-af)      explicitly specified when <0.05.\n"
"  --homozyg-window-snp <ct>      : Set --homozyg scan window size (def. 50).\n"
"  --homozyg-window-het <ct>      : Set --homozyg scan-hit max hets (def. 1).\n"
"  --homozyg-window-missing <ct>  : Set --homozyg scan-hit max missing (def. 5).\n"
"  --homozyg-window-threshold <r> : Set minimum ROH scan-hit rate (def. 0.05).\n"
              );
...
"                       * You can use '--tests all' to include all terms.\n"
    ...
    HelpPrint("mnl-ref\0glm\0", &help_ctrl, 0,
"  --mnl-ref <pheno name>=<category> ... :\n"
"    Analyze the named categorical phenotype(s) with multinomial logistic\n"
"    regression in --glm, using the given category as the reference.  For each\n"
"    variant, the model is fitted with and without the genotype term, and a\n"
"    likelihood-ratio test with (category count - 1) degrees of freedom is\n"
"    reported (TEST=LRT_<df>DF), along with per-category odds ratios, standard\n"
"    errors and Wald tests from the full model: one row per non-reference\n"
"    category, identified by the CATEGORY column.  Results are written to\n"
"    <output prefix>.<pheno name>.glm.multinomial.hybrid (default\n"
"    'firth-fallback' mode), .glm.multinomial.firth ('firth') or\n"
"    .glm.multinomial ('no-firth').\n"
"    * Each argument is split at its first '=', so phenotypes whose names\n"
"      contain '=' can't be named here.\n"
"    * The 'tz' column is named Z_OR_CHISQ_STAT: it holds the Wald Z-score on\n"
"      the per-category rows, and the likelihood-ratio chi-square on the LRT\n"
"      row.\n"
"    * When some category's samples all have zero A1 dosage, the\n"
"      maximum-likelihood estimate doesn't exist.  This is common for rare\n"
"      variants and small categories: at MAF 1%, a 38-sample category has no A1\n"
"      allele about 47% of the time.  Such variants, and those whose fit fails\n"
"      to converge or gives invalid standard errors, are analyzed with Firth's\n"
"      penalized likelihood (multinomial form of Bull SB, Mak C, Greenwood CMT\n"
"      (2002) A modified score function estimator for multinomial logistic\n"
"      regression in small samples) in the default 'firth-fallback' mode, and\n"
"      the FIRTH? column says which variants were.  'firth' uses it for every\n"
"      variant; with 'no-firth', those variants are NA (error code SEPARATION\n"
"      or MNL_CONVERGE_FAIL).\n"
"      * The omnibus test of a penalized fit is the penalized likelihood-ratio\n"
"        test: the null model maximizes the same penalized likelihood (whose\n"
"        penalty involves the genotype column) with the genotype coefficients\n"
"        fixed at zero, so it is refitted for every variant.\n"
"      * Its standard errors follow logistf's convention, as for 'firth' on\n"
"        case/control phenotypes: the inverse of the information matrix with\n"
"        each sample's contribution weighted by 1 + its leverage.  With more\n"
"        than two categories this is the natural extension; these standard\n"
"        errors are never larger than the plain inverse-information ones (with\n"
"        few samples per coefficient they can be noticeably smaller), and the\n"
"        likelihood-ratio row does not depend on this choice.\n"
"      * A penalized fit that hasn't converged after 100 iterations is still\n"
"        reported, with error code UNFINISHED.\n"
"      * If a covariate separates some category (so that even the\n"
"        covariate-only model has no maximum-likelihood estimate), 'no-firth'\n"
"        skips or refuses the phenotype, while the other modes fit the\n"
"        covariate-only model with Firth regression and use Firth regression\n"
"        for every variant.  The penalized likelihood can then have more than\n"
"        one local maximum, so the separated category's results may depend on\n"
"        where the fit starts.\n"
"      * 'firth-residualize' and 'single-prec-cc' do not apply to multinomial\n"
"        regression.\n"
"    * EMPTY_CATEGORY is reported (in every mode, without fitting) when a\n"
"      variant's missing calls leave some category with no samples: that\n"
"      category's coefficients would not be determined by the data, and the\n"
"      omnibus test would not have (category count - 1) degrees of freedom.\n"
"    * SAMPLE_CT<=PREDICTOR_CT is reported when a variant has no more\n"
"      non-missing samples than the model has coefficients ((category count -\n"
"      1) times the number of predictors).\n"
"    * --adjust uses the likelihood-ratio p-values.  Genomic control ('gc') is\n"
"      not applied to them, as for --glm's other multi-df joint tests.\n"
"    * The case/control-only columns (a1countcc, totallelecc, gcountcc,\n"
"      a1freqcc) are not reported.\n"
"    * Not supported yet: multiallelic variants (reported as NA, with error code\n"
"      MULTIALLELIC_UNSUPPORTED); --parameters, --tests, and --glm's\n"
"      'genotypic', 'hethom', 'dominant', 'recessive', 'hetonly',\n"
"      'interaction', 'local-covar=' and permutation modifiers.\n"
               );
    HelpPrint("glm\0linear\0logistic\0epistasis\0vif\0max-corr\0", &help_ctrl, 0,
"  --vif <max VIF>    : Set VIF threshold for --glm/--epistasis\n"
"                       multicollinearity check (default 50).  (This is no\n"
"                       longer skipped for case/control phenotypes.)\n"
"  --max-corr <val>   : Skip --glm/--epistasis regression when the absolute\n"
"                       value of the correlation between two predictors exceeds\n"
...
"                               clumps.\n"
    ...
    HelpPrint("epi1\0epi2\0epistasis\0epistasis-boost\0fast-epistasis\0", &help_ctrl, 0,
"  --epi1 <p-value>   : Set --epistasis[-boost] reporting threshold (default\n"
"                       1e-4 for --epistasis, 5e-6 for --epistasis-boost).\n"
"  --epi2 <p-value>   : Set threshold for contributing to SIG_E count (default\n"
"                       0.01).\n"
              );
