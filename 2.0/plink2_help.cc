...
"  --homozyg-min-af <min allele frequency>  (alias: --homozyg-maf)\n"
...
"    * These results are only meaningful on an LD-pruned variant set: linkage\n"
"      disequilibrium extends runs that are not evidence of autozygosity.  Use\n"
"      --indep-pairwise first.\n"
"    * --homozyg-min-af excludes variants with minor allele frequency below\n"
"      it, since rare variants are homozygous in nearly everyone and pad out\n"
"      runs the same way.  It has no default, and is required whenever the\n"
"      data still contains a variant below 0.05; a variant set that is\n"
"      already all-common needs no floor and the flag can be omitted.  0.05\n"
"      is a reasonable value alongside the other defaults; 0 reproduces PLINK\n"
"      1.x, which applied no frequency floor.  There is no default because\n"
"      the right floor depends on the other parameters: if you have tuned\n"
"      them for a low-MAF variant set, silently filtering to 0.05 would\n"
"      throw away exactly what you were looking at.\n"
...
"    set:\n"
"      BURDEN_P: weighted burden score test.\n"
"      SKAT_P: sequence kernel association test (Wu MC, Lee S, Cai T, Li Y,\n"
"              Boehnke M, Lin X (2011) Rare-variant association testing for\n"
"              sequencing data with the sequence kernel association test).\n"
"      SKATO_P: SKAT-O over a grid of eight rho values.  The exact\n"
"               minimum-p correction needs a numerical integration; the grid\n"
"               p-values are combined with a Cauchy sum instead, which is the\n"
"               variant REGENIE calls SKATO-ACAT.  Results differ slightly\n"
"               from textbook SKAT-O for that reason.\n"
"      ACATV_P: individual score tests above --vc-mac-thresh (default 10),\n"
"               plus one burden test over everything below it, combined with\n"
"               a Cauchy sum.\n"
"      ACATO_P: Cauchy combination of the four above.\n"
"    * All of these share one covariates-only null fit and one genotype pass\n"
"      per set, so running the battery costs little more than running one.\n"
"      Quantitative phenotypes get a linear null, case/control a logistic one.\n"
"    * This null is what SKAT as published assumes, and it is valid for\n"
"      unrelated samples.  It does not adjust for relatedness or population\n"
"      structure; for that, use a package that fits a mixed-model null.\n"
"    * --vc-max-af sets the frequency ceiling for inclusion (default 0.01),\n"
"      --vc-params the Beta weight shape (default 1 and 25).  Covariates must\n"
...
"  --make-gene-masks\n"
"  --build-mask {max | sum}\n"
"  --mask-max-af <max alt allele frequency>\n"
"    Collapse each set of variants into a single mask, and write the masks as\n"
"    an ordinary dosage fileset (<output prefix>.pgen + .pvar + .psam), one\n"
"    variant per set.  --set-list is required, in the same format --acat-file\n"
"    uses.\n"
"    * A mask is a pseudo-variant whose dosage summarizes a sample's burden of\n"
"      rare alternate alleles across the set, which is how modern gene-based\n"
"      tests are built.  Writing them out as a fileset rather than testing\n"
"      them here means --glm applies to them unchanged, with covariates,\n"
"      Firth, and everything else it already does, and --acat-file can then\n"
"      combine the per-mask results.\n"
"    * 'max' (default) gives each sample the largest alternate dosage it\n"
"      carries at any variant in the set; 'sum' adds them instead, capped at\n"
"      2.\n"
"    * --mask-max-af sets the frequency ceiling for inclusion (default 0.01).\n"
"      A set with no variant below it produces no mask, and is reported.\n"
"    * Missing calls count as reference, which is the usual burden convention:\n"
"      an uncalled site is not evidence of carrying anything, and mean-\n"
"      imputing at these frequencies would smear a fractional carrier across\n"
"      every sample.\n"
"    * Multiallelic variants are left out, since there is no single alternate\n"
"      dosage to add to a burden score.\n"
"    * The INFO column records how many variants went into each mask.\n\n"
               );
    HelpPrint("acat-file\0set-list\0acat-params\0", &help_ctrl, 1,
"  --acat-file <filename> ['zs'] ['input-log10']\n"
"             ['test='<test name, case-sensitive>]\n"
"  --set-list <filename>\n"
"  --acat-params <a1> <a2>\n"
"    Combine per-variant association p-values into one p-value per set, with\n"
"    the Cauchy combination test of Liu Y, Chen S, Li Z, Morrison AC,\n"
"    Boerwinkle E, Lin X (2019) ACAT: A Fast and Powerful p Value Combination\n"
"    Method for Rare-Variant Analysis in Sequencing Studies.  Results are\n"
"    written to <output prefix>.acat.\n"
"    * --acat-file takes association results in the format --glm writes; ID\n"
"      and P columns are required.  Use 'test=' to select one test when the\n"
"      file has a TEST column, and 'input-log10' when it holds -log10(p).\n"
"    * --set-list is required, and names the sets.  One line per set:\n"
"      set name, chromosome, position, then a comma-separated list of variant\n"
"      IDs.  Members with no p-value in the results file are skipped, and the\n"
"      count that remained is reported in the NVAR_TESTED column.\n"
"    * If the results file has an allele frequency column, variants are\n"
"      weighted by Beta(MAF; a1, a2)^2 * MAF * (1 - MAF), which is what makes\n"
"      rarer variants count for more.  --acat-params sets a1 and a2 (default\n"
"      1 and 25).  Without a frequency column, all variants weigh the same.\n"
"    * The combination needs no LD information and no correlation structure\n"
"      between the tests, which is what makes it cheap.  It is asymptotically\n"
"      valid under arbitrary dependence; that guarantee is asymptotic in the\n"
"      significance level, so treat far-tail p-values as approximate.\n"
"    * This is the combination of the p-values you supply, and not the full\n"
"      ACAT-V, which additionally collapses variants below a minor allele\n"
"      count threshold into a single burden test.  That step needs genotypes.\n\n"
               );
