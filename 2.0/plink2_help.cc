...
"    chrom/pos/ref/alt to the .lmendel[.zst] output file.\n\n"
    ...
    HelpPrint("model\0cell\0fisher\0trend\0model-dom\0model-gen\0model-rec\0model-trend\0assoc\0", &help_ctrl, 1,
"  --model [{fisher | fisher-midp}] [\'trend-only\'] [\'zs\']\n"
"          [\'cols=\'<column set descriptor>]\n"
"    Case/control association analysis with the contingency-table tests PLINK\n"
"    1.x provides.  One 2x3 table per variant (case/control by genotype)\n"
"    yields five tests, reported one row each:\n"
"      GENO     2df chi-square on the full table\n"
"      TREND    Cochran-Armitage trend test\n"
"      ALLELIC  1df chi-square on the collapsed allele counts\n"
"      DOM      A1A1 + A1A2 vs. A2A2\n"
"      REC      A1A1 vs. A1A2 + A2A2\n"
"    --glm\'s \'genotypic\'/\'dominant\'/\'recessive\' modes give regression\n"
"    analogues of three of these, but not the contingency tests themselves.\n"
"    * With \'fisher\'/\'fisher-midp\', exact tests are used instead:\n"
"      Fisher-Freeman-Halton for GENO and Fisher\'s exact test for the 2x2\n"
"      tables.  TREND has no exact analogue, so its asymptotic p-value is\n"
"      reported either way.  \'fisher-midp\' applies Lancaster\'s mid-p\n"
"      adjustment.\n"
"    * \'trend-only\' reports just the TREND row.\n"
"    * A1 is the ALT allele (or the --a1-allele setting), so DOM and REC are\n"
"      relative to that; PLINK 1.x keyed them on the minor allele instead.\n"
"      Multiallelic variants are collapsed to REF vs. non-REF.\n"
"    * Non-autosomal variants are excluded.\n"
"    Supported column sets are:\n"
"      chrom: Chromosome ID.\n"
"      pos: Base-pair coordinate.\n"
"      (ID is always present, and positioned here.)\n"
...
"                    and the REF column is present.\n"
"      provref: Force provref column even if all-\'N\'.\n"
"      a1: Allele the genotype counts are keyed on.\n"
"      test: Which of the five tests the row reports.\n"
"      casects: Case counts, in the units the test uses.\n"
"      ctrlcts: Control counts.\n"
"      chisq: Test statistic.  Omitted by the exact tests.\n"
"      df: Degrees of freedom.  Omitted by the exact tests.\n"
"      p: p-value.\n"
"    The default is chrom,pos,a1,test,casects,ctrlcts,chisq,df,p.\n\n"
               );
    HelpPrint("cell\0model\0", &help_ctrl, 1,
"  --cell <thresh>\n"
"    Suppress --model\'s GENO, DOM and REC tests when a genotype cell count is\n"
"    below the given threshold.  Defaults to 5 for the chi-square tests, and\n"
"    to 0 for the exact ones, which do not need the small-cell correction the\n"
"    threshold stands in for.\n\n"
               );
...
"      inflation factor until none exceeds the threshold.  On chrX, males and\n"
"      females are scored on the same 0/1/2 scale, so chrX results can differ\n"
"      from PLINK 1.9's default ('--ld-xchr 1', which halves the male scale).\n"
