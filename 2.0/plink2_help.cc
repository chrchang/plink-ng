...
"        ['multinomial' | 'multinomial='<test>] ['multinomial-ref='<level>]\n"
"    Basic association analysis on quantitative, case/control, and/or\n"
...
"    * Categorical phenotypes are skipped unless 'multinomial' is specified.\n"
"      Each is then analyzed by multinomial logistic regression, with one\n"
"      omnibus test of the genotype term(s) against all K phenotype levels, on\n"
"      (K-1) x (number of tested columns) degrees of freedom.  Results are\n"
"      written to <output prefix>.<phenotype name>.glm.multinomial (with\n"
"      'no-firth'), .glm.multinomial.hybrid (the default 'firth-fallback'), or\n"
"      .glm.multinomial.firth ('firth').\n"
"      * In the default additive model, a variant gets one row, which tests all\n"
"        its non-omitted alleles jointly (A1 lists them).  With 'dominant',\n"
"        'recessive', 'hetonly', 'genotypic', or 'hethom', each non-omitted\n"
"        allele gets a row, which tests its recoded column(s) with the other\n"
"        non-omitted alleles as additive covariates.\n"
"      * The reference level is the first in natural-sort order; change it with\n"
"        'multinomial-ref='<level>.\n"
"      * 'multinomial=lrt' (the default) requests a likelihood ratio test,\n"
"        'multinomial=wald' a Wald test, and 'multinomial=score' a faster score\n"
"        test.  Treat the latter as a screen: for rare variants in small\n"
"        levels, it can diverge from the likelihood ratio test.\n"
"      * When the values of a genotype column in some level all lie at or below\n"
"        (or at or above) those of every other level, e.g. when a level has no\n"
"        copy of one allele, the maximum-likelihood estimate does not exist.\n"
"        With 'no-firth', likelihood ratio and Wald statistics are then NA,\n"
"        with error code SEPARATION.  UNFINISHED marks a fit that did not\n"
"        converge; its statistics are NA as well.\n"
"      * As for case/control phenotypes, 'firth-fallback' (the default) redoes\n"
"        these variants, and any other whose fit fails, with Firth regression\n"
"        (the multinomial form of Bull SB et al. (2002) Bias reduction in\n"
"        multinomial logistic regression), while 'firth' uses it for every\n"
"        variant.  The likelihood ratio test is then the penalized one, as in\n"
"        R's logistf, and the standard errors come from the inverse of the\n"
"        information matrix with each sample weighted by 1 + its leverage\n"
"        (logistf's convention; with two levels, the results match --glm\n"
"        firth's).  A Firth fit that does not converge is still reported, with\n"
"        error code UNFINISHED.  When a covariate separates the levels, so\n"
"        that the covariate-only fit fails, 'firth-fallback' and 'firth' use\n"
"        Firth regression for every variant.  The score test never uses Firth\n"
"        regression, and cannot be combined with 'firth'.  'firth-residualize',\n"
"        'cc-residualize', and 'single-prec-cc' have no effect here.\n"
"      * The MIN_EXPECTED column, always present, is the smallest expected cell\n"
"        of the allele x level count table under independence (one row per\n"
"        allele in the additive model, A1 versus the other alleles otherwise).\n"
"        Asymptotic p-values are unreliable when it is small (Cochran's rule\n"
"        asks for at least 5).\n"
"      * 'interaction', --parameters, --tests, local covariates, and permutation\n"
...
"    With 'multinomial', a1countcc, totallelecc, and a1freqcc report each\n"
"    phenotype level instead of cases and controls; 'beta' adds a coefficient\n"
"    column per non-reference level (with standard errors and confidence bounds\n"
"    when se/ci are also requested), while 'orbeta' has no effect; and 'tz'\n"
"    reports the chi-square statistic and its degrees of freedom.  Columns with\n"
"    one value per A1 allele or per tested column hold comma-separated lists:\n"
"    A1 alleles in the additive model, (ADD, DOMDEV) for 'genotypic', and\n"
"    (HOM, HET) for 'hethom'.\n\n"
               );
