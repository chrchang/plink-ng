...
int32_t main(int32_t argc, char** argv) {
  ...
  uintptr_t col_ct;
  if (argc != 3) {
    printf("Usage: dbl2text [input filename] [output filename]\n");
    exit(1);
  }
  ...
  if (((uint64_t)row_ct) * row_ct * sizeof(double) != file_size) {
    row_ct = (int32_t)sqrt((double)((int64_t)(file_size / (sizeof(double) / 2))));
    if (((uint64_t)row_ct) * (row_ct + 1) * (sizeof(double) / 2) != file_size) {
      printf("Error: %s's size indicates it is not a binary double-precision matrix.\n", argv[1]);
      goto main_ret_INVALID_FORMAT;
    }
    is_triangle = 1;
  }
