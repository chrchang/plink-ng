...
THREAD_RET_TYPE qassoc_gen_perms_thread(void* arg) {
  ...
  // and populates g_perm_vecstd[] with permutations of those values.  The nth
  // permutation (if n is zero-based) is in
  //   g_perm_vecstd[n * f(g_perm_vec_ct)]..[(n + 1) * f(g_perm_vec_ct) - 1].
  // Also requires g_sfmtp_arr[] and g_assoc_thread_ct to be initialized.
  intptr_t tidx = (intptr_t)arg;
  uint32_t pheno_nm_ct = g_pheno_nm_ct;
 ...
 model_assoc_more_perms:
  ...
  do {
	...
	if (marker_uidx >= chrom_end) {
      ...
      if (load_and_collapse_incl(bedfile, loadbuf_raw, unfiltered_indiv_ct, loadbuf_ptr, pheno_nm_ct, pheno_nm)) {
	goto model_assoc_ret_READ_FAIL;
      }
      if (model_adapt) {
...
int32_t qassoc(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uint32_t model_modifier, uint32_t model_mperm_val, double pfilter, uint32_t mtest_adjust, double adjust_lambda, uintptr_t* marker_exclude, uintptr_t marker_ct, char* marker_ids, uintptr_t max_marker_id_len, uint32_t plink_maxsnp, uint32_t* marker_pos, char* marker_alleles, uintptr_t max_marker_allele_len, uintptr_t* marker_reverse, uint32_t zero_extra_chroms, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uint32_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, uint32_t aperm_min, uint32_t aperm_max, double aperm_alpha, double aperm_beta, double aperm_init_interval, double aperm_interval_slope, uint32_t mperm_save, uint32_t pheno_nm_ct, uintptr_t* pheno_nm, double* pheno_d, uintptr_t* sex_male, uint32_t xmhh_exists, uint32_t nxmhh_exists, uint32_t perm_batch_size) {
  unsigned char* wkspace_mark = wkspace_base;
  uintptr_t unfiltered_indiv_ct4 = (unfiltered_indiv_ct + 3) / 4;
  ...
  if (wkspace_alloc_ul_checked(&g_loadbuf, MODEL_BLOCKSIZE * pheno_nm_ctv2 * sizeof(intptr_t)) ||
      ...
      wkspace_alloc_ul_checked(&indiv_raw_include2, unfiltered_indiv_ctv2 * sizeof(intptr_t)) ||
  ...
  x_code = chrom_info_ptr->x_code;
  y_code = chrom_info_ptr->y_code;
  gender_req = ((x_code != -1) && is_set(chrom_info_ptr->chrom_mask, x_code)) || ((y_code != -1) && is_set(chrom_info_ptr->chrom_mask, y_code));
  if (gender_req) {
 ...
 qassoc_more_perms:
  ...
  do {
	...
	cur_loadbuf = loadbuf_ptr;
      }
      if (fread(cur_loadbuf, 1, unfiltered_indiv_ct4, bedfile) < unfiltered_indiv_ct4) {
	goto qassoc_ret_READ_FAIL;
      }
      if (g_is_haploid) {
	...
	g_adapt_m_table[block_size] = marker_idx2++;
      ...
      if (is_set(marker_reverse, marker_uidx)) {
	reverse_loadbuf((unsigned char*)loadbuf_ptr, pheno_nm_ct);
      }
      mu_table[block_size++] = marker_uidx;
      if (marker_idx + block_size == marker_unstopped_ct) {
	break;
...
int32_t gxe_assoc(FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uintptr_t* marker_exclude, uintptr_t marker_ct, char* marker_ids, uintptr_t max_marker_id_len, uint32_t plink_maxsnp, uintptr_t* marker_reverse, uint32_t zero_extra_chroms, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uintptr_t indiv_ct, uintptr_t* indiv_exclude, uintptr_t* pheno_nm, double* pheno_d, uintptr_t* gxe_covar_nm, uintptr_t* gxe_covar_c, uintptr_t* sex_male, uint32_t xmhh_exists, uint32_t nxmhh_exists) {
  ...
  for (indiv_idx2 = 0; indiv_idx2 < covar_nm_ct; indiv_idx2++) {
    ...
    if (is_set(gxe_covar_c, indiv_idx)) {
      set_bit_noct(group2_include2, indiv_idx2 * 2);
    }
    indiv_idx++;
  }
  ...
  if (xmhh_exists || y_exists) {
	...
	if (is_set(indiv_male_include2, indiv_idx2 * 2)) {
	  ...
	  if (is_set(group2_include2, indiv_idx2 * 2)) {
	    set_bit_noct(group2_male_include2, indiv_idx * 2);
...
int32_t glm_scan_conditions(char* condition_mname, char* condition_fname, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude, uintptr_t marker_ct, char* marker_ids, uintptr_t max_marker_id_len, Chrom_info* chrom_info_ptr, uint32_t xmhh_exists, uint32_t nxmhh_exists, uintptr_t* loadbuf_raw, FILE* bedfile, uintptr_t bed_offset, uintptr_t unfiltered_indiv_ct, uintptr_t* sex_male, uintptr_t* load_mask, uintptr_t* indiv_valid_ct_ptr, uintptr_t* condition_ct_ptr, uint32_t** condition_uidxs_ptr, uintptr_t* indiv_include2, uintptr_t* indiv_male_include2) {
  ...
  if (condition_ct) {
	...
	haploid_fix(xmhh_exists, nxmhh_exists, indiv_include2, indiv_male_include2, unfiltered_indiv_ct, is_x, is_y, (unsigned char*)loadbuf_raw);
      }
...
uint32_t glm_loadbuf_to_doubles(uintptr_t* loadbuf_raw, uintptr_t* load_mask, uintptr_t indiv_valid_ct, double* covar_row, double* geno_map, uintptr_t* cur_missing) {
  ...
  for (indiv_idx = 0; indiv_idx < indiv_valid_ct; indiv_idx++) {
    ...
    if (cur_genotype == 1) {
      set_bit_noct(cur_missing, indiv_idx);
      cur_missing_ct++;
    } else {
...
uint32_t glm_loadbuf_to_doubles_x(uintptr_t* loadbuf_raw, uintptr_t* load_mask, uintptr_t* sex_male, uintptr_t indiv_valid_ct, double* covar_row, double* geno_map, uintptr_t* cur_missing) {
  uintptr_t indiv_uidx = 0;
  uint32_t cur_missing_ct = 0;
...
uint32_t glm_linear_robust_cluster_covar(uintptr_t param_ct, uintptr_t indiv_valid_ct, double* covars_collapsed, double* covars_transposed_buf, double* param_2d_buf, MATRIX_INVERT_BUF1_TYPE* mi_buf, double* param_2d_buf2, uintptr_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, uint32_t* indiv_to_cluster) {
  ...
  // earlier vif check should guarantee matrix is nonsingular?
...
int32_t glm_assoc_nosnp(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uint32_t glm_modifier, double glm_vif_thresh, uint32_t glm_xchr_model, uint32_t glm_mperm_val, Range_list* parameters_range_list_ptr, Range_list* tests_range_list_ptr, double ci_size, double ci_zt, double pfilter, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude, uintptr_t marker_ct, char* marker_ids, uintptr_t max_marker_id_len, uintptr_t* marker_reverse, char* condition_mname, char* condition_fname, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uintptr_t indiv_ct, uintptr_t* indiv_exclude, uint32_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, uint32_t aperm_min, uint32_t aperm_max, double aperm_alpha, double aperm_beta, double aperm_init_interval, double aperm_interval_slope, uint32_t mperm_save, uint32_t pheno_nm_ct, uintptr_t* pheno_nm, uintptr_t* pheno_c, double* pheno_d, uintptr_t covar_ct, char* covar_names, uintptr_t max_covar_name_len, uintptr_t* covar_nm, double* covar_d, uintptr_t* sex_male, uint32_t xmhh_exists, uint32_t nxmhh_exists, uint32_t perm_batch_size) {
  ...
  uint32_t male_ct = 0;
  uint32_t y_exists = (chrom_info_ptr->y_code != -1) && is_set(chrom_info_ptr->chrom_mask, chrom_info_ptr->y_code);
  // uint32_t perm_2success_ct = 0;
  uint32_t perm_attempt_ct = 0;
  // uint32_t tcnt = 0;
  uint32_t regress_fail = 0;
  int32_t retval = 0;
#ifndef NOLAPACK
  char dgels_trans = 'N';
  int32_t dgels_m = 0;
  ...
  char* param_names;
  double* pheno_collapsed;
  double* covars_collapsed; // covariate-major
  double* covars_transposed_buf;
  char* outname_end2;
  char* wptr;
  double* dptr;
  double* dptr2;
  ...
  if (condition_mname || condition_fname) {
    ...
    retval = glm_scan_conditions(condition_mname, condition_fname, unfiltered_marker_ct, marker_exclude, marker_ct, marker_ids, max_marker_id_len, chrom_info_ptr, xmhh_exists, nxmhh_exists, loadbuf_raw, bedfile, bed_offset, unfiltered_indiv_ct, sex_male, load_mask, &indiv_valid_ct, &condition_ct, &condition_uidxs, indiv_include2, indiv_male_include2);
    wkspace_left += topsize;
    if (retval) {
  ...
  if (wkspace_alloc_c_checked(&param_names, param_ct * max_param_name_len) ||
      wkspace_alloc_d_checked(&pheno_collapsed, indiv_valid_ct * sizeof(double)) ||
  ...
  if (wkspace_alloc_d_checked(&param_2d_buf, param_ct * param_ct * sizeof(double)) ||
  ...
  if (ii == -1) {
    ...
    logprint("Warning: Skipping --linear/--logistic nosnp since VIF check failed.\n");
    goto glm_assoc_nosnp_ret_1;
  }
  ...
  if (pheno_d) {
    ...
    if (wkspace_alloc_d_checked(&dgels_a, param_ct * indiv_valid_ct * sizeof(double)) ||
        wkspace_alloc_d_checked(&dgels_b, ulii * indiv_valid_ct * sizeof(double)) ||
    ...
    fill_double_zero(&(dgels_b[indiv_valid_ct]), ulii * ((uint32_t)dgels_ldb) - indiv_valid_ct);
    dgels_nrhs = (int32_t)((uint32_t)ulii);
    ...
    dgels_(&dgels_trans, &dgels_m, &dgels_n, &dgels_nrhs, dgels_a, &dgels_m, dgels_b, &dgels_ldb, dgels_work, &dgels_lwork, &dgels_info);
    if (dgels_info) {
      logprint("Warning: Skipping --linear/--logistic nosnp since regression failed.\n");
      goto glm_assoc_nosnp_ret_1;
    }
#endif
  } else {
  ...
  if (pheno_d) {
    ...
    if (glm_linear_robust_cluster_covar(param_ct, indiv_valid_ct, covars_collapsed, covars_transposed_buf, param_2d_buf, mi_buf, param_2d_buf2, g_cluster_ct, g_cluster_map, g_cluster_starts, g_indiv_to_cluster)) {
      // can this actually happen, or does the earlier VIF check prevent it?
      logprint("Warning: Skipping --linear/--logistic nosnp since X'X is singular.\n");
      goto glm_assoc_nosnp_ret_1;
    }
  }
  ...
  g_adaptive_ci_zt = ltqnorm(1 - aperm_beta / 2.0);
  while (g_perms_done < perms_total) {
    ...
    perm_vec_ctcl8m = (g_perm_vec_ct + (CACHELINE_DBL - 1)) & (~(CACHELINE_DBL - 1));
    // (Note that, for now, the main nosnp regression loop is not
    // multithreaded; only the permutation generation process is.)
    //
    // memory allocation dependencies:
    // g_perm_vecstd: (8 * perm_vec_ct, CL-aligned) * indiv_valid_ct
    // max(T):
    //   g_maxt_thread_results: 8 * perm_vec_ct, CL-aligned
    //   g_mperm_save_all (if needed): 8 * perm_vec_ct
    if (wkspace_alloc_d_checked(&g_perm_vecstd, perm_vec_ctcl8m * sizeof(double) * indiv_valid_ct)) {
      goto glm_assoc_nosnp_ret_NOMEM;
    }
    if (perm_maxt) {
      if (wkspace_alloc_d_checked(&g_maxt_thread_results, perm_vec_ctcl8m * sizeof(double))) {
        goto glm_assoc_nosnp_ret_NOMEM;
      }
      if (mperm_save & MPERM_DUMP_ALL) {
        if (wkspace_alloc_d_checked(&g_mperm_save_all, g_perm_vec_ct * sizeof(double))) {
	  goto glm_assoc_nosnp_ret_NOMEM;
	}
      }
    }
    ...
    if (pheno_d) {
      // todo: linear per-permutation-block stuff
    } else {
