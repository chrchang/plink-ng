...
// can only permute indices for --linear, since 'standard-beta' without
// 'no-snp' may cause the actual phenotype values to vary
static uint32_t* g_perm_indices;
...
THREAD_RET_TYPE qassoc_gen_perms_thread(void* arg) {
  ...
  for (; indiv_idx < pheno_nm_ct; indiv_idx++) {
    tot_quotient = 0x100000000LLU / (indiv_idx + 1);
...
THREAD_RET_TYPE linear_gen_perms_thread(void* arg) {
  // Used by --linear.  (Can't use qassoc_gen_perms_thread() if 'standard-beta'
  // modifier is active without 'no-snp'.)  Requires g_pheno_nm_ct,
  // g_sfmtp_arr, g_assoc_thread_ct, and g_perm_vec_ct to be initialized, and
  // space must be allocated for g_perm_indices.
  //
  // g_perm_indices is permutation-major: the nth permutation is stored from
  //   [n * indiv_valid_ct] to [(n + 1) * indiv_valid_ct - 1]
  // inclusive.
  //
  // todo: check if this is a better way to generate permutations in the
  // qassoc format as well
  intptr_t tidx = (intptr_t)arg;
  ...
  uint32_t* perm_indices = &(g_perm_indices[pmin * indiv_valid_ct]);
  ...
  for (poffset = 0; poffset < pdiff; poffset++) {
    ...
    for (indiv_idx = 1; indiv_idx < indiv_valid_ct; indiv_idx++) {
      ...
      urand %= indiv_idx + 1;
      perm_indices[indiv_idx] = perm_indices[urand];
      perm_indices[urand] = indiv_idx;
    }
    perm_indices = &(perm_indices[indiv_valid_ct]);
  }
  THREAD_RETURN;
}
...
THREAD_RET_TYPE linear_gen_cluster_perms_thread(void* arg) {
  ...
  for (poffset = 0; poffset < pdiff; poffset++) {
	...
	in_cluster_positions[cluster_idx] += 1;
      ...
      if (!cur_in_cluster_pos) {
        perm_indices[indiv_idx] = indiv_idx;
      } else {
	...
	uii = cluster_map[cluster_starts[cluster_idx] + urand];
        perm_indices[indiv_idx] = perm_indices[uii];
	perm_indices[uii] = indiv_idx;
      }
    }
    perm_indices = &(perm_indices[indiv_valid_ct]);
  }
...
int32_t model_assoc(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uint32_t model_modifier, uint32_t model_cell_ct, uint32_t model_mperm_val, double ci_size, double ci_zt, double pfilter, uint32_t mtest_adjust, double adjust_lambda, uintptr_t* marker_exclude, uintptr_t marker_ct, char* marker_ids, uintptr_t max_marker_id_len, uint32_t plink_maxsnp, uint32_t* marker_pos, char* marker_alleles, uintptr_t max_marker_allele_len, uintptr_t* marker_reverse, uint32_t zero_extra_chroms, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uint32_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, uint32_t aperm_min, uint32_t aperm_max, double aperm_alpha, double aperm_beta, double aperm_init_interval, double aperm_interval_slope, uint32_t mperm_save, uint32_t pheno_nm_ct, uintptr_t* pheno_nm, uintptr_t* pheno_c, uintptr_t* sex_male) {
  ...
  if (model_assoc) {
    if (wkspace_alloc_d_checked(&orig_odds, marker_ct * sizeof(double)) ||
        wkspace_alloc_ui_checked(&g_set_cts, marker_ct * sizeof(int32_t))) {
      goto model_assoc_ret_NOMEM;
    }
  }
  if ((!model_assoc) || model_maxt) {
    if (wkspace_alloc_ui_checked(&g_het_cts, marker_ct * sizeof(int32_t)) ||
  ...
  if (model_perms) {
	...
	if (model_assoc || (model_modifier & (MODEL_PDOM | MODEL_PREC))) {
	  if (wkspace_alloc_ui_checked(&g_precomp_ui, g_precomp_width * 6 * MODEL_BLOCKSIZE * sizeof(int32_t)) ||
	      wkspace_alloc_d_checked(&g_precomp_d, g_precomp_width * 3 * MODEL_BLOCKSIZE * sizeof(double))) {
	    goto model_assoc_ret_NOMEM;
	  }
	} else if (model_perm_best) {
	  if (wkspace_alloc_ui_checked(&g_precomp_ui, g_precomp_width * 18 * MODEL_BLOCKSIZE * sizeof(int32_t)) ||
	...
	if (wkspace_alloc_ui_checked(&g_precomp_ui, g_precomp_width * 18 * MODEL_BLOCKSIZE * sizeof(int32_t)) ||
      ...
      if (wkspace_alloc_ui_checked(&g_precomp_ui, g_precomp_width * 4 * MODEL_BLOCKSIZE * sizeof(int32_t))) {
	goto model_assoc_ret_NOMEM;
      }
    } else if (model_perm_best) {
      if (wkspace_alloc_ui_checked(&g_precomp_ui, g_precomp_width * 12 * MODEL_BLOCKSIZE * sizeof(int32_t))) {
	goto model_assoc_ret_NOMEM;
      }
    }
    if (model_perm_best) {
 ...
 qassoc_more_perms:
  ...
  if (!perm_pass_idx) {
	...
	for (uii = 0; uii < marker_ct; uii++) {
      ...
      retval = multcomp(outname, outname_end, marker_idx_to_uidx, marker_ct, marker_ids, max_marker_id_len, plink_maxsnp, zero_extra_chroms, chrom_info_ptr, g_orig_chisq, pfilter, mtest_adjust, adjust_lambda, 1, tcnt);
      if (retval) {
	goto qassoc_ret_1;
      }
    }
...
uint32_t glm_linear_robust_cluster_covar(uintptr_t cur_batch_size, uintptr_t param_ct, uintptr_t indiv_valid_ct, double* covars_cov_major, double* covars_indiv_major, uint32_t* pheno_idx_perms, double* pheno_d2, double* coef, double* param_2d_buf, MATRIX_INVERT_BUF1_TYPE* mi_buf, double* param_2d_buf2, uint32_t cluster_ct1, uint32_t* indiv_to_cluster1, double* cluster_param_buf, double* cluster_param_buf2, double* indiv_1d_buf, double* linear_results, uintptr_t constraint_ct, double* constraints_con_major, double* param_df_buf, double* param_df_buf2, double* df_df_buf, double* df_buf, uint32_t* perm_fail_ct_ptr, uintptr_t* perm_fails) {
  ...
  if (!cluster_ct1) {
	...
	for (param_idx = 0; param_idx < param_ct; param_idx++) {
        ...
        partial -= pheno_d2[*pheno_idx_ptr++];
	dxx += partial * partial;
      }
	...
	*dptr2++ = param_2d_buf[param_idx * param_ct_p1]; // S0[i][i]
    ...
    pheno_idx_ptr = pheno_idx_perms; // Y[i] idx
    for (perm_idx = 0; perm_idx < cur_batch_size; perm_idx++) {
...
uint32_t glm_logistic_robust_cluster_covar(uintptr_t cur_batch_size, uintptr_t param_ct, uintptr_t indiv_valid_ct, double* covars_cov_major, double* covars_indiv_major, uintptr_t* perm_vecs, double* coef, double* vbuf, double* initial_t2_buf, double* t2_buf, double* t3_buf, double* param_2d_buf, MATRIX_INVERT_BUF1_TYPE* mi_buf, double* param_2d_buf2, uint32_t cluster_ct1, uint32_t* indiv_to_cluster1, double* cluster_param_buf, double* cluster_param_buf2, double* indiv_1d_buf, double* logistic_results, uintptr_t constraint_ct, double* constraints_con_major, double* df_df_buf, double* df_buf, uint32_t* perm_fail_ct_ptr, uintptr_t* perm_fails) {
  ...
  for (perm_idx = 0; perm_idx < cur_batch_size; perm_idx++) {
	...
	*dptr++ = -9;
    ...
    coef = &(coef[param_ct]);
    perm_vecs = &(perm_vecs[indiv_valid_ctv2]);
  }
  *perm_fail_ct_ptr = perm_fail_ct;
  return 0;
}

uint32_t glm_fill_design(uintptr_t* loadbuf_collapsed, double* fixed_covars_cov_major, uintptr_t indiv_valid_ct, double* pheno_d2, uint32_t* indiv_to_cluster1, uint32_t cur_param_ct, uint32_t standard_beta, uint32_t hethom, uintptr_t condition_list_start_idx, uintptr_t interaction_start_idx, uintptr_t sex_start_idx, uintptr_t* active_params, uintptr_t* haploid_params, uint32_t include_sex, uint32_t is_nonx_haploid, double* cur_covars_cov_major, double* cur_covars_indiv_major, double* cur_pheno_d2, uint32_t* cur_indiv_to_cluster1_buf, uint32_t** cur_indiv_to_cluster1_ptr) {
  ...
  if (standard_beta) {
    // may want missing_ct == 0 case precomputed
    copy_when_nonmissing(loadbuf_collapsed, (char*)pheno_d2, sizeof(double), indiv_valid_ct, missing_ct, (char*)cur_pheno_d2);
    dxx = g_pheno_sum;
    dyy = g_pheno_ssq;
    if (missing_ct) {
      ulptr = loadbuf_collapsed;
      ulptr_end = &(loadbuf_collapsed[(indiv_valid_ct + (BITCT2 - 1)) / BITCT2]);
      indiv_idx_stop = 0;
      while (ulptr < ulptr_end) {
	cur_word = *ulptr++;
	cur_word = cur_word & (~(cur_word >> 1)) & FIVEMASK;
	while (cur_word) {
          indiv_idx = indiv_idx_stop + (CTZLU(cur_word) / 2);
          dzz = pheno_d2[indiv_idx];
	  dxx -= dzz;
	  dyy -= dzz * dzz;
	  cur_word &= cur_word - 1;
	}
    ...
    dzz = dxx / ((double)((intptr_t)cur_indiv_valid_ct)); // mean
    dyy = sqrt(((double)((intptr_t)(cur_indiv_valid_ct - 1))) / (dyy - dxx * dzz)); // 1 / stdev
    dptr = cur_pheno_d2;
    for (indiv_idx = 0; indiv_idx < cur_indiv_valid_ct; indiv_idx++) {
      *dptr = ((*dptr) - dzz) * dyy;
      dptr++;
...
int32_t glm_assoc(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uint32_t glm_modifier, double glm_vif_thresh, uint32_t glm_xchr_model, uint32_t glm_mperm_val, Range_list* parameters_range_list_ptr, Range_list* tests_range_list_ptr, double ci_size, double ci_zt, double pfilter, uint32_t mtest_adjust, double adjust_lambda, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude, uintptr_t marker_ct, char* marker_ids, uintptr_t max_marker_id_len, uint32_t plink_maxsnp, uint32_t* marker_pos, char* marker_alleles, uintptr_t max_marker_allele_len, uintptr_t* marker_reverse, uint32_t zero_extra_chroms, char* condition_mname, char* condition_fname, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uintptr_t indiv_ct, uintptr_t* indiv_exclude, uint32_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, uint32_t aperm_min, uint32_t aperm_max, double aperm_alpha, double aperm_beta, double aperm_init_interval, double aperm_interval_slope, uint32_t mperm_save, uint32_t pheno_nm_ct, uintptr_t* pheno_nm, uintptr_t* pheno_c, double* pheno_d, uintptr_t covar_ct, char* covar_names, uintptr_t max_covar_name_len, uintptr_t* covar_nm, double* covar_d, uintptr_t* sex_nm, uintptr_t* sex_male, uint32_t hh_exists, uint32_t perm_batch_size) {
  ...
  uint32_t do_perms = perm_adapt | perm_maxt;
  uint32_t fill_orig_chiabs = do_perms || mtest_adjust;
  uint32_t marker_unstopped_ct = marker_ct;
  ...
  if (!((glm_modifier & GLM_NO_X_SEX) || ((!sex_covar_everywhere) && (!x_present)))) {
    ...
    // for sex; --allow-no-sex may still have an effect on other analyses in
    // the current run)
    do {
  ...
  if ((genotypic_or_hethom && ((active_params[0] & 6) == 6)) || tests_range_list_ptr->name_ct || (glm_modifier & GLM_TEST_ALL)) {
    if (wkspace_alloc_ul_checked(&joint_test_params, param_ctl_max * sizeof(intptr_t))) {
      goto glm_assoc_ret_NOMEM;
    }
    fill_ulong_zero(joint_test_params, param_ctl_max);
    if (tests_range_list_ptr->name_ct) {
      numeric_range_list_to_bitfield(tests_range_list_ptr, param_ct_max - 1, joint_test_params, 1, 1);
      constraint_ct_max = popcount_longs(joint_test_params, 0, param_ctl_max);
    } else if (glm_modifier & GLM_TEST_ALL) {
      constraint_ct_max = param_ct_max - 1;
      fill_bits(joint_test_params, 0, constraint_ct_max);
    } else {
      // genotypic/hethom, neither of first two terms excluded by --parameters,
      // no --tests
      constraint_ct_max = 2;
      joint_test_params[0] = 3;
	...
	max_param_name_len = slen;
      ...
      wkspace_reset((unsigned char*)joint_test_params);
  ...
  if (wkspace_alloc_d_checked(&orig_stats, marker_ct * param_ctx_max_m1 * sizeof(double)) ||
      wkspace_alloc_c_checked(&param_names, param_ctx_max * max_param_name_len) ||
      wkspace_alloc_d_checked(&fixed_covars_cov_major, (variation_in_sex + interaction_start_idx - condition_list_start_idx) * indiv_valid_ct * sizeof(double)) ||
      wkspace_alloc_uc_checked(&g_perm_adapt_stop, marker_ct) ||
      // g_set_cts repurposed as nonmissing count
      wkspace_alloc_ui_checked(&g_set_cts, marker_ct * sizeof(int32_t))) {
    goto glm_assoc_ret_NOMEM;
  }
  ...
  for (param_idx_fixed = 0; param_idx_fixed < condition_ct; param_idx_fixed++) {
    ...
    if (!g_is_x) {
      glm_loadbuf_to_doubles(g_loadbuf, indiv_valid_ct, &(fixed_covars_cov_major[param_idx_fixed * indiv_valid_ct]), geno_map_ptr, NULL);
    } else {
      glm_loadbuf_to_doubles_x(g_loadbuf, sex_male_collapsed, indiv_valid_ct, &(fixed_covars_cov_major[param_idx_fixed * indiv_valid_ct]), geno_map_ptr, NULL);
    }
    if (is_set(active_params, uii + condition_list_start_idx)) {
  ...
  for (uii = 0; uii < covar_ct; param_idx_fixed++, uii++) {
    ...
    indiv_uidx = 0;
    dptr = &(fixed_covars_cov_major[param_idx_fixed * indiv_valid_ct]);
    dptr2 = &(covar_d[uii]);
    for (indiv_idx = 0; indiv_idx < indiv_ct; indiv_uidx++, indiv_idx++) {
  ...
  if (fill_orig_chiabs) {
	...
	fill_double_zero(g_maxt_extreme_stat, perms_total);
	if (mperm_save & MPERM_DUMP_ALL) {
	  if (wkspace_alloc_d_checked(&mperm_save_stats, marker_ct * perm_batch_size * sizeof(double))) {
	    goto glm_assoc_ret_NOMEM;
	  }
	  memcpy(outname_end, ".mperm.dump.all", 16);
	  if (fopen_checked(&outfile_msa, outname, "w")) {
  ...
  if (cluster_starts) {
    ...
    if (cluster_ct1) {
      ...
      if (wkspace_alloc_ui_checked(&indiv_to_cluster1, indiv_valid_ct * sizeof(int32_t))) {
	goto glm_assoc_ret_NOMEM;
      }
  ...
  for (tidx = 0; tidx < g_thread_ct; tidx++) {
    ...
    dptr = g_glm_mt[tidx].cur_covars_cov_major;
    if (cluster_ct1) {
	...
	dgels_(&dgels_trans, &dgels_m, &dgels_n, &dgels_nrhs, g_glm_mt[0].dgels_a, &dgels_m, g_glm_mt[0].dgels_b, &dgels_ldb, &dxx, &dgels_lwork, &dgels_info);
	if (dxx > 2147483647.0) {
	...
	dgels_lwork = (int32_t)dxx;
      ...
      if (wkspace_alloc_d_checked(&(g_glm_mt[tidx].dgels_work), dgels_lwork * sizeof(double))) {
	goto glm_assoc_ret_NOMEM;
      }
      if (standard_beta) {
        if (wkspace_alloc_d_checked(&(g_glm_mt[tidx].pheno_d2), indiv_valid_ct * sizeof(double))) {
          goto glm_assoc_ret_NOMEM;
        }
      } else {
	g_glm_mt[tidx].pheno_d2 = g_pheno_d2;
      }
    } else {
#endif
  ...
  if (pheno_d) {
    ...
    if (do_perms) {
      if (wkspace_alloc_ui_checked(&g_precomputed_mods, (indiv_valid_ct - 1) * sizeof(int32_t)) ||
          wkspace_alloc_ui_checked(&g_perm_indices, perm_batch_size * indiv_valid_ct * sizeof(int32_t))) {
	goto glm_assoc_ret_NOMEM;
      }
      precompute_mods(indiv_valid_ct, g_precomputed_mods);
    }
    if (wkspace_alloc_ui_checked(&unpermuted_indices, indiv_valid_ct * sizeof(int32_t))) {
      goto glm_assoc_ret_NOMEM;
    }
    for (indiv_idx = 0; indiv_idx < indiv_valid_ct; indiv_idx++) {
      unpermuted_indices[indiv_idx] = indiv_idx;
    }
  } else {
#endif
 ...
 glm_assoc_more_perms:
  ...
  do {
	...
	wptr_start = memcpya(tbuf, "   0", 4);
      } else {
	wptr_start = fw_strcpy(4, chrom_info_ptr->nonstd_names[uii], tbuf);
      }
      *wptr_start++ = ' ';
	...
	if (constraint_ct_max) {
          cur_constraint_ct = popcount_bit_idx(joint_test_params, 0, constraint_ct_max - np_sex);
	} else {
	  cur_constraint_ct = 0;
	}
	cur_param_names = param_names;
	...
	if (constraint_ct_max) {
	  for (ulii = 0; ulii < param_ctl_max; ulii++) {
	    uljj = joint_test_params[ulii] & haploid_params[ulii];
	    while (uljj) {
	...
	// 1. fill design matrix and make transposed copy; fill g_set_cts[] in
	//    the process
	...
	marker_idx2 = g_adapt_m_table[marker_bidx];
        cur_indiv_valid_ct = indiv_valid_ct - glm_fill_design(loadbuf_ptr, fixed_covars_cov_major, indiv_valid_ct, g_pheno_d2, indiv_to_cluster1, cur_param_ct, standard_beta, hethom, condition_list_start_idx, interaction_start_idx, sex_start_idx, active_params, haploid_params, include_sex, g_is_haploid && (!g_is_x), g_glm_mt[0].cur_covars_cov_major, g_glm_mt[0].cur_covars_indiv_major, g_glm_mt[0].pheno_d2, g_glm_mt[0].cur_indiv_to_cluster1_buf, &(g_glm_mt[0].cur_indiv_to_cluster1));
	g_set_cts[marker_idx2] = cur_indiv_valid_ct;
	if (cur_indiv_valid_ct > cur_param_ct) {
	  ...
	  if (pheno_d) {
	    memcpy(g_glm_mt[0].dgels_a, g_glm_mt[0].cur_covars_cov_major, cur_param_ct * cur_indiv_valid_ct * sizeof(double));
            memcpy(g_glm_mt[0].dgels_b, g_glm_mt[0].pheno_d2, cur_indiv_valid_ct * sizeof(double));
            dgels_(&dgels_trans, &dgels_m, &dgels_n, &dgels_nrhs, g_glm_mt[0].dgels_a, &dgels_m, g_glm_mt[0].dgels_b, &dgels_ldb, g_glm_mt[0].dgels_work, &dgels_lwork, &dgels_info);
	    if (glm_linear_robust_cluster_covar(1, cur_param_ct, cur_indiv_valid_ct, g_glm_mt[0].cur_covars_cov_major, g_glm_mt[0].cur_covars_indiv_major, unpermuted_indices, g_glm_mt[0].pheno_d2, g_glm_mt[0].dgels_b, g_glm_mt[0].param_2d_buf, g_glm_mt[0].mi_buf, g_glm_mt[0].param_2d_buf2, cluster_ct1, g_glm_mt[0].cur_indiv_to_cluster1, g_glm_mt[0].cluster_param_buf, g_glm_mt[0].cluster_param_buf2, g_glm_mt[0].indiv_1d_buf, g_glm_mt[0].regression_results, cur_constraint_ct, constraints_con_major, g_glm_mt[0].param_df_buf, g_glm_mt[0].param_df_buf2, g_glm_mt[0].df_df_buf, g_glm_mt[0].df_buf, &perm_fail_ct, g_glm_mt[0].perm_fails) || perm_fail_ct) {
	      regression_fail = 1;
	    }
	  } else {
#endif
	    if (glm_logistic_robust_cluster_covar(1, cur_param_ct, cur_indiv_valid_ct, g_glm_mt[0].cur_covars_cov_major, g_glm_mt[0].cur_covars_indiv_major, pheno_c_collapsed, g_glm_mt[0].logistic_coef, g_glm_mt[0].logistic_vbuf, g_glm_mt[0].logistic_initial_t2_buf, g_glm_mt[0].logistic_t2_buf, g_glm_mt[0].logistic_t3_buf, g_glm_mt[0].param_2d_buf, g_glm_mt[0].mi_buf, g_glm_mt[0].param_2d_buf2, cluster_ct1, g_glm_mt[0].cur_indiv_to_cluster1, g_glm_mt[0].cluster_param_buf, g_glm_mt[0].cluster_param_buf2, g_glm_mt[0].indiv_1d_buf, g_glm_mt[0].regression_results, cur_constraint_ct, constraints_con_major, g_glm_mt[0].df_df_buf, g_glm_mt[0].df_buf, &perm_fail_ct, g_glm_mt[0].perm_fails) || perm_fail_ct) {
	      regression_fail = 1;
	    }
#ifndef NOLAPACK
	  }
	...
	*wptr_start2++ = ' ';
	...
	if (max_marker_allele_len == 1) {
	  wptr_start2 = memseta(wptr_start2, 32, 4);
	  *wptr_start2++ = marker_alleles[marker_uidx2 * 2];
	} else {
	  *wptr_start2++ = ' ';
	  wptr_start2 = fw_strcpy(4, &(marker_alleles[marker_uidx2 * 2 * max_marker_allele_len]), wptr_start2);
	}
	*wptr_start2++ = ' ';
	orig_stats_ptr = &(orig_stats[marker_idx2 * param_ctx_max_m1]);
	if (!regression_fail) {
#ifndef NOLAPACK
	  if (pheno_d) {
	    for (param_idx = 1; param_idx < cur_param_ct; param_idx++) {
	  ...
	  g_perm_adapt_stop[marker_idx2] = 1;
    ...
    if (do_perms) {
      // split loaded-and-unfinished markers evenly between threads.  each
      // thread does the following for every marker in its set:
      // 1. fill design matrix and make transposed copy, do this more
      //    efficiently if g_set_cts[] indicates there are no missing genotypes
...
int32_t glm_assoc_nosnp(pthread_t* threads, FILE* bedfile, uintptr_t bed_offset, char* outname, char* outname_end, uint32_t glm_modifier, double glm_vif_thresh, uint32_t glm_xchr_model, uint32_t glm_mperm_val, Range_list* parameters_range_list_ptr, Range_list* tests_range_list_ptr, double ci_size, double ci_zt, double pfilter, uintptr_t unfiltered_marker_ct, uintptr_t* marker_exclude, uintptr_t marker_ct, char* marker_ids, uintptr_t max_marker_id_len, uintptr_t* marker_reverse, char* condition_mname, char* condition_fname, Chrom_info* chrom_info_ptr, uintptr_t unfiltered_indiv_ct, uintptr_t indiv_ct, uintptr_t* indiv_exclude, uint32_t cluster_ct, uint32_t* cluster_map, uint32_t* cluster_starts, uint32_t mperm_save, uint32_t pheno_nm_ct, uintptr_t* pheno_nm, uintptr_t* pheno_c, double* pheno_d, uintptr_t covar_ct, char* covar_names, uintptr_t max_covar_name_len, uintptr_t* covar_nm, double* covar_d, uintptr_t* sex_nm, uintptr_t* sex_male, uint32_t hh_exists, uint32_t perm_batch_size) {
  ...
  if (pheno_d) {
    ...
    dgels_nrhs = perm_batch_size;
    dgels_ldb = dgels_m;
    if (wkspace_alloc_ui_checked(&g_perm_indices, perm_batch_size * indiv_valid_ct * sizeof(int32_t)) ||
  ...
  while (perms_done < glm_mperm_val) {
	...
	model_assoc_gen_cluster_perms_thread((void*)ulii);
      ...
      join_threads(threads, g_assoc_thread_ct);
      if (glm_logistic_robust_cluster_covar(cur_batch_size, param_ct, indiv_valid_ct, covars_cov_major, covars_indiv_major, g_perm_vecs, logistic_coef, logistic_vbuf, logistic_initial_t2_buf, logistic_t2_buf, logistic_t3_buf, param_2d_buf, mi_buf, param_2d_buf2, cluster_ct1, indiv_to_cluster1, cluster_param_buf, cluster_param_buf2, indiv_1d_buf, regression_results, constraint_ct, constraints_con_major, df_df_buf, df_buf, &perm_fail_ct, perm_fails)) {
	perm_fail_ct = cur_batch_size;
	fill_bits(perm_fails, 0, cur_batch_size);
      }
      perm_fail_total += perm_fail_ct;
