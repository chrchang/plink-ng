...
int32_t plink(char* outname, char* outname_end, char* pedname, char* mapname, char* famname, char* cm_map_fname, char* cm_map_chrname, char* phenoname, char* extractname, char* excludename, char* keepname, char* removename, char* keepfamname, char* removefamname, char* filtername, char* freqname, char* read_dists_fname, char* read_dists_id_fname, char* evecname, char* mergename1, char* mergename2, char* mergename3, char** missing_mid_templates, char* missing_marker_id_match, char* makepheno_str, char* phenoname_str, Two_col_params* a1alleles, Two_col_params* a2alleles, char* recode_allele_name, char* covar_fname, char* update_alleles_fname, char* read_genome_fname, Two_col_params* update_chr, Two_col_params* update_cm, Two_col_params* update_map, Two_col_params* update_name, char* update_ids_fname, char* update_parents_fname, char* update_sex_fname, char* loop_assoc_fname, char* flip_fname, char* flip_subset_fname, char* indiv_sort_fname, char* filtervals_flattened, char* condition_mname, char* condition_fname, char* filter_attrib_fname, char* filter_attrib_liststr, char* filter_attrib_indiv_fname, char* filter_attrib_indiv_liststr, double thin_keep_prob, uint32_t thin_keep_ct, uint32_t min_bp_space, uint32_t mfilter_col, uint32_t fam_cols, int32_t missing_pheno, char* output_missing_pheno, uint32_t mpheno_col, uint32_t pheno_modifier, Chrom_info* chrom_info_ptr, Oblig_missing_info* om_ip, Family_info* fam_ip, double check_sex_fthresh, double check_sex_mthresh, uint32_t check_sex_f_yobs, uint32_t check_sex_m_yobs, double exponent, double min_maf, double max_maf, double geno_thresh, double mind_thresh, double hwe_thresh, double tail_bottom, double tail_top, uint64_t misc_flags, uint64_t filter_flags, uint64_t calculation_type, uint32_t dist_calc_type, uintptr_t groupdist_iters, uint32_t groupdist_d, uintptr_t regress_iters, uint32_t regress_d, uint32_t parallel_idx, uint32_t parallel_tot, uint32_t splitx_bound1, uint32_t splitx_bound2, uint32_t ppc_gap, uint32_t sex_missing_pheno, uint32_t update_sex_col, uint32_t hwe_modifier, uint32_t genome_modifier, double genome_min_pi_hat, double genome_max_pi_hat, Homozyg_info* homozyg_ptr, Cluster_info* cluster_ptr, uint32_t neighbor_n1, uint32_t neighbor_n2, Set_info* sip, Ld_info* ldip, Epi_info* epi_ip, Clump_info* clump_ip, Rel_info* relip, Score_info* sc_ip, uint32_t recode_modifier, uint32_t allelexxxx, uint32_t merge_type, uint32_t indiv_sort, int32_t marker_pos_start, int32_t marker_pos_end, uint32_t snp_window_size, char* markername_from, char* markername_to, char* markername_snp, Range_list* snps_range_list_ptr, uint32_t covar_modifier, Range_list* covar_range_list_ptr, uint32_t write_covar_modifier, uint32_t write_covar_dummy_max_categories, uint32_t mwithin_col, uint32_t model_modifier, uint32_t model_cell_ct, uint32_t model_mperm_val, uint32_t glm_modifier, double glm_vif_thresh, uint32_t glm_xchr_model, uint32_t glm_mperm_val, Range_list* parameters_range_list_ptr, Range_list* tests_range_list_ptr, double ci_size, double pfilter, uint32_t mtest_adjust, double adjust_lambda, uint32_t gxe_mcovar, Aperm_info* apip, uint32_t mperm_save, uintptr_t ibs_test_perms, uint32_t perm_batch_size, double lasso_h2, double lasso_minlambda, Range_list* lasso_select_covars_range_list_ptr, uint32_t testmiss_modifier, uint32_t testmiss_mperm_val, Ll_str** file_delete_list_ptr) {
  ...
  if (calculation_type & CALC_FREQ) {
	...
	memcpy(outname_end, ".frq", 5);
      ...
      retval = write_freqs(outname, plink_maxsnp, unfiltered_marker_ct, marker_exclude, set_allele_freqs, zero_extra_chroms, chrom_info_ptr, marker_ids, max_marker_id_len, marker_allele_ptrs, max_marker_allele_len, hwe_ll_allfs, hwe_lh_allfs, hwe_hh_allfs, hwe_hapl_allfs, hwe_haph_allfs, indiv_f_ct, indiv_f_male_ct, misc_flags, marker_reverse);
    }
  ...
  if (calculation_type & CALC_TESTMISHAP) {
    ...
    retval = test_mishap(bedfile, bed_offset, outname, outname_end, unfiltered_marker_ct, marker_exclude, marker_reverse, marker_ct, marker_ids, max_marker_id_len, plink_maxsnp, marker_allele_ptrs, min_maf, chrom_info_ptr, unfiltered_indiv_ct, indiv_exclude, indiv_ct);
    if (retval) {
      goto plink_ret_1;
    }
  }
  ...
  if (calculation_type & CALC_REGRESS_PCS) {
    retval = calc_regress_pcs(evecname, regress_pcs_modifier, max_pcs, bedfile, bed_offset, marker_ct, unfiltered_marker_ct, marker_exclude, marker_reverse, marker_ids, max_marker_id_len, marker_allele_ptrs, zero_extra_chroms, chrom_info_ptr, marker_pos, indiv_ct, unfiltered_indiv_ct, indiv_exclude, person_ids, max_person_id_len, sex_nm, sex_male, pheno_d, missing_phenod, outname, outname_end, hh_exists);
    if (retval) {
      goto plink_ret_1;
    }
  }
  ...
  */
  ...
  if (calculation_type & (CALC_MODEL | CALC_GXE | CALC_GLM | CALC_LASSO | CALC_CMH | CALC_HOMOG | CALC_TESTMISS | CALC_TDT)) {
    ...
    *outname_end = '.';
	...
	retval = gxe_assoc(bedfile, bed_offset, outname, outname_end, marker_exclude, marker_ct, marker_ids, max_marker_id_len, plink_maxsnp, marker_reverse, zero_extra_chroms, chrom_info_ptr, unfiltered_indiv_ct, indiv_ct, indiv_exclude, pheno_nm, pheno_d, gxe_covar_nm, gxe_covar_c, sex_male, hh_exists);
	if (retval) {
	  goto plink_ret_1;
	}
      }
...
int32_t main(int32_t argc, char** argv) {
  ...
  do {
	...
	if ((param_ct == 2) && (indiv_sort != INDIV_SORT_FILE)) {
          sprintf(logbuf, "Error: '--indiv-sort %s' does not accept a second parameter.%s", argv[cur_arg + 1], errstr_append);
	  goto main_ret_INVALID_CMDLINE_3;
	}
	...
	if (scan_double(argv[cur_arg + uii], &family_info.mendel_max_trio_error) || (family_info.mendel_max_trio_error < 0.0) || (family_info.mendel_max_trio_error > 1.0)) {
	  sprintf(logbuf, "Error: Invalid --me maximum per-trio error rate '%s'.%s", argv[cur_arg + uii], errstr_append);
	  goto main_ret_INVALID_CMDLINE_3;
	}
	if (scan_double(argv[cur_arg + ujj], &family_info.mendel_max_var_error) || (family_info.mendel_max_var_error < 0.0) || (family_info.mendel_max_var_error > 1.0)) {
	  sprintf(logbuf, "Error: Invalid --me maximum per-variant error rate '%s'.%s", argv[cur_arg + ujj], errstr_append);
	  goto main_ret_INVALID_CMDLINE_3;
	}
...
#else
	if (rel_info.modifier & REL_CALC_COV) {
	  sprintf(logbuf, "Error: --pca flag cannot coexist with a covariance matrix calculation.%s", errstr_append);
	  goto main_ret_INVALID_CMDLINE_3;
	} else if (parallel_tot > 1) {
	  sprintf(logbuf, "Error: --parallel and --pca cannot be used together.%s", errstr_append);
	  goto main_ret_INVALID_CMDLINE_3;
	} else if (rel_info.modifier & REL_CALC_SINGLE_PREC) {
	  sprintf(logbuf, "Error: --pca flag cannot be used with a single-precision relationship matrix\ncalculation.%s", errstr_append);
	  goto main_ret_INVALID_CMDLINE_3;
	}
        if (enforce_param_ct_range(param_ct, argv[cur_arg], 0, 4)) {
          goto main_ret_INVALID_CMDLINE_3;
	...
	if (!(calculation_type & CALC_PCA)) {
	  sprintf(logbuf, "Error: --pca-cluster-names must be used with --pca.%s", errstr_append);
	  goto main_ret_INVALID_CMDLINE_3;
	}
	if (enforce_param_ct_range(param_ct, argv[cur_arg], 1, 0x7fffffff)) {
          goto main_ret_INVALID_CMDLINE_3;
	...
	if (!(calculation_type & CALC_PCA)) {
	  sprintf(logbuf, "Error: --pca-clusters must be used with --pca.%s", errstr_append);
	  goto main_ret_INVALID_CMDLINE_3;
	}
        if (enforce_param_ct_range(param_ct, argv[cur_arg], 1, 1)) {
          goto main_ret_INVALID_CMDLINE_3;
	...
	if (score_info.data_fname) {
	  ...
	  for (uii = 3; uii <= param_ct; uii++) {
		...
		if ((uint32_t)ii == score_info.data_varid_col) {
	          sprintf(logbuf, "Error: --q-score-range variant ID and data column numbers cannot match.%s", errstr_append);
	          goto main_ret_INVALID_CMDLINE_3;
		}
	...
	*/
	...
	for (uii = 1; uii <= param_ct; uii++) {
	    ...
	    main_r2_matrix_conflict:
              sprintf(logbuf, "Error: --r/--r2 '%s' cannot be used with matrix output.%s", (ld_info.modifier & LD_INTER_CHR)? "inter-chr" : ((ld_info.modifier & LD_INPHASE)? "in-phase" : ((ld_info.modifier & LD_DPRIME)? "dprime" : "with-freqs")), errstr_append);
	      goto main_ret_INVALID_CMDLINE_3;
	    }
	...
	if (missing_mid_templates[0] && (!strcmp(missing_mid_templates[0], argv[cur_arg + 1]))) {
	  // users can still shoot themselves in the foot by splitting this
	  // across two runs, but at least it's harder
	  sprintf(logbuf, "Error: --set-missing-snp-ids and --set-missing-nonsnp-ids cannot be given the\nsame template string.  Use --set-missing-var-ids if you're sure there are no\nposition overlaps.%s", errstr_append);
	  goto main_ret_INVALID_CMDLINE_3;
	}
	...
	if (missing_mid_templates[0] || missing_mid_templates[1]) {
	  sprintf(logbuf, "Error: --set-missing-var-ids cannot be used with --set-missing-snp-ids or\n--set-missing-nonsnp-ids.%s", errstr_append);
	  goto main_ret_INVALID_CMDLINE_3;
	}
        if (enforce_param_ct_range(param_ct, argv[cur_arg], 1, 1)) {
          goto main_ret_INVALID_CMDLINE_3;
	...
	ujj = 0; // number of numeric parameters so far
	    ...
	    if (score_info.modifier & SCORE_NO_MEAN_IMPUTATION) {
	      sprintf(logbuf, "Error: --score 'sum' and 'no-mean-imputation' modifiers cannot be used\ntogether.%s", errstr_append);
              goto main_ret_INVALID_CMDLINE_3;
	    }
	    ...
	    if (score_info.modifier & SCORE_SUM) {
	      ...
	      sprintf(logbuf, "Error: --score 'center' and 'no-mean-imputation' modifiers cannot be used\ntogether.%s", errstr_append);
              goto main_ret_INVALID_CMDLINE_3;
	    }
	    ...
	    if (!ujj) {
	      ...
	      if ((uint32_t)ii == score_info.varid_col) {
	        sprintf(logbuf, "Error: --score variant ID and allele column numbers cannot match.%s", errstr_append);
	        goto main_ret_INVALID_CMDLINE_3;
	      }
	      ...
	      if ((uint32_t)ii == score_info.varid_col) {
	        sprintf(logbuf, "Error: --score variant ID and effect column numbers cannot match.%s", errstr_append);
	        goto main_ret_INVALID_CMDLINE_3;
	      } else if ((uint32_t)ii == score_info.allele_col) {
	        sprintf(logbuf, "Error: --score allele and effect column numbers cannot match.%s", errstr_append);
	        goto main_ret_INVALID_CMDLINE_3;
	      }
	...
	if (!(calculation_type & CALC_SCORE)) {
	  ...
	  sprintf(logbuf, "Error: --score-no-mean-imputation cannot be used with --score 'center'\nmodifier.%s", errstr_append);
          goto main_ret_INVALID_CMDLINE_3;
	}
	...
	if (!(calculation_type & CALC_LD)) {
	  sprintf(logbuf, "Error: --with-freqs must be used with --r/--r2.%s", errstr_append);
	  goto main_ret_INVALID_CMDLINE_3;
	}
        ld_info.modifier |= LD_WITH_FREQS;
	if (ld_info.modifier & (LD_MATRIX_SHAPEMASK | LD_MATRIX_BIN | LD_MATRIX_SPACES)) {
  ...
  if (score_info.data_fname) {
    if (!score_info.range_fname) {
      ...
      sprintf(logbuf, "Error: --q-score-range must be used with --score.%s", errstr_append);
      goto main_ret_INVALID_CMDLINE_3;
    }
  }
 ...
 main_ret_1:
  ...
  free_cond(missing_mid_templates[0]);
  free_cond(missing_mid_templates[1]);
