...
, "w", stdout)) {
  ...
  do {
	...
	rplugin_port = atoi(argv[cur_arg + 1]);
	...
	goto main_param_zero;
        ...
        retval = alloc_string(&rplugin_host, argv[cur_arg + 1]);
        if(retval)
        	goto main_ret_INVALID_CMDLINE_WWA;
      } else {
	goto main_ret_INVALID_CMDLINE_UNRECOGNIZED;
