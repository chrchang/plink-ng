...
However, if the external-index-file representation is used, sequential reading,
...
\item \texttt{0x10} is the standard PLINK 2 format, with variable-width variant
  records.
\item \texttt{0x20} is the external-index-file PLINK 2 format.  The index file
  starts with \texttt{0x6c 0x1b 0x30}.
\end{itemize}

Mode values \texttt{0x05..0x0f}, \texttt{0x11..0x1f}, \texttt{0x21..0x2f}, and
\texttt{0x31..0x7f} are reserved for use by future versions of this
...
If the storage mode isn't \texttt{0x01} or \texttt{0x20}, the next eight bytes
are the number of variants (as a little-endian \texttt{uint32}; everything else
in this specification is also little-endian), followed by the number of samples
as a \texttt{uint32} (call this $N$).  This is followed by a byte describing
how the rest of the header is formatted.

If the storage mode is \texttt{0x20}, there is no more information in that
file's header.  The external index file is formatted just like a PGEN header
(except with third byte \texttt{0x30}).
...
\item Bits 0-3 of the twelfth byte indicate how variant-record types and
  ...
  \item \sout{\texttt{8}: 0 bits per record type, 2 bits per record length
    ($\ell -\lceil N/4\rceil$ is stored when the record length is $\ell$
    byte(s)).  Record type is inferred as 0 when record length is minimal, and
    8 (i.e. multiallelic-variant hard-call) when record length has 1-3 extra
    bytes.  This is designed for single-sample filesets sharing a single PVAR.}
  \item \sout{\texttt{9}: 0 bits per record type, 4 bits per record length
    ($\ell -\lceil N/4\rceil$ stored when record length is $\ell$).  Record
    type is inferred as 0 when record length is minimal, and 8 when
    record-length has 1-15 extra bytes.}
  \end{itemize}
