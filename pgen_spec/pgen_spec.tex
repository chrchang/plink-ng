...
1.5, every other even sample ID has an ALT allele dosage of 0.75 on a 0..2
scale, and no odd sample IDs have associated dosages.  Then a bitarray is a
more compact way to represent the sample ID list than a difflist, so bits 5 and
6 of the variant record type would normally be set.  Auxiliary data track \#3
would contain 61047 bytes with value \texttt{0x55}, followed by a byte with
...
All initial lines starting with '\#' are part of the header.
...
\item The last header line must start with ``\#FID'' or ``\#IID'', and no
  earlier header lines can start this way.
...
\item The usual PLINK 1 encoding of binary phenotypes is control = 1, case = 2,
  ...
  preferred representation of missing values).  Therefore, PLINK 2 defaults to
  interpreting any phenotype column containing only these values as binary, and
  any phenotype column containing a numeric value outside \{-9, 0, 1, 2\} as
  quantitative.
\end{itemize}
